...
router.post("/", authenticate, async (req: any, res) => {
  try {
    ...
    const { 
      supplierName, productDescription, countryOfOrigin, 
      carrier, brokerId, originPort, destinationPort,
      alias, estimatedDeliveryDate, clientId
    } = req.body;
...
router.post("/:id/track", authenticate, async (req: any, res) => {
  try {
    ...
    if (newStatus !== shipment.status) {
      ...
      const eventType = newStatus === "cleared" ? ShipmentEvents.ARRIVED : 
                        newStatus === "in_transit" ? ShipmentEvents.TRANSIT : ShipmentEvents.UPDATED;
...
router.put("/:id", authenticate, async (req: any, res) => {
  try {
    const { 
      status, brokerId, alias, estimatedDeliveryDate, clientId,
      supplierName, productDescription, countryOfOrigin,
      originPort, destinationPort, carrier
    } = req.body;
    ...
    const newHistory = status && !currentHistory.includes(status) 
