...
export default function Simulator() {
    ...
    const fetchBaseline = async () => {
      try {
        ...
        const saudiEmployees = employees.filter(
          (e) => e.nationality === "سعودي" || e.nationality === "Saudi"
        ).length;
  ...
  const calculateHiringImpact = () => {
    ...
    const additionalPayroll =
      (hiringParams.saudiCount + hiringParams.expatCount) * hiringParams.averageSalary;
    const gosiEmployer =
      hiringParams.saudiCount * hiringParams.averageSalary * 0.11 +
      hiringParams.expatCount * hiringParams.averageSalary * 0.02; // 11% saudi, 2% expat

    const newComplianceScore = Math.min(
      100,
      Math.max(
        0,
        baseline.complianceScore + hiringParams.saudiCount * 2 - hiringParams.expatCount * 1
      )
    );
  ...
  const calculateSalaryImpact = () => {
    ...
    const affectedRatio =
      baseline.totalEmployees > 0
        ? Math.min(1, salaryParams.affectedCount / baseline.totalEmployees)
        : 0;
    ...
    const saudiRatio =
      baseline.totalEmployees > 0 ? baseline.saudiEmployees / baseline.totalEmployees : 0;
    ...
    <div className="space-y-6">
      <div className="bg-zinc-900 text-white rounded-[2rem] p-8 shadow-xl">
        ...
        <p className="text-zinc-400 text-sm max-w-2xl leading-relaxed">
          قم بمحاكاة قرارات التوظيف، زيادة الرواتب، أو التوسع، وشاهد التأثير المباشر على نسبة
          التوطين (نطاقات)، تكلفة الرواتب (تأمينات)، ومستوى المخاطرة بشكل استباقي.
        </p>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar border-b border-zinc-200 pb-2">
        {(["hiring", "salary", "expansion"] as const).map((tab) => (
          ...
          >
            {tab === "hiring"
              ? "محاكاة التوظيف (نطاقات)"
              : tab === "salary"
                ? "تعديل الرواتب والمزايا"
                : "محاكاة توسع الفروع"}
          </button>
        ))}
      </div>
        ...
        >
          <div className="lg:col-span-4 space-y-6">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    min="0"
                    max="20"
                    value={hiringParams.saudiCount}
                    onChange={(e) =>
                      setHiringParams((prev) => ({ ...prev, saudiCount: parseInt(e.target.value) }))
                    }
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    min="0"
                    max="20"
                    value={hiringParams.expatCount}
                    onChange={(e) =>
                      setHiringParams((prev) => ({ ...prev, expatCount: parseInt(e.target.value) }))
                    }
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-zinc-700">
                    متوسط الراتب المتوقع (ريال)
                  </label>
                  <input
                    type="number"
                    step="500"
                    value={hiringParams.averageSalary}
                    onChange={(e) =>
                    ...
                    className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-zinc-50 p-6 rounded-[2rem] border border-zinc-200 shadow-sm">
              <h4 className="font-bold border-b border-zinc-200 pb-2 mb-4 text-xs">
                الوضع الحالي (قبل المحاكاة)
              </h4>
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-bold">
                    {((baseline.saudiEmployees / baseline.totalEmployees) * 100).toFixed(1)}%
                  </span>
                </div>
                <div className="flex justify-between items-center text-sm">
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            <div className="bg-white p-8 rounded-[2rem] border border-zinc-200 shadow-sm flex-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div
                  className={cn(
                    ...
                    impact.newComplianceScore >= 80
                      ? "bg-emerald-50 border-emerald-200 text-emerald-900"
                      : impact.newComplianceScore >= 60
                        ? "bg-amber-50 border-amber-200 text-amber-900"
                        : "bg-rose-50 border-rose-200 text-rose-900"
                  )}
                >
                  <span className="text-xs font-bold uppercase tracking-widest opacity-80">
                    درجة الامتثال (Nitaqat & Mudad)
                  </span>
                  ...
                  <span className="text-sm font-medium opacity-90 mt-1">
                    {impact.newComplianceScore > baseline.complianceScore
                      ? "تحسن في مؤشر الامتثال"
                      : impact.newComplianceScore < baseline.complianceScore
                        ? "تراجع في مؤشر الامتثال"
                        : "لم يتغير مؤشر الامتثال"}
                  </span>
                </div>
              </div>

              <h4 className="font-bold text-zinc-900 mb-4 border-b border-zinc-100 pb-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-zinc-50 p-4 rounded-xl border border-zinc-100">
                  ...
                  <p className="text-lg font-black text-zinc-900">
                    {impact.newPayroll.toLocaleString()} SAR
                  </p>
                  <p className="text-[10px] text-zinc-400 mt-1">
                    + {(impact.newPayroll - baseline.monthlyPayroll).toLocaleString()} SAR زيادة
                  </p>
                </div>
                <div className="bg-zinc-50 p-4 rounded-xl border border-zinc-100">
                  <p className="text-xs text-zinc-500 font-bold mb-1">تأمينات الموظف (GOSI)</p>
                  <p className="text-lg font-black text-rose-600">
                    {impact.gosiImpact.toLocaleString()} SAR
                  </p>
                ...
                <div className="bg-zinc-50 p-4 rounded-xl border border-zinc-100">
                  ...
                  <p className="text-[10px] text-zinc-400 mt-1">
                    لا يوجد أثر مباشر على ضريبة القيمة المضافة
                  </p>
                </div>
              </div>
              <div className="mt-8 p-4 bg-blue-50 border border-blue-200 rounded-xl flex gap-4 items-start">
                ...
                <div>
                  ...
                  <ul className="text-xs text-blue-800 space-y-1 list-disc list-inside">
                    {impact.newComplianceScore < 70 && (
                      <li>
                        الحذر: التوظيف المقترح سيقلل من درجة الامتثال مما قد يمنع المؤسسة من
                        المناقصات الحكومية.
                      </li>
                    )}
                    {hiringParams.expatCount > hiringParams.saudiCount * 2 && (
                      <li>
                        نسبة توظيف الوافدين للسعوديين عالية، ينصح بزيادة السعوديين للحفاظ على النطاق
                        الأخضر.
                      </li>
                    )}
                    {impact.gosiImpact > 10000 && (
                      <li>
                        تأكد من تحديث ميزانية الرواتب في نظام حماية الأجور (Mudad) لتفادي غرامات
                        التأخير.
                      </li>
                    )}
                    {impact.newComplianceScore >= 80 && (
                      <li>
                        قرارات التوظيف الحالية ممتازة وتدعم الاستدامة والامتثال للقوانين السعودية.
                      </li>
                    )}
                  </ul>
                </div>
              ...
              <div className="mt-8">
                <h4 className="font-bold text-zinc-900 mb-4 border-b border-zinc-100 pb-2">
                  سجل المراجعة (Audit Log - JSON Format)
                </h4>
        ...
        >
          <div className="lg:col-span-4 space-y-6">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    max="10"
                    placeholder="1"
                    value={expansionParams.branchesCount}
                    onChange={(e) =>
                      setExpansionParams({
                        ...expansionParams,
                        branchesCount: parseInt(e.target.value) || 0,
                      })
                    }
                    className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none"
                  />
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    step="10000"
                    placeholder="150000"
                    value={expansionParams.avgRevenue}
                    onChange={(e) =>
                      setExpansionParams({
                        ...expansionParams,
                        avgRevenue: parseInt(e.target.value) || 0,
                      })
                    }
                    className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none"
                  />
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-zinc-700">
                    تكلفة التجهيزات للفرع (تدفع مرة واحدة)
                  </label>
                  <input
                    type="number"
                    step="50000"
                    placeholder="300000"
                    value={expansionParams.capexSetup}
                    onChange={(e) =>
                      setExpansionParams({
                        ...expansionParams,
                        capexSetup: parseInt(e.target.value) || 0,
                      })
                    }
                    className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none"
                  />
            ...
            <div className="bg-blue-50 p-6 rounded-[2rem] border border-blue-200 shadow-sm">
              <h4 className="font-bold text-blue-900 border-b border-blue-200 pb-2 mb-4 text-xs">
                قوانين ZATCA ذات الصلة
              </h4>
              <p className="text-xs text-blue-800 leading-relaxed">
                أي إيراد جديد يخضع لضريبة القيمة المضافة 15%. وتكاليف التجهيزات (رأس المال) يمكنك
                خصم ضريبتها (Input VAT) من إجمالي الضريبة المستحقة مما يحسن التدفق النقدي في الأشهر
                الأولى.
              </p>
            </div>
          </div>
            ...
            <div className="bg-white p-8 rounded-[2rem] border border-zinc-200 shadow-sm flex-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="p-6 rounded-2xl border bg-emerald-50 border-emerald-200 text-emerald-900 flex flex-col gap-2">
                  ...
                  <span className="text-4xl font-black">
                    +{expansionImpact.revenue.toLocaleString()} ريال
                  </span>
                  <span className="text-sm font-medium opacity-90 mt-1">
                    يضاف إليها {expansionImpact.outputVat.toLocaleString()} ريال (ضريبة Output VAT)
                  </span>
                </div>

                <div className="p-6 rounded-2xl border bg-amber-50 border-amber-200 text-amber-900 flex flex-col gap-2">
                  <span className="text-xs font-bold uppercase tracking-widest opacity-80">
                    الضريبة القابلة للخصم (Input VAT)
                  </span>
                  <span className="text-4xl font-black">
                    {expansionImpact.inputVat.toLocaleString()} ريال
                  </span>
                  <span className="text-sm font-medium opacity-90 mt-1">
                    من تجهيزات الفرع (
                    {(expansionParams.capexSetup * expansionParams.branchesCount).toLocaleString()}{" "}
                    ريال)
                  </span>
                </div>
              </div>
              ...
              <div className="mt-8 p-4 bg-zinc-50 border border-zinc-200 rounded-xl">
                <h4 className="text-sm font-bold text-zinc-900 mb-2">
                  التدفق النقدي الضريبي (الشهر الأول)
                </h4>
                <p className="text-xs text-zinc-600 mb-4">
                  في الشهر الأول، ستكون ضريبة المخرجات أقل من ضريبة المدخلات، مما يعني وجود رصيد
                  دائن لدى هيئة الزكاة والضريبة والجمارك.
                </p>
                <div className="flex justify-between items-center bg-white p-4 rounded-lg border border-zinc-100">
                  ...
                  <span className="font-black text-emerald-600">
                    {expansionImpact.netVatMonth1 <= 0 ? "-" : "+"}
                    {Math.abs(expansionImpact.netVatMonth1).toLocaleString()} ريال{" "}
                    {expansionImpact.netVatMonth1 <= 0 ? "(رصيد مسترد)" : "(مستحقة للدفع)"}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-8">
                <h4 className="font-bold text-zinc-900 mb-4 border-b border-zinc-100 pb-2">
                  سجل المراجعة الضريبية (Audit Log - JSON Format)
                </h4>
                <div className="bg-zinc-900 rounded-xl p-4 overflow-x-auto" dir="ltr">
                  <pre className="text-xs text-emerald-400 font-mono">
                    {JSON.stringify(
                      {
                        ...
                        zatca_compliance:
                          expansionImpact.netVatMonth1 <= 0
                            ? "ELIGIBLE_FOR_VAT_RETURN"
                            : "VAT_PAYMENT_REQUIRED",
                      },
                      null,
                      2
                    )}
        ...
        >
          <div className="lg:col-span-4 space-y-6">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    max="50"
                    placeholder="5"
                    value={salaryParams.increasePercent}
                    onChange={(e) =>
                      setSalaryParams({
                        ...salaryParams,
                        increasePercent: parseInt(e.target.value) || 0,
                      })
                    }
                    className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none"
                  />
                ...
                <div className="space-y-2">
                  ...
                  <input
                    type="number"
                    placeholder="45"
                    value={salaryParams.affectedCount}
                    max={baseline.totalEmployees}
                    onChange={(e) =>
                      setSalaryParams({
                        ...salaryParams,
                        affectedCount: parseInt(e.target.value) || 0,
                      })
                    }
                    className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none"
                  />
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-zinc-700">
                    تعديل بدل السكن (مقطوع لكل موظف)
                  </label>
                  <input
                    type="number"
                    step="1000"
                    placeholder="2000"
                    value={salaryParams.housingAllowanceChange}
                    onChange={(e) =>
                      setSalaryParams({
                        ...salaryParams,
                        housingAllowanceChange: parseInt(e.target.value) || 0,
                      })
                    }
                    className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none"
                  />
            ...
            <div className="bg-amber-50 p-6 rounded-[2rem] border border-amber-200 shadow-sm">
              <h4 className="font-bold text-amber-900 border-b border-amber-200 pb-2 mb-4 text-xs">
                حماية الأجور (Mudad) & تأمينات (GOSI)
              </h4>
              <p className="text-xs text-amber-800 leading-relaxed">
                الزيادة في الراتب الأساسي أو بدل السكن تؤثر تلقائياً على اشتراكات التأمينات
                الاجتماعية (GOSI) وملف حماية الأجور. تذكر رفع العقود المحدثة في (مدد) لتفادي
                الانحرافات.
              </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            <div className="bg-white p-8 rounded-[2rem] border border-zinc-200 shadow-sm flex-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="p-6 rounded-2xl border bg-amber-50 border-amber-200 text-amber-900 flex flex-col gap-2">
                  <span className="text-xs font-bold uppercase tracking-widest opacity-80">
                    تكلفة الرواتب الجديدة (شهرياً)
                  </span>
                  <span className="text-4xl font-black">
                    {salaryImpact.newPayroll.toLocaleString()} ريال
                  </span>
                  <span className="text-sm font-medium opacity-90 mt-1">
                    زيادة بنحو {salaryImpact.increaseAmount.toLocaleString()} ريال شهرياً
                  </span>
                </div>

                <div className="p-6 rounded-2xl border bg-rose-50 border-rose-200 text-rose-900 flex flex-col gap-2">
                  <span className="text-xs font-bold uppercase tracking-widest opacity-80">
                    أثر تأمينات (GOSI) الإضافي
                  </span>
                  <span className="text-4xl font-black">
                    +{salaryImpact.gosiImpact.toLocaleString()} ريال
                  </span>
                  <span className="text-sm font-medium opacity-90 mt-1">
                    يتحملها صاحب العمل شهرياً
                  </span>
                </div>
              </div>
              ...
              <div className="mt-8 p-4 bg-zinc-50 border border-zinc-200 rounded-xl">
                ...
                <p className="text-xs text-zinc-600 mb-4">
                  وفقاً لنظام (Mudad)، أي اختلاف بين المسير المرفوع والمبالغ المحولة عبر البنك بنسبة
                  تتجاوز 10% سيؤدي إلى رفض الملف وإيقاف الخدمات.
                </p>
                <div className="flex justify-between items-center bg-white p-4 rounded-lg border border-zinc-100">
                  <span className="font-bold text-zinc-700">تطابق ملف حماية الأجور (متوقع)</span>
                  <span className="font-black text-amber-600">
                    {salaryImpact.increaseAmount > baseline.monthlyPayroll * 0.1
                      ? "يتطلب تحديث العقود"
                      : "الزيادة ضمن المقبول"}
                  </span>
                </div>
              ...
              <div className="mt-8">
                <h4 className="font-bold text-zinc-900 mb-4 border-b border-zinc-100 pb-2">
                  سجل المراجعة الامتثالية (Audit Log - JSON Format)
                </h4>
                <div className="bg-zinc-900 rounded-xl p-4 overflow-x-auto" dir="ltr">
