...
export default function EmailMarketing() {
  ...
  // Navigation / Tabs
  const [activeTab, setActiveTab] = useState<"dashboard" | "campaigns" | "templates" | "automations" | "contacts">("dashboard");
  ...
  const [campBlocks, setCampBlocks] = useState<Array<{ type: string; text?: string; url?: string; code?: string }>>([
  ...
  const [aiSubjectOptions, setAiSubjectOptions] = useState<Array<{ category: string; subject: string; preview: string }>>([]);
  ...
  const handleSendCampaign = async (id: string) => {
    try {
      toast.loading(isAr ? "جاري جدولة وإرسال البريد الإلكتروني..." : "Scheduling and dispatching campaign...", { id: "sending" });
      const token = await auth.currentUser?.getIdToken();
      ...
      if (res.ok) {
        ...
        toast.success(isAr ? "تم إرسال الحملة بنجاح! تم رصد مؤشرات الأداء فوراً." : "Campaign sent successfully! Analytics tracked in real time.");
        fetchData();
      } else {
        toast.dismiss("sending");
  ...
  const handleDeleteCampaign = async (id: string) => {
    if (!confirm(isAr ? "هل أنت متأكد من حذف هذه الحملة؟" : "Are you sure you want to delete this campaign?")) return;
    try {
      const token = await auth.currentUser?.getIdToken();
  ...
  const generateAiContent = async (genType: "subject_lines" | "full_email") => {
    if (!aiPrompt) {
      toast.error(isAr ? "يرجى تحديد تفاصيل أو هدف الحملة أولاً" : "Please provide campaign details or goal first");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (genType === "subject_lines") {
          ...
          toast.success(isAr ? "تم ابتكار عناوين جذابة فائقة الفعالية!" : "High-converting subject lines crafted!");
        } else {
          ...
          toast.success(isAr ? "تم إعداد محتوى الرسالة وتعبئتها في المصمم!" : "Email body compiled and filled in the designer!");
        }
      } else {
  ...
  const generateAiWorkflow = async () => {
    if (!aiWorkflowGoal) {
      toast.error(isAr ? "يرجى كتابة سيناريو أو هدف الأتمتة" : "Please write automation scenario or goal first");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(isAr ? "تم تخطيط وهيكلة الأتمتة الذكية بنجاح!" : "Smart flow planned and designed successfully!");
      } else {
        toast.error(isAr ? "فشل بناء سير العمل بالذكاء الاصطناعي" : "Failed to structure flow with AI");
      }
    } catch (err) {
  ...
  const handleSaveAutomation = async () => {
    ...
    try {
      ...
      const payload = {
        name: autName,
        triggerEvent: autTrigger,
        steps: autSteps.length > 0 ? autSteps : [
      ...
      if (res.ok) {
        toast.success(isAr ? "تم بناء وحفظ سير الأتمتة بنجاح" : "Automation flow built and saved successfully");
  ...
  const handleSyncCRM = async () => {
    try {
      toast.loading(isAr ? "جاري فحص ومزامنة العملاء من CRM..." : "Scanning and importing contacts from CRM...", { id: "sync" });
      const token = await auth.currentUser?.getIdToken();
      const res = await fetch("/api/email-marketing/contacts", {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
        toast.dismiss("sync");
        toast.success(isAr ? "تمت المزامنة بنجاح! تم استيراد أحدث العملاء المؤهلين." : "Sync completed! Imported qualified contacts.");
        fetchData();
      }
    } catch (err) {
      toast.dismiss("sync");
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 p-6 font-sans">
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 pb-5 mb-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            {isAr ? "نظام أتمتة تسويقية رائد، وحملات مدعومة بالذكاء الاصطناعي متكاملة مع نظام المبيعات" : "Next-gen campaign builder, flow orchestrations, and predictive copywriter."}
          </p>
        </div>
      ...
      <div className="max-w-7xl mx-auto mb-6">
        <div className="flex gap-1 border-b border-slate-200 overflow-x-auto whitespace-nowrap scrollbar-none">
          {[
            { id: "dashboard", label: isAr ? "لوحة القيادة والأداء" : "Dashboard Analytics", icon: BarChart3 },
            { id: "campaigns", label: isAr ? "الحملات والمسودات" : "Campaigns & Broadcasts", icon: Send },
            { id: "templates", label: isAr ? "قوالب مصممة مسبقاً" : "Saved Templates", icon: LayoutIcon },
            { id: "automations", label: isAr ? "أتمتة وسيناريوهات التنقيط" : "Automation Journeys", icon: GitPullRequest },
            { id: "contacts", label: isAr ? "قوائم الاتصال والشرائح" : "Audience & Segmentations", icon: Users }
          ].map((tab) => (
            <button
              key={tab.id}
      ...
      <div className="max-w-7xl mx-auto">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-slate-500 mt-4 text-sm">{isAr ? "تحميل لوحة التسويق..." : "Loading marketing engine..."}</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                    ...
                    <h3 className="text-3xl font-bold text-slate-900 mt-2">{totalSent.toLocaleString()}</h3>
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden bg-gradient-to-br from-indigo-900 to-slate-900 text-white">
                    ...
                    <h3 className="text-3xl font-extrabold text-white mt-2">
                      {totalRevenue.toLocaleString()} <span className="text-sm font-medium">{isAr ? "ر.س" : "SAR"}</span>
                    </h3>
                    <div className="flex items-center gap-1 mt-2 text-xs text-indigo-200">
                      <Sparkles className="w-3.5 h-3.5 text-amber-400 animate-pulse" />
                      <span>{isAr ? "مدفوعة بالكامل بقنوات التسويق" : "Generated via direct campaigns"}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm lg:col-span-2">
                    <div className="flex justify-between items-center mb-4">
                      <div>
                        <h4 className="text-base font-bold text-slate-900">{isAr ? "أداء الحملات الأخيرة" : "Recent Campaigns Analytics"}</h4>
                        <p className="text-xs text-slate-500 mt-0.5">{isAr ? "مقارنة المشاهدات والمبيعات ومعدلات التفاعل" : "Open and conversion statistics per broadcast"}</p>
                      </div>
                    </div>
                    <div className="h-80">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={campaignHistoryData} margin={{ top: 20, right: 10, left: 10, bottom: 5 }}>
                          <CartesianGrid strokeDasharray="3 3" vertical={false} />
                          <XAxis dataKey="name" stroke="#94a3b8" fontSize={12} tickLine={false} />
                          <YAxis stroke="#94a3b8" fontSize={12} tickLine={false} />
                          <Tooltip />
                          <Legend />
                          <Bar dataKey="opens" name={isAr ? "عمليات الفتح" : "Opens"} fill="#4f46e5" radius={[4, 4, 0, 0]} />
                          <Bar dataKey="clicks" name={isAr ? "النقرات" : "Clicks"} fill="#0ea5e9" radius={[4, 4, 0, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
                    <div>
                      <h4 className="text-base font-bold text-slate-900">{isAr ? "صحة التوصيل والDeliverability" : "Deliverability & Spam Score"}</h4>
                      <p className="text-xs text-slate-500 mt-0.5">{isAr ? "مراقبة بروتوكولات SPF, DKIM, DMARC" : "Continuous IP reputation surveillance"}</p>
                    </div>
                    ...
                    <div className="my-6 flex flex-col items-center">
                      <div className="relative w-40 h-40 flex items-center justify-center">
                        ...
                        <svg className="w-full h-full transform -rotate-90">
                          ...
                          <circle cx="80" cy="80" r="70" stroke="#10b981" strokeWidth="12" fill="transparent" strokeDasharray="440" strokeDashoffset="44" strokeLinecap="round" />
                        </svg>
                        <div className="absolute text-center">
                          <span className="text-4xl font-extrabold text-slate-900">99%</span>
                          <span className="text-xs text-slate-500 block font-medium mt-1">{isAr ? "ممتاز" : "Excellent"}</span>
                        </div>
                      </div>

                      <div className="grid grid-cols-2 gap-4 w-full mt-6 text-center text-xs">
                        <div className="p-2.5 bg-slate-50 rounded-xl">
                          <span className="text-slate-400 block">{isAr ? "معدل الارتداد" : "Bounce Rate"}</span>
                          <span className="text-sm font-bold text-slate-800 mt-1 block">0.3%</span>
                        </div>
                        <div className="p-2.5 bg-slate-50 rounded-xl">
                          <span className="text-slate-400 block">{isAr ? "معدل الشكاوى" : "Spam Complaint"}</span>
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                  <h4 className="text-base font-bold text-slate-900 mb-2">{isAr ? "تحليل الأجهزة ونظم التشغيل للعملاء" : "Customer Device & Client Analytics"}</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                    <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
                      <Smartphone className="w-10 h-10 text-indigo-600" />
                      <div>
                        <span className="text-xs text-slate-400 block">{isAr ? "جوال / أجهزة ذكية" : "MOBILE DEVICES"}</span>
                    ...
                    <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
                      ...
                      <div>
                        <span className="text-xs text-slate-400 block">{isAr ? "حواسب مكتبية" : "DESKTOP CLIENTS"}</span>
                    ...
                    <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
                      ...
                      <div>
                        <span className="text-xs text-slate-400 block">{isAr ? "أجهزة أخرى" : "OTHER / TABLETS"}</span>
              ...
              >
                ...
                <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                  <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <div>
                      <h3 className="text-lg font-bold text-slate-900">{isAr ? "جميع الحملات والرسائل الإخبارية" : "All Broadcast Campaigns"}</h3>
                      <p className="text-xs text-slate-500 mt-1">{isAr ? "إنشاء وحفظ وتعديل وإرسال النشرات المخصصة" : "Create, customize and trigger scheduled campaigns"}</p>
                    </div>
                  </div>

                  <div className="divide-y divide-slate-100">
                    {campaigns.map((camp) => (
                      <div key={camp.id} className="p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-slate-50 transition">
                        <div className="flex items-start gap-3">
                          <div className={cn(
                            "p-2.5 rounded-xl",
                            camp.status === "Sent" ? "bg-emerald-50 text-emerald-600" :
                            camp.status === "Scheduled" ? "bg-indigo-50 text-indigo-600" : "bg-slate-100 text-slate-600"
                          )}>
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-400 mt-2">
                              <span>Segment: <strong className="text-slate-600">{camp.targetSegment}</strong></span>
                              <span>•</span>
                              <span>Created: <strong>{new Date(camp.createdAt).toLocaleDateString()}</strong></span>
                              {camp.ampEnabled && (
                                <>
                                  <span>•</span>
                                  <span className="text-indigo-600 font-medium flex items-center gap-0.5">⚡ AMP</span>
                                </>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end border-t md:border-transparent pt-3 md:pt-0">
                          {camp.status === "Sent" ? (
                            <div className="flex items-center gap-6 text-center">
                              <div>
                                <span className="text-[10px] text-slate-400 font-semibold block uppercase">{isAr ? "إرسال" : "SENT"}</span>
                                <span className="text-sm font-bold text-slate-800">{camp.sentCount}</span>
                              </div>
                              <div>
                                <span className="text-[10px] text-slate-400 font-semibold block uppercase">{isAr ? "فتح" : "OPENS"}</span>
                                <span className="text-sm font-bold text-teal-600">
                                  {camp.sentCount > 0 ? ((camp.openCount / camp.sentCount) * 100).toFixed(0) : 0}%
                                </span>
                              </div>
                              <div>
                                <span className="text-[10px] text-slate-400 font-semibold block uppercase">{isAr ? "نقرات" : "CLICKS"}</span>
                                <span className="text-sm font-bold text-indigo-600">
                                  {camp.openCount > 0 ? ((camp.clickCount / camp.openCount) * 100).toFixed(0) : 0}%
                                </span>
                              </div>
                              {camp.revenueGenerated > 0 && (
                                <div>
                                  <span className="text-[10px] text-slate-400 font-semibold block uppercase">{isAr ? "مبيعات" : "REVENUE"}</span>
                                  <span className="text-sm font-bold text-amber-600">{camp.revenueGenerated.toLocaleString()} ر.س</span>
                                </div>
                              )}
                            </div>
                          ) : (
            ...
            {activeTab === "templates" && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {templates.map((temp) => (
                    <div key={temp.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col justify-between group hover:border-indigo-300 hover:shadow-md transition">
                      <div className="p-5">
                        ...
                        <div className="mt-4 p-3 bg-slate-50 rounded-xl space-y-1.5 text-xs text-slate-500 max-h-40 overflow-y-auto">
                          {temp.jsonStructure?.blocks?.map((block, idx) => (
                            <div key={idx} className="flex items-center gap-1.5">
                              <span className="px-1.5 py-0.5 bg-indigo-100 text-indigo-700 font-mono text-[9px] rounded uppercase">{block.type}</span>
                              <span className="truncate">{block.text || block.url || block.code}</span>
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              >
                ...
                <div className="bg-gradient-to-r from-indigo-900 via-slate-900 to-indigo-950 text-white p-6 rounded-2xl shadow-md">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-semibold text-indigo-200 uppercase tracking-widest">{isAr ? "مساعد تصميم الأتمتة بالذكاء الاصطناعي" : "AI Workflow Copilot"}</span>
                  </div>
                  <h3 className="text-lg font-bold">{isAr ? "صمم قمع ومسار أتمتة مبيعات متطور بلحظات" : "Design high-converting multi-step email automations"}</h3>
                  <p className="text-xs text-indigo-200 mt-1">{isAr ? "اكتب هدف الأتمتة والحدث المحفز، وسيقوم نظامنا بتصميم المسار والمؤقتات بالكامل" : "Describe your workflow goal to instantly generate dynamic triggers, waits and checkpoints."}</p>
                  ...
                  <div className="mt-4 flex flex-col md:flex-row gap-2">
                    <input
                      ...
                      placeholder={isAr ? "مثال: استعادة العملاء الذين لم يشتروا منذ 30 يوم، وتقديم عروض تصاعدية مخصصة" : "e.g. Onboard new real-estate clients with deep tutorials and exclusive discounts"}
                    ...
                    >
                      <option value="Lead Created" className="text-slate-950">Lead Created (عميل محتمل جديد)</option>
                      <option value="Cart Abandoned" className="text-slate-950">Cart Abandoned (سلة متروكة)</option>
                      <option value="E-commerce Purchase" className="text-slate-950">E-commerce Purchase (عملية شراء)</option>
                    </select>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {automations.map((aut) => (
                    <div key={aut.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex items-center gap-3 text-right text-xs">
                          <div>
                            <span className="text-slate-400 block">{isAr ? "المسجلين" : "ENROLLED"}</span>
                            <span className="font-bold text-slate-800">{aut.enrolledCount}</span>
                          </div>
                          <div>
                            <span className="text-slate-400 block">{isAr ? "المكتملين" : "COMPLETED"}</span>
                            ...
                            <div className="p-2.5 bg-slate-50 rounded-xl flex-1 text-xs">
                              <div className="flex justify-between items-center">
                                <span className="font-semibold text-slate-800 flex items-center gap-1">
                                  {step.type === "email" ? <Mail className="w-3.5 h-3.5 text-indigo-500" /> :
                                   step.type === "wait" ? <Clock className="w-3.5 h-3.5 text-amber-500" /> :
                              ...
                              {step.description && <p className="text-[10px] text-slate-400 mt-1">{step.description}</p>}
                              ...
                              {step.yesSteps && step.yesSteps.length > 0 && (
                                <div className="mt-2 pl-3 border-l border-emerald-300 space-y-2 bg-emerald-50/50 p-1.5 rounded-lg">
                                  <span className="text-[10px] font-bold text-emerald-700 block">✓ {isAr ? "نعم / تفاعل" : "Yes / Interacted"}</span>
                                  {step.yesSteps.map(ys => (
                                    <div key={ys.id} className="text-[10px] text-slate-600">{ys.label}</div>
                                  ))}
                                </div>
                              )}
                              {step.noSteps && step.noSteps.length > 0 && (
                                <div className="mt-2 pl-3 border-l border-rose-300 space-y-2 bg-rose-50/50 p-1.5 rounded-lg">
                                  <span className="text-[10px] font-bold text-rose-700 block">✗ {isAr ? "لا / لم يتفاعل" : "No / Bounce"}</span>
                                  {step.noSteps.map(ns => (
                                    <div key={ns.id} className="text-[10px] text-slate-600">{ns.label}</div>
                                  ))}
                                </div>
                              )}
                            </div>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                  ...
                  <div className="p-5 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-50/50">
                    <div className="flex-1 w-full md:w-auto relative">
                      ...
                      <input
                        type="text"
                        placeholder={isAr ? "البحث بالاسم، البريد أو الشركة..." : "Search by name, email, company..."}
                        value={contactSearch}
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b border-slate-100 bg-slate-50 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                          ...
                          <th className="py-3 px-5">{isAr ? "البريد الإلكتروني" : "EMAIL ADDRESS"}</th>
                            ...
                            <td className="py-3 px-5">
                              <div className="flex flex-wrap gap-1">
                                {c.segmentTags?.map((tag, idx) => (
                                  <span key={idx} className="px-1.5 py-0.5 bg-indigo-50 text-indigo-700 rounded text-[9px] font-medium flex items-center gap-0.5">
            ...
            >
              ...
              <div className="lg:col-span-7 p-6 overflow-y-auto space-y-4 border-r border-slate-100 max-h-[90vh]">
                <div className="flex justify-between items-center pb-2 border-b border-slate-100">
                  <h3 className="text-lg font-bold text-slate-900">
                    {editingCampaign ? (isAr ? "تعديل النشرة التسويقية" : "Edit Marketing Broadcast") : (isAr ? "تصميم حملة بريد إلكتروني جديدة" : "Design New Campaign")}
                  </h3>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 block uppercase mb-1">{isAr ? "اسم الحملة الداخلي" : "Campaign Title"}</label>
                    <input
                      type="text"
                      value={campName}
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 block uppercase mb-1">{isAr ? "الجمهور المستهدف" : "Target Segment"}</label>
                    <select
                      value={campSegment}
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-400 block uppercase mb-1">{isAr ? "عنوان البريد الإلكتروني" : "Email Subject Line"}</label>
                  <input
                    type="text"
                    value={campSubject}
                ...
                <div className="p-3.5 bg-slate-50 rounded-xl space-y-2.5">
                  ...
                  {abEnabled && (
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-400 block">{isAr ? "عنوان بديل (النسخة B)" : "Variant B Subject Line"}</label>
                      <input
                        type="text"
                        value={campSubjectB}
                ...
                <div className="flex items-center justify-between p-3 border border-indigo-100 bg-indigo-50/50 rounded-xl">
                  <div className="text-left">
                    <span className="text-xs font-bold text-indigo-950 block">⚡ AMP Email (أجهزة تفاعلية)</span>
                    <span className="text-[10px] text-slate-500 block">اضغط زر تشغيل المكونات والتحقق داخل الرسائل</span>
                  </div>
                  <input
                    type="checkbox"
                    checked={campAmp}
                ...
                <div className="space-y-2">
                  <span className="text-[10px] font-bold text-slate-400 block uppercase">{isAr ? "مكونات الرسالة المصممة" : "Email Builder Blocks"}</span>
                  <div className="space-y-2.5">
                    {campBlocks.map((block, idx) => (
                      <div key={idx} className="flex gap-2 items-center bg-slate-50 p-2.5 rounded-xl border border-slate-200">
                        <span className="px-2 py-0.5 bg-slate-200 text-slate-600 font-mono text-[9px] rounded uppercase">{block.type}</span>
                        <input
                          type="text"
                          value={block.text || block.url || block.code || ""}
                          onChange={(e) => {
                            const updated = [...campBlocks];
                            if (block.type === "header" || block.type === "text" || block.type === "button") {
                  ...
                  <div className="flex gap-1.5 pt-2">
                    <button
                      onClick={() => setCampBlocks([...campBlocks, { type: "text", text: "فقرة نصية جديدة" }])}
                      className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 text-[10px] font-semibold rounded-lg"
                    >
                      + Text
                    </button>
                    <button
                      onClick={() => setCampBlocks([...campBlocks, { type: "header", text: "عنوان فرعي جديد" }])}
                      className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 text-[10px] font-semibold rounded-lg"
                    >
                      + Header
                    </button>
                    <button
                      onClick={() => setCampBlocks([...campBlocks, { type: "image", url: "https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=600" }])}
                      className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 text-[10px] font-semibold rounded-lg"
                    >
                      + Image
                    </button>
                    <button
                      onClick={() => setCampBlocks([...campBlocks, { type: "button", text: "اضغط لتسجيل العرض", url: "https://madarij.sa" }])}
                      className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 text-[10px] font-semibold rounded-lg"
                    >
                      + Button
                    </button>
              ...
              <div className="lg:col-span-5 bg-slate-50 p-6 overflow-y-auto max-h-[90vh] space-y-4">
                <div className="flex items-center gap-2 pb-2 border-b border-slate-200">
                  ...
                  <h4 className="text-sm font-bold text-slate-900">{isAr ? "منشئ المحتوى ومحفز المبيعات الذكي" : "AI Copywriting Copilot"}</h4>
                </div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      placeholder={isAr ? "مثال: إعلان ميزات الفوترة الجديدة المتوافقة مع زكاة وضريبة ودخل مع تقديم كود خصم محدود" : "e.g. Announcing new automated payroll system with 20% early adopter deal"}
                      className="w-full bg-white border border-slate-200 px-3 py-2 rounded-xl text-xs focus:outline-none"
                    />
                  </div>

                  <div>
                    <label className="text-[10px] font-bold text-slate-500 block mb-1">{isAr ? "نبرة الرسالة" : "Tone of Voice"}</label>
                    <select
                      value={aiTone}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      {aiGenerating ? <RefreshCw className="w-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                    ...
                    >
                      {aiGenerating ? <RefreshCw className="w-3 animate-spin" /> : <Cpu className="w-3 h-3" />}
                ...
                {aiSubjectOptions.length > 0 && (
                  <div className="space-y-2 border-t border-slate-200 pt-4">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">{isAr ? "العناوين الذكية المقترحة" : "AI Suggested Subjects"}</span>
                    <div className="space-y-2">
                      {aiSubjectOptions.map((opt, i) => (
                        <div
                          ...
                          onClick={() => {
                            ...
                            toast.success(isAr ? "تم اختيار العنوان وتطبيقه!" : "Subject line applied!");
                          }}
                        ...
                        >
                          <span className="text-[9px] font-bold text-indigo-600 uppercase block">{opt.category}</span>
                ...
                {aiBodyResponse && (
                  <div className="space-y-2 border-t border-slate-200 pt-4 bg-indigo-50/50 p-4 rounded-xl">
                    <span className="text-[10px] font-bold text-indigo-700 uppercase tracking-widest block">{isAr ? "أفضل أوقات الإرسال المقترحة" : "Send Time Optimization Advice"}</span>
          ...
          <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
            ...
            >
              <div className="flex justify-between items-center pb-2 border-b border-slate-100">
                ...
                <button onClick={() => setShowAutModal(false)} className="text-slate-400 hover:text-slate-600">
                  <XCircle className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-[10px] font-bold text-slate-400 block uppercase mb-1">{isAr ? "اسم سير الأتمتة" : "Workflow Name"}</label>
                  <input
                    type="text"
                    value={autName}
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-400 block uppercase mb-1">{isAr ? "المحفز / نقطة البداية" : "Trigger Event"}</label>
                  <select
                    value={autTrigger}
                    onChange={(e) => setAutTrigger(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl text-xs focus:outline-none"
              ...
              <div className="space-y-2.5 bg-slate-50 p-4 rounded-2xl border border-slate-200 max-h-60 overflow-y-auto">
                <span className="text-[10px] font-bold text-slate-400 block uppercase">{isAr ? "الخطوات المضافة" : "Flow Sequence"}</span>
                {autSteps.length === 0 ? (
                  <p className="text-xs text-slate-400 italic text-center py-4">{isAr ? "لا توجد خطوات مضافة بعد" : "No steps configured yet"}</p>
                ) : (
                  <div className="space-y-2">
                    {autSteps.map((step, idx) => (
                      <div key={idx} className="flex gap-2 items-center bg-white p-2.5 rounded-xl border border-slate-150 text-xs font-semibold">
                        <span className="px-1.5 py-0.5 bg-indigo-100 text-indigo-700 text-[10px] uppercase rounded">{step.type}</span>
                ...
                <div className="flex gap-1 pt-2">
                  <button
                    onClick={() => setAutSteps([...autSteps, { id: `${Date.now()}`, type: "email", label: "ارسل بريد إلكتروني ترحيبي", delayDays: 0 }])}
                    className="px-2 py-1 bg-white border border-slate-200 text-slate-700 text-[9px] font-bold rounded"
                  >
                    + Email
                  </button>
                  <button
                    onClick={() => setAutSteps([...autSteps, { id: `${Date.now()}`, type: "wait", label: "انتظار 3 أيام", value: 3 }])}
                    className="px-2 py-1 bg-white border border-slate-200 text-slate-700 text-[9px] font-bold rounded"
                  >
                    + Wait
                  </button>
        ...
        {showContactModal && (
          <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-[10px] font-bold text-slate-400 block uppercase mb-1">{isAr ? "الاسم الكريم" : "Contact Name"}</label>
                  <input
                    type="text"
                    value={contactName}
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-400 block uppercase mb-1">{isAr ? "البريد الإلكتروني" : "Email Address"}</label>
                  <input
                    type="email"
                    value={contactEmail}
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-400 block uppercase mb-1">{isAr ? "المنشأة / الشركة" : "Company"}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-400 block uppercase mb-1">{isAr ? "وسوم وتصنيف (مفصولة بفاصلة)" : "Segment Tags (comma separated)"}</label>
                  <input
                    type="text"
                    value={contactTags}
