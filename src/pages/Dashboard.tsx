...
export default function Dashboard() {
  ...
  const [quarterlyData, setQuarterlyData] = useState<
    { name: string; current: number; projected: number }[]
  >([
  ...
  useEffect(() => {
    ...
    // Load user specialized config and merge to ensure new widgets (like openwa_status) are included if missing
    if (user.dashboardConfig) {
      const loaded = user.dashboardConfig;
      const missing = DEFAULT_CONFIG.filter((def) => !loaded.some((l: any) => l.id === def.id));
    ...
    // --- REAL-TIME OPENWA PERSISTENT CONNECTION MONITOR & QUEUE STATS LISTENER ---
    ...
    const checkStatus = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Alert user immediately if disconnected and status changed
    ...
    // Poll every 10 seconds for instant disconnection warning
    const intervalId = setInterval(checkStatus, 10000);

    // Listen to queue stats in Firestore for live dashboard metrics
    let unsubQueue = () => {};
  ...
  useEffect(() => {
    ...
    const unsubPayroll = onSnapshot(
      ...
      (snapshot) => {
        ...
        const sortedByPeriod = [...runs].sort((a: any, b: any) =>
    ...
    const unsubEmployees = onSnapshot(
      ...
      (snapshot) => {
        ...
        const saudiEmployees = emps.filter(
          (e: any) =>
            e.nationality?.includes("سعودي") || e.nationality?.toLowerCase().includes("saudi")
        ).length;
        ...
        emps.forEach((emp: any) => {
          if (emp.contractEndDate) {
            const daysLeft =
              (new Date(emp.contractEndDate).getTime() - Date.now()) / (1000 * 3600 * 24);
    ...
    const unsubShipments = onSnapshot(
      ...
      (snapshot) => {
        ...
        setActiveShipments(
          shList.filter((s: any) => s.status !== "delivered" && s.status !== "cancelled")
        );
      },
      (error) => {
    ...
    if (activeView === "ceo") {
      currentStats = [
        {
          ...
          change: dashboardStats?.trends?.revenue
        ...
        },
        {
          ...
          change: dashboardStats?.trends?.compliance
            ? `${dashboardStats.trends.compliance}%`
            : "غير متوفر",
        ...
        },
        {
          ...
          change: dashboardStats?.trends?.payroll
            ? `${dashboardStats.trends.payroll}%`
            : "غير متوفر",
      ...
      currentStats = [
        {
          ...
          change:
            dashboardStats?.saudiEmployees / (dashboardStats?.employeesCount || 1) > 0.3
              ? "النطاق الأخضر"
              : "النطاق الأصفر",
          trend: "up",
          icon: Users,
      ...
      currentStats = [
        ...
        },
        {
          ...
          change: dashboardStats?.pendingInvoices
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                <p className="text-zinc-500 dark:text-zinc-400 text-sm font-black mb-1">
                  {stat.name}
                </p>
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-black text-lg text-zinc-900">مراقب اتصال واتساب (OpenWA)</h3>
                  <p className="text-xs text-zinc-400 font-bold">
                    التحقق المستمر من قنوات الربط وحالة طابور المعالجة الآلي
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 self-start sm:self-center">
                {waStatus === "connected" && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-black bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full shadow-sm">
                    <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                    متصل حياً
                  </span>
                )}
                {waStatus === "disconnected" && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-black bg-rose-50 text-rose-700 border border-rose-200 rounded-full shadow-sm animate-pulse">
                    <span className="w-2 h-2 rounded-full bg-rose-500" />
                    انقطع الاتصال
                  </span>
                )}
                {waStatus === "disabled" && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-black bg-zinc-100 text-zinc-600 border border-zinc-200 rounded-full shadow-sm">
                    <span className="w-2 h-2 rounded-full bg-zinc-400" />
                    غير مفعّل
                  </span>
                )}
                {waStatus === "checking" && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-black bg-blue-50 text-blue-700 border border-blue-200 rounded-full shadow-sm">
                    <span className="w-2 h-2 rounded-full bg-blue-500 animate-spin border-t-transparent border border-solid" />
                    جاري الفحص...
                  </span>
                )}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Box 1: Status Details */}
              <div className="p-5 bg-zinc-50 rounded-2xl border border-zinc-100 flex flex-col justify-between">
                <div>
                  <span className="text-[10px] text-zinc-400 font-black tracking-wider uppercase block mb-1">
                    تفاصيل الجلسة
                  </span>
                  <p className="text-xs font-bold text-zinc-600 leading-relaxed">
                    {waStatus === "connected" &&
                      "الربط نشط حالياً وتتم مزامنة المحادثات الواردة مع الفرص البيعية فوراً."}
                    {waStatus === "disconnected" &&
                      "الجلسة منقطعة حالياً. يرجى إعادة الاتصال لتفادي توقف أتمتة الردود والرسائل."}
                    {waStatus === "disabled" &&
                      "ربط واتساب غير مفعل حالياً. يمكنك تفعيله من شاشة الإعدادات بضغطة زر."}
              ...
              <div className="p-5 bg-zinc-50 rounded-2xl border border-zinc-100 flex flex-col justify-between">
                <div>
                  ...
                  <div className="flex items-baseline gap-2 mb-1">
                    ...
                    <span className="text-xs text-zinc-400 font-bold">مسجلة بالخادم</span>
                  </div>
                  <p className="text-xs text-zinc-500 font-medium leading-relaxed">
                    تخزين الأحداث والرسائل آلياً في Firestore لمنع فقدان البيانات عند حدوث ضغط عالٍ
                    للرسائل قبل معالجتها.
                  </p>
                </div>
              </div>

              {/* Box 3: Realtime Processor Status */}
              <div className="p-5 bg-zinc-50 rounded-2xl border border-zinc-100 flex flex-col justify-between">
                <div>
                  <span className="text-[10px] text-zinc-400 font-black tracking-wider uppercase block mb-1">
                    المعالج الذكي الآلي
                  ...
                  <div className="flex items-baseline gap-2 mb-1">
                    ...
                    <span className="text-xs text-zinc-400 font-bold">معلقة قيد المعالجة</span>
                  </div>
                  <p className="text-xs text-zinc-500 font-medium leading-relaxed">
                    {pendingQueueCount > 0
                      ? "جاري الربط والتصنيف التلقائي للرسائل مع حسابات وملفات العملاء في الـ CRM حياً."
                      : "جميع الرسائل الواردة معالجة ومؤرشفة بالكامل ومرتبطة بملفات العملاء بنجاح."}
                  </p>
                </div>
              </div>
            </div>
          ...
          >
            ...
            <div className="flex items-center justify-between mb-8 relative z-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-black text-2xl text-zinc-900 tracking-tight mb-1">
                    محرك ذكاء مدارج للنمو (AI)
                  </h3>
                  <p className="text-xs font-black text-primary tracking-widest uppercase">
                    توصيات مدارج الذكية المخصصة لك لتسريع المبيعات هذا الربع
                  </p>
                </div>
              </div>
              <Link
              ...
              >
                <div>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className="text-[10px] font-black bg-rose-50 text-rose-600 px-2.5 py-1 rounded-full uppercase tracking-wider border border-rose-100 shadow-sm">
                      عائد فوري
                    </span>
                  </div>
                  <h4 className="font-black text-zinc-900 mb-2">أتمتة الواتساب + CRM</h4>
                  <p className="text-xs text-zinc-500 leading-relaxed font-medium">
                    لديك 12 عميل محتمل لم يتم متابعتهم. تفعيل ردود الواتساب التلقائية المدعومة من
                    مدارج سيزيد نسبة الإغلاق بـ 40٪ فوراً.
                  </p>
                </div>
              ...
              >
                <div>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className="text-[10px] font-black bg-blue-50 text-blue-600 px-2.5 py-1 rounded-full uppercase tracking-wider border border-blue-100 shadow-sm">
                      حماية الثروة
                    </span>
                  </div>
                  <h4 className="font-black text-zinc-900 mb-2">الربط المباشر بـ ZATCA</h4>
                  <p className="text-xs text-zinc-500 leading-relaxed font-medium">
                    تفادى الغرامات المدمّرة للمنشآت الناشئة. استخدم الربط المتكامل والمجاني مع هيئة
                    الزكاة (المرحلة 2) من مدارج بضغطة زر.
                  </p>
                </div>
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className="text-[10px] font-black bg-primary text-white px-2.5 py-1 rounded-full uppercase tracking-wider shadow-[0_0_15px_rgba(16,185,129,0.5)]">
                      برنامج الشركاء
                    </span>
                  </div>
                  <h4 className="font-black text-white mb-2 text-lg">دعوة الموردين للشبكة</h4>
                  <p className="text-xs text-zinc-400 leading-relaxed font-medium">
                    شارك مدارج مع 3 من مورديك أو عملائك واستفد من 3 أشهر مجانية من باقة Premium +
                    تفعيل مزامنة الفواتير المشتركة بينهم.
                  </p>
                </div>
            ...
            <div className="p-6 border-b border-zinc-100 flex justify-between items-center bg-gray-50/50">
              <div>
                ...
                <p className="text-sm font-medium text-zinc-500">
                  موجز مسيرات الرواتب الحديثة وحالتها
                </p>
              </div>
                  ...
                  {!dashboardStats ? (
                    [1, 2, 3].map((i) => (
                      <tr key={i} className="animate-pulse">
                        ...
                        <td className="px-6 py-4">
                          <div className="h-6 bg-zinc-200 rounded-lg w-14" />
                        </td>
                        <td className="px-6 py-4">
                    ...
                    dashboardStats.recentPayroll.map((run: any, idx: number) => (
                      <tr key={run.id || idx} className="hover:bg-zinc-50 transition-colors">
                        ...
                        <td className="px-6 py-4 font-black text-zinc-900 text-left">
                          {run.totalNet.toLocaleString()} ر.س
                        </td>
                      </tr>
                    ))
                  ) : (
                ...
                <div className="p-6 bg-white rounded-3xl border border-zinc-100 shadow-sm flex-1">
                  ...
                  <div className="h-[100px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart
                        data={
                          dashboardStats?.chartData?.slice(-3).map((d: any, i: number) => ({
                            name: d.name,
        ...
        return (
          ...
          >
            <div className="flex justify-between items-center mb-6">
              ...
              <Link to="/app/settings" className="text-xs font-bold text-primary hover:underline">
                عرض الكل
              </Link>
            </div>
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-bold text-zinc-900 line-clamp-1">
                        {log.action || log.payload?.action || `نشاط في ${log.module}`}
                      </p>
        ...
        <section className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          >
            <div>
              ...
              <p className="text-zinc-500 dark:text-zinc-400 text-sm font-black mb-1">
                قوة العمل الحالية (Headcount)
              </p>
              <h3 className="text-3xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight">
                {totalCount} موظف
              </h3>
              <p className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-2 font-bold">
                عقود موثقة ومطابقة في قوى
              </p>
            </div>
          </motion.div>
          ...
          <div className="flex items-center gap-4 mb-6 relative z-10">
            ...
            <div>
              <h3 className="font-black text-xl text-zinc-900">
                مساعد شؤون الموظفين الذكي (HR Advisory)
              </h3>
              <p className="text-xs font-bold text-primary">
                توصيات حية لتحسين امتثال الموارد البشرية وتفادي مخالفات الأجور
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 relative z-10">
            <div className="bg-white p-5 rounded-3xl border border-primary/10 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-zinc-500 leading-relaxed">
                  تطالب وزارة الموارد البشرية بتغطية 100% من عقود الموظفين رقمياً على منصة قوى. قم
                  بتسجيل وتوثيق العقود فوراً لتجنب إيقاف الاستقدام.
                </p>
              </div>
              <Link
                to="/app/fwcos"
            ...
            <div className="bg-white p-5 rounded-3xl border border-primary/10 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-zinc-500 leading-relaxed">
                  معدل التوطين الحالي لديك يسمح لك بالاستفادة من برامج دعم أجور المواطنين بنسب تصل
                  إلى 50٪ لمدد تصل إلى سنتين. قدم عبر برامج صندوق هدف الآن.
                </p>
              </div>
              <a
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-xs text-zinc-500 font-medium">
                  عقود شارفت على الانتهاء تتطلب إجراءً فورياً لتلافي الإيقاف
                </p>
              </div>
              <Link to="/app/fwcos" className="text-xs font-bold text-primary hover:underline">
                إدارة الكادر
              </Link>
            </div>
              ...
              {dashboardStats?.expiringContractsAlerts?.length > 0 ? (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-zinc-400 mt-0.5">
                          موعد التجديد المقترح: قبل 10 أيام من الانتهاء
                        </p>
                      </div>
                    </div>
                    <Link
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-zinc-500 text-xs font-bold uppercase tracking-widest leading-none">
                    مؤشر أمان العقود مستقر وممتاز
                  </p>
                  <p className="text-zinc-400 text-[10px] font-medium mt-1">
                    لا توجد عقود تنتهي خلال الـ 30 يوماً القادمة
                  </p>
                </div>
              )}
            </div>
          ...
          <section className="bg-white rounded-3xl border border-zinc-100 shadow-sm p-6 overflow-hidden">
            <div className="flex justify-between items-center mb-6">
              ...
              <Link to="/app/payroll" className="text-xs font-bold text-primary hover:underline">
                كل المسيرات
              </Link>
            </div>
      ...
      <div className="space-y-8 animate-fade-in" dir={isAr ? "rtl" : "ltr"}>
        ...
        <section className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          >
            ...
            <div>
              ...
              <p className="text-zinc-500 dark:text-zinc-400 text-sm font-black mb-1">
                التزامات ضريبة القيمة المضافة (Collected VAT)
              </p>
              <h3 className="text-3xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight">
                {collectedVat.toLocaleString()} ر.س
              </h3>
              <p className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-2 font-bold">
          ...
          >
            <div>
              ...
              <p className="text-zinc-500 dark:text-zinc-400 text-sm font-black mb-1">
                المدفوعات المستحقة للتحصيل (Receivables)
              </p>
              <h3 className="text-2xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight">
                {totalPendingAmountAr.toLocaleString()} ر.س
              </h3>
              <p className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-2 font-bold">
                فواتير بانتظار السداد أو التسوية للمرحلة 2
              </p>
            </div>
          </motion.div>
          ...
          >
            <div>
              ...
              <p className="text-zinc-500 dark:text-zinc-400 text-sm font-black mb-1">
                إجمالي كشوف الإيرادات المصونة (Paid Cash)
              </p>
              <h3 className="text-2xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight">
                {paidInvoicesAmount.toLocaleString()} ر.س
              </h3>
              <p className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-2 font-bold">
                التدفقات النقدية الداخلة التي طابقت بنجاح
              </p>
            </div>
          </motion.div>
        </section>
        ...
        <section className="bg-gradient-to-r from-primary/10 via-primary/5 to-transparent rounded-[2rem] border border-primary/20 p-6 relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 mb-6 relative z-10">
            ...
            <div>
              <h3 className="font-black text-xl text-zinc-900">
                محرك الرقابة والامتثال الضريبي (ZATCA Advisory)
              </h3>
              <p className="text-xs font-bold text-primary">
                توصيات حية للامتثال لمتطلبات الفوترة الإلكترونية والتقارير المالية للربع الحالي
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 relative z-10">
            <div className="bg-white p-5 rounded-3xl border border-primary/10 flex flex-col justify-between">
              <div>
                ...
                <h4 className="font-bold text-zinc-900 mt-2 mb-1">
                  الربط الإلكتروني للمرحلة الثانية
                </h4>
                <p className="text-xs text-zinc-500 leading-relaxed">
                  يتيح لك نظام مدارج إرسال فواتيرك مباشرة إلى بورتال فاتورة (ZATCA) لحظياً باستخدام
                  التوقيعات الرقمية المشفرة. فعل التكامل وتخلص من القلق تماماً.
                </p>
              </div>
              <Link
                to="/app/integrations"
            ...
            <div className="bg-white p-5 rounded-3xl border border-primary/10 flex flex-col justify-between">
              <div>
                ...
                <h4 className="font-bold text-zinc-900 mt-2 mb-1">
                  تحصيل المبالغ المستحقة المتأخرة
                </h4>
                <p className="text-xs text-zinc-500 leading-relaxed">
                  متوسط فترات السداد من عملائك ارتفعت بمعدل 5 أيام. أتمتة إرسال رسائل التذكير
                  بالفواتير عبر مدارج يقلل الذمم المدينة بنسبة 25%.
                </p>
              </div>
              <Link
                to="/app/invoices"
            ...
            <div className="bg-white p-5 rounded-3xl border border-primary/10 flex flex-col justify-between">
              <div>
                <span className="text-[9px] font-black bg-purple-50 text-purple-600 px-2 py-0.5 rounded-full border border-purple-100">
                  الإقرار الضريبي
                </span>
                <h4 className="font-bold text-zinc-900 mt-2 mb-1">أرشفة وحساب الإقرار بضغطة زر</h4>
                <p className="text-xs text-zinc-500 leading-relaxed">
                  قاربت نهاية الفترة الضريبية الحالية. نظام الفرز الآلي في مدارج يسمح لك بمشاهدة
                  وتحميل تقرير الإقرارات الربع سنوي المدقق المتوافق مع متطلبات الهيئة.
                </p>
              </div>
              <Link
                to="/app/analytics"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="p-6 bg-white rounded-3xl border border-zinc-100 shadow-sm h-[380px] lg:col-span-8 flex flex-col">
            ...
            <p className="text-xs text-zinc-400 font-bold mb-6">
              قيمة الفواتير الصادرة المعتمدة مقارنةً بالمتحصلات الفعلية شهرياً
            </p>
          ...
          <div className="p-6 bg-white rounded-3xl border border-zinc-100 shadow-sm lg:col-span-4 flex flex-col justify-between">
            <div>
              <h3 className="font-black text-zinc-900 text-base mb-1">
                تدقيق الضوابط والامتثال المالي
              </h3>
              <p className="text-[11px] text-zinc-400 font-bold mb-4">
                قائمة التحقق التفاعلية لسلامة الدفاتر
              </p>
              ...
              <div className="space-y-3.5 pt-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-zinc-600 font-bold">
                    تطابق الرقم الضريبي VAT ومرحلة ZATCA 2
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="p-1 rounded-lg bg-emerald-100 text-emerald-700 shrink-0">
                  ...
                  <span className="text-xs text-zinc-600 font-bold">
                    ترميز المنتجات وإشعارات الخصم الضريبية
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="p-1 rounded-lg bg-emerald-100 text-emerald-700 shrink-0">
                  ...
                  <span className="text-xs text-zinc-600 font-bold">
                    تسوية كشوف الأجور ومطابقتها لمسيرات تأمينات
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  {collectedVat > 0 ? (
                  ...
                  <span className="text-xs text-zinc-600 font-bold">
                    احتساب الفروقات الضريبية للداخلة والمخرجات
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-zinc-100 pt-4 mt-6">
              <span className="text-[10px] text-zinc-400 font-black tracking-widest uppercase block mb-1">
                الرتبة في شبكة مدارجOS
              </span>
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="font-black text-lg text-zinc-900">
                دفتر فواتير المبيعات الصادرة (المتكامل)
              </h3>
              <p className="text-xs text-zinc-500 font-medium">
                سجلات فواتيرك الصادرة من صفحة الفواتير في النظام مجلوبة حياً
              </p>
            </div>
            <Link to="/app/invoices" className="text-xs font-bold text-primary hover:underline">
              إدارة الفواتير
            </Link>
          </div>
      ...
      <div className="space-y-8 animate-fade-in" dir={isAr ? "rtl" : "ltr"}>
        ...
        <section className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          >
            ...
            <div>
              ...
              <p className="text-zinc-500 dark:text-zinc-400 text-sm font-black mb-1">
                إجمالي الشحنات النشطة (Active Shipments)
              </p>
              <h3 className="text-3xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight">
                {activeShipments.length} شحنة جارية
              </h3>
              <p className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-2 font-bold">
          ...
          >
            <div>
              ...
              <p className="text-zinc-500 dark:text-zinc-400 text-sm font-black mb-1">
                سجلات الاستيراد الموثقة (Historical)
              </p>
              <h3 className="text-2xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight">
                {shipmentsCount} شحنة إجمالية
              </h3>
              <p className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-2 font-bold">
                بين المخلص والناقل والمستودعات في الرياض
              </p>
            </div>
          </motion.div>
          ...
          >
            <div>
              ...
              <p className="text-zinc-500 dark:text-zinc-400 text-sm font-black mb-1">
                مخلصين جمارك معتمدين (Brokers linked)
              </p>
              <h3 className="text-2xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight">
                3 مخلصين نشطين
              </h3>
              <p className="text-[10px] text-zinc-400 dark:text-zinc-500 mt-2 font-bold">
                مرتبطين بفسح الجمركية بالسعودية
              </p>
            </div>
          </motion.div>
        </section>
        ...
        <section className="bg-gradient-to-r from-primary/10 via-primary/5 to-transparent rounded-[2rem] border border-primary/20 p-6 relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 mb-6 relative z-10">
            ...
            <div>
              <h3 className="font-black text-xl text-zinc-900">
                مساعد اللوجستية الذكي (Operations Advisory)
              </h3>
              <p className="text-xs font-bold text-primary">
                توصيات حية لتمثيل سلاسل التوريد وتتبع خطوط الشحن البحري والجوي
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 relative z-10">
            <div className="bg-white p-5 rounded-3xl border border-primary/10 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-zinc-500 leading-relaxed">
                  تأكد من إدراج رقم السجل التجاري والرمز الجمركي الموحد في حسابك لتفادي حدوث تعليق
                  المعاملات اللوجستية في الموانئ السعودية عن طريق ربط منصة فسح.
                </p>
              </div>
              <Link
                to="/app/suppliers"
            ...
            <div className="bg-white p-5 rounded-3xl border border-primary/10 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-zinc-500 leading-relaxed">
                  تم رصد تأخير بنسبة 4 أيام في موانئ الشحن المغادرة من جنوب شرق آسيا. ننصح بطلب
                  زيادة الكمية الاحتياطية لتفادي نفاد المخزون هذا الشهر.
                </p>
              </div>
              <Link
            ...
            <div className="bg-white p-5 rounded-3xl border border-primary/10 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-zinc-500 leading-relaxed">
                  بمجرد وصول الشحنة، يسمح لك مدارج بإنشاء مطابقة فواتير مشتركة وتحويل الفواتير
                  الأجنبية بعملات متعددة (USD, RMB, SAR) بذكاء وامتثال ضريبي.
                </p>
              </div>
              <Link
                to="/app/suppliers"
        ...
        <section className="bg-white rounded-3xl border border-zinc-100 shadow-sm p-6">
          <h3 className="font-black text-lg text-zinc-900 mb-1">
            خط سير الشحنات الدولي النشط (Transit Maps)
          </h3>
          <p className="text-xs text-zinc-500 font-medium mb-6">
            مراقب المسار المباشر للحاويات من ميناء التصدير لبلد المنشأ وحتى التسليم بالمستودعات
          </p>
          ...
          <div className="relative p-6 pt-12 md:p-12 bg-zinc-50 rounded-2xl border border-zinc-100 overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <div className="flex flex-col items-center text-center relative z-10 group">
              ...
              <p className="text-[10px] text-zinc-400 mt-1 w-32 font-bold">
                تحميل الشحنة وإتمام الجمارك بالخارج
              </p>
            </div>
            ...
            <div className="flex flex-col items-center text-center relative z-10 group">
              ...
              <p className="text-[10px] text-zinc-400 mt-1 w-32 font-bold">
                بين المحيطات وخطوط الملاحة البحري
              </p>
            </div>
            ...
            <div className="flex flex-col items-center text-center relative z-10 group">
              ...
              <p className="text-[10px] text-zinc-400 mt-1 w-32 font-bold">
                الاستلام والمطابقة وحساب التكلفة
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white rounded-3xl border border-zinc-100 shadow-sm p-6 overflow-hidden">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="font-black text-lg text-zinc-900">
                سجل الشحنات وسلاسل التوريد (المتصل)
              </h3>
              <p className="text-xs text-zinc-500 font-medium">
                قائمة الشحنات قيد التشغيل المجلوبة من قاعدة البيانات مباشرة
              </p>
            </div>
            <Link to="/app/suppliers" className="text-xs font-bold text-primary hover:underline">
              مراجعة الموردين والشحنات
            </Link>
          </div>
              ...
              <tbody className="divide-y divide-zinc-100">
                {activeShipments?.slice(0, 5).map((ship: any, idx: number) => (
                  <tr key={ship.id || idx} className="hover:bg-zinc-50/50 transition-colors">
                    ...
                    <td className="py-3 font-bold text-zinc-950">
                      {ship.originPort || "ميناء غير محدد"}
                    </td>
                    <td className="py-3 text-center text-zinc-700 font-bold">
                      {ship.destinationPort || "ميناء المملكة"}
                    </td>
                    <td className="py-3 text-center text-zinc-500 font-bold">
                      {ship.carrier || "ميرسك / ناقل لوجستي"}
                    </td>
                    <td className="py-3 text-center text-zinc-500 font-bold">
                      {ship.expectedDelivery
                        ? new Date(ship.expectedDelivery).toLocaleDateString("ar-SA")
                        : "-"}
                    </td>
                    <td className="py-3 text-center">
                      <span
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-20">
          ...
          <div className="flex items-center gap-4 relative z-10 w-full md:w-auto">
            ...
            <div>
              <h3 className="text-lg font-black text-rose-900 mb-1">
                تنبيه عاجل: بوابة واتساب (OpenWA) غير متصلة!
              </h3>
              <p className="text-sm font-bold text-rose-700">
                جاري توقف أتمتة الردود وحملات الرسائل والمزامنة التلقائية مع CRM بسبب انقطاع الجلسة.
              </p>
            </div>
          </div>
      ...
      {[...systemAlerts, ...(dashboardStats?.expiringContractsAlerts || [])].filter(
        ...
        <div className="space-y-4">
          {[...systemAlerts, ...(dashboardStats?.expiringContractsAlerts || [])]
            .filter((a) => !dismissedLocalAlerts.includes(a.id))
            .map((alert) => (
              <motion.div
                key={alert.id}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-black mb-1">
                حالة طوارئ: إيقاف الخدمات (Emergency Lockdown)
              </h2>
              <p className="text-sm font-medium text-rose-100">
                تم تجاوز المدة النظامية (15 يوم) لاعتماد ورفع مسير الرواتب لشهر{" "}
                {dashboardStats.lockdownPeriod}. بعض الخدمات ستظل مقيدة إلى حين المعالجة بملف WPS أو
                مدد.
              </p>
            </div>
          </div>
            ...
            {config.map((item) => (
              <Reorder.Item
                ...
                className={cn(
                  ...
                  item.visible
                    ? "border-zinc-200"
                    : "opacity-50 border-dashed border-zinc-300 bg-zinc-50"
                )}
              >
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-400 font-medium">
                      {item.visible ? "مرئي في اللوحة الرئيسية" : "مخفي من اللوحة الرئيسية"}
                    </p>
                  </div>
                </div>
                <button
          ...
          {activeView === "ceo" && (
            <>
              {config
                .filter((w) => w.visible)
                .map((widget) => (
                  <React.Fragment key={widget.id}>{renderWidget(widget.id)}</React.Fragment>
                ))}
            </>
          )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <h2 className="text-2xl font-black text-zinc-900 mb-4">
                أهلاً بك في نظام مدارج المتكامل!
              </h2>
              <p className="text-sm font-medium text-zinc-500 mb-8 leading-relaxed max-w-md">
                لقد تم إعداد مساحة العمل الخاصة بك بنجاح. مدارج يربط مبيعاتك، فواتيرك، رواتب موظفيك
                والشحن في مكان واحد متصل ومؤتمت. نصيحتنا للبدء هي إضافة عملائك المحتملين أو
                الحاليين.
              </p>
                ...
                <div className="flex items-center gap-3">
                  <div>
                    ...
                    <p className="text-xs font-bold text-emerald-600">
                      افتح واجهة واتساب ويب مباشرة بقوالب سعودية مسبقة
                    </p>
                  </div>
