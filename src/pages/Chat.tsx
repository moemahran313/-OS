...
export default function Chat() {
  ...
  // OpenWA Integration State
  const [isConfigOpen, setIsConfigOpen] = useState(false);
  const [openwaUrlInput, setOpenwaUrlInput] = useState(
    settings.openwaUrl || "http://localhost:8080"
  );
  const [openwaApiKeyInput, setOpenwaApiKeyInput] = useState(settings.openwaApiKey || "");
  const [openwaEnabledInput, setOpenwaEnabledInput] = useState(settings.openwaEnabled || false);
  const [isTesting, setIsTesting] = useState(false);
  const [testResult, setTestResult] = useState<{ success: boolean; message: string } | null>(null);
  const [isCopied, setIsCopied] = useState(false);

  useEffect(() => {
    if (settings.openwaUrl) setOpenwaUrlInput(settings.openwaUrl);
    if (settings.openwaApiKey !== undefined) setOpenwaApiKeyInput(settings.openwaApiKey);
    if (settings.openwaEnabled !== undefined) setOpenwaEnabledInput(settings.openwaEnabled);
  }, [settings]);
  ...
  const handleTestConnection = async () => {
    ...
    setTestResult(null);
    try {
      const response = await fetch("/api/openwa/test", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          openwaUrl: openwaUrlInput,
          openwaApiKey: openwaApiKeyInput,
        }),
      });

      const data = await response.json();
      if (response.ok && data.connected) {
        setTestResult({ success: true, message: data.message || "تم الاتصال بنجاح بمحرك OpenWA!" });
        toast.success("تم الاتصال بمحرك OpenWA بنجاح!");
      } else {
        setTestResult({ success: false, message: data.error || "فشل الاتصال بمحرك OpenWA." });
        toast.error("فشل الاتصال بمحرك OpenWA");
      }
    } catch (err: any) {
      setTestResult({ success: false, message: err.message || "خطأ في الشبكة" });
  ...
  const handleSaveConfig = () => {
    updateSettings({
      openwaUrl: openwaUrlInput,
      openwaApiKey: openwaApiKeyInput,
      openwaEnabled: openwaEnabledInput,
    });
    toast.success("تم حفظ إعدادات ربط واتساب بنجاح!");
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    if (settings.openwaEnabled && settings.openwaUrl) {
      try {
        ...
        console.error("OpenWA send request failed, fallback to local save", err);
  ...
  useEffect(() => {
    ...
    const unreadMessages =
      activeLead.messages?.filter((m) => m.sender === "client" && m.status !== "read") || [];
    if (unreadMessages.length > 0) {
      const updatedMessages =
        activeLead.messages?.map((m) => {
          if (m.sender === "client") {
  ...
  return (
    <div
      className="flex h-[calc(100vh-140px)] bg-zinc-50 rounded-[2.5rem] border border-zinc-200 shadow-sm overflow-hidden"
      dir="rtl"
    >
      ...
      <div className="w-[380px] border-l border-zinc-200 bg-white flex flex-col h-full shrink-0">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className={cn(
                  "p-1.5 rounded-xl border border-zinc-200 hover:border-primary/50 hover:bg-zinc-50 transition-all cursor-pointer",
                  isConfigOpen
                    ? "bg-primary/10 text-primary border-primary/20"
                    : "bg-white text-zinc-500"
                )}
                title="إعدادات ربط واتساب (OpenWA)"
              >
        ...
        {isConfigOpen ? (
          /* WhatsApp OpenWA Configuration View */
          <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-zinc-50/50">
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-extrabold text-xs text-zinc-500">تهيئة محرك ربط واتساب</h3>
              <button
                onClick={() => setIsConfigOpen(false)}
                className="text-xs font-bold text-zinc-600 hover:text-zinc-900 transition-colors bg-white px-2.5 py-1.5 rounded-xl border border-zinc-200 shadow-sm"
              >
                العودة للمحادثات
              </button>
            </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-xs font-black text-zinc-800 block">
                    تفعيل ربط واتساب حياً
                  </span>
                  <span className="text-[10px] text-zinc-500 block leading-relaxed mt-0.5">
                    تبديل بين التفعيل حياً أو محاكاة الـ AI بالكامل
                  </span>
                </div>
                <button
                  onClick={() => setOpenwaEnabledInput(!openwaEnabledInput)}
                  className={cn(
                    "w-11 h-6 rounded-full p-0.5 transition-colors duration-200 shrink-0 cursor-pointer",
                    openwaEnabledInput ? "bg-emerald-500" : "bg-zinc-300"
                  )}
                >
                  <div
                    className={cn(
                      "w-5 h-5 bg-white rounded-full shadow-md transform transition-transform duration-200",
                      openwaEnabledInput ? "-translate-x-5" : "translate-x-0"
                    )}
                  />
                </button>
              </div>
            ...
            <div className="bg-white border border-zinc-200 rounded-2xl p-4 shadow-sm space-y-4">
              <div>
                <label className="text-[11px] font-black text-zinc-500 block mb-1.5">
                  رابط خادم OpenWA API
                </label>
                <input
                  type="text"
                  value={openwaUrlInput}
                  onChange={(e) => setOpenwaUrlInput(e.target.value)}
                  placeholder="http://localhost:8080"
                  className="w-full text-xs font-bold px-3 py-2.5 bg-zinc-50 border border-zinc-200 rounded-xl focus:border-primary/50 focus:bg-white focus:outline-none transition-all"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="password"
                  value={openwaApiKeyInput}
                  onChange={(e) => setOpenwaApiKeyInput(e.target.value)}
                  placeholder="••••••••••••••••"
                  className="w-full text-xs font-bold px-3 py-2.5 bg-zinc-50 border border-zinc-200 rounded-xl focus:border-primary/50 focus:bg-white focus:outline-none transition-all"
                />
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  disabled={isTesting}
                  className="flex-1 bg-zinc-100 hover:bg-zinc-200 text-zinc-800 text-xs font-black py-2.5 rounded-xl border border-zinc-200 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                >
                  {isTesting ? "جاري الفحص..." : "فحص الاتصال"}
                </button>
                <button
                  onClick={handleSaveConfig}
                  className="flex-1 bg-primary hover:bg-primary/95 text-white text-xs font-black py-2.5 rounded-xl transition-all cursor-pointer"
                >
                  حفظ الإعدادات
                </button>
              </div>
              ...
              {testResult && (
                <div
                  className={cn(
                    "p-3 rounded-xl border text-[11px] font-bold flex items-start gap-2 animate-fadeIn",
                    testResult.success
            ...
            <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-4 shadow-sm space-y-3">
              <div>
                <span className="text-xs font-black text-emerald-900 block mb-1">
                  رابط الويب هوك (Webhook Link)
                </span>
                <span className="text-[10px] text-emerald-700/80 block leading-relaxed">
                  انسخ الرابط التالي وضعه كـ Webhook في خادم OpenWA لتلقي الرسائل وتحديثها في الـ
                  CRM تلقائياً:
                </span>
              </div>
              <div className="flex gap-1.5 items-center bg-white border border-emerald-100 rounded-xl p-2">
                <input
                  type="text"
                  readOnly
                  value={`${window.location.origin}/api/openwa/webhook?userId=${user?.uid}`}
                  className="flex-1 text-[10px] font-mono text-emerald-800 bg-transparent border-none outline-none select-all overflow-ellipsis"
                />
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(
                      `${window.location.origin}/api/openwa/webhook?userId=${user?.uid}`
                    );
                    setIsCopied(true);
                    toast.success("تم نسخ رابط الويب هوك!");
                    setTimeout(() => setIsCopied(false), 2000);
                  }}
                  className="px-2 py-1 text-[10px] font-black bg-emerald-100 text-emerald-700 rounded-lg hover:bg-emerald-200 transition-all shrink-0 cursor-pointer"
                >
            ...
            <div className="bg-white border border-zinc-200 rounded-2xl p-4 shadow-sm">
              <h4 className="text-[11px] font-black text-zinc-700 mb-2">
                تعليمات التهيئة السريعة:
              </h4>
              <ul className="list-decimal list-inside text-[10px] text-zinc-500 font-bold space-y-2 leading-relaxed">
                <li>قم بتشغيل خادم OpenWA محلياً.</li>
                <li>تأكد من إمكانية وصول خادم مدارج للـ API الخاص بـ OpenWA.</li>
                <li>ضع رابط الويب هوك الموضح أعلاه في إعدادات OpenWA الخاصة بك.</li>
                <li>الآن ستتدفق كافة الرسائل مباشرة في الـ CRM!</li>
              </ul>
            </div>
          </div>
        ) : (
              ...
              {filteredLeads.map((lead) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-full text-right p-4 flex items-start gap-3 transition-colors cursor-pointer",
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="text-[11px] font-black text-zinc-400 block mb-1 truncate">
                        {lead.company}
                      </span>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      toast.success(
                        `تم تحويل الصادرات لـ ${activeLead.name} إلى مسودة فاتورة معتمدة!`
                      );
                    }}
              ...
              <div className="p-4 border-t border-zinc-200 bg-white shrink-0">
                ...
                <div className="flex flex-wrap gap-2 mb-3 items-center">
                  ...
                  <button
                    onClick={() =>
                      sendQuickReply(
                        "أهلاً بك، تم إرسال مسودة العقد المالي معتمد بالخطوات. يرجى تزويدنا بالموافقة للتصنيف."
                      )
                    }
                  ...
                  <button
                    onClick={() =>
                      sendQuickReply(
                        "لقد قمنا بتوليد فاتورتكم الإلكترونية بالكامل. يمكنك مراجعتها وسدادها من خلال هذا الرابط المدقّق."
                      )
                    }
                    className="px-2.5 py-1 text-[11px] font-bold bg-zinc-100 hover:bg-primary/5 hover:text-primary hover:border-primary/30 text-zinc-600 border border-zinc-200 rounded-lg transition-all"
                  >
                  ...
                  <button
                    onClick={() =>
                      sendQuickReply(
                        "تم اعتماد ملف مدد ومطابقة كافة مسيرات الرواتب لشهر الالتزام بنجاح."
                      )
                    }
                    className="px-2.5 py-1 text-[11px] font-bold bg-zinc-100 hover:bg-primary/5 hover:text-primary hover:border-primary/30 text-zinc-600 border border-zinc-200 rounded-lg transition-all"
                  >
            ...
            <div>
              ...
              {(() => {
                ...
                const clientMessages =
                  activeLead.messages?.filter((m) => m.sender === "client").map((m) => m.text) ||
                  [];
                ...
                if (
                  fullText.includes("شراء") ||
                  fullText.includes("سعر") ||
                  fullText.includes("تسعيرة") ||
                  fullText.includes("دفع") ||
                  fullText.includes("فاتورة")
                ) {
                  intent = "جاهز للشراء (High Intent)";
                  score = 90;
                  colorClass = "text-emerald-700 bg-emerald-50 border-emerald-200";
                } else if (
                  fullText.includes("سؤال") ||
                  fullText.includes("استفسار") ||
                  fullText.includes("تفاصيل") ||
                  fullText.includes("كيف")
                ) {
                  intent = "جمع معلومات (Information Gathering)";
                  score = 60;
                  colorClass = "text-blue-700 bg-blue-50 border-blue-200";
                } else if (
                  fullText.includes("مشكلة") ||
                  fullText.includes("دعم") ||
                  fullText.includes("خطأ")
                ) {
                ...
                return (
                  <div className="space-y-4">
                    <div className="bg-zinc-50 border border-zinc-200 rounded-2xl p-4 shadow-sm">
                      ...
                      <div
                        className={`text-xs font-black px-3 py-1.5 rounded-lg border text-center ${colorClass}`}
                      >
                        {intent}
                      </div>
                    </div>
                    ...
                    <div className="bg-purple-50 border border-purple-100 rounded-2xl p-4 shadow-sm">
                      <h4 className="text-[11px] font-black text-purple-800 mb-2 uppercase tracking-wide">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {["تسعيرة", "دفع", "سريع", "عقد"]
                          ...
                          .map((keyword) => (
                            <span
                              key={keyword}
                              className="bg-purple-100 text-purple-700 px-2.5 py-0.5 rounded-full text-[10px] font-bold"
                            >
                              {keyword}
                            </span>
                          ))}
                        {["تسعيرة", "دفع", "سريع", "عقد"].filter((k) => fullText.includes(k))
                          .length === 0 && (
                          <span className="text-xs text-purple-600/70 font-medium">
                            لم يتم العثور على كلمات مفتاحية واضحة
                          </span>
                        )}
                      </div>
                    </div>
              ...
              <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-zinc-900">
                        سلسلة التخلي (Abandonment)
                      </h4>
                      <p className="text-[10px] text-zinc-500 mt-0.5 font-medium">
                        متابعة آلية بعد 24 ساعة من التجاهل
                      </p>
                    </div>
                  </div>
                  <div className="relative">
                    <input
                      type="checkbox"
                      id="abandonment-toggle"
                      className="sr-only peer"
                      defaultChecked
                    />
                    <label
                      htmlFor="abandonment-toggle"
                      className="w-9 h-5 bg-zinc-300 peer-checked:bg-primary rounded-full cursor-pointer relative block transition-colors before:content-[''] before:w-3.5 before:h-3.5 before:bg-white before:rounded-full before:absolute before:top-[3px] before:left-[3px] peer-checked:before:translate-x-4 before:transition-transform"
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between bg-zinc-50 border border-zinc-200 rounded-2xl p-4 shadow-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-zinc-900">
                        تذكير الدفع (Payment Nudge)
                      </h4>
                      <p className="text-[10px] text-zinc-500 mt-0.5 font-medium">
                        عند إصدار الفاتورة وعدم الدفع
                      </p>
                    </div>
                  </div>
                  <div className="relative">
