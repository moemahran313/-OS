...
export default function SmartNegotiations() {
  ...
  const [currentSubPage, setCurrentSubPage] = useState<"schedule" | "live" | "analyze" | "tasks">(
    "schedule"
  );
  ...
  const [meetingTitle, setMeetingTitle] = useState(
    isAr
      ? "اجتماع مراجعة عقد التوريد السنوي لشبكات الخوادم"
      : "Annual Server Networking Supply Contract Review"
  );
  const [status, setStatus] = useState<"Scheduled" | "Live" | "Analyzing" | "Completed">(
    "Completed"
  );
  ...
  const [transcriptLogs, setTranscriptLogs] = useState<
    { time: string; speaker: string; textAr: string; textEn: string }[]
  >([]);
  ...
  const [localMeetings, setLocalMeetings] = useState<
    { id: string; title: string; link: string; date: string }[]
  >([
    {
      id: "meet-1",
  ...
  const handleGoogleConnect = async () => {
    try {
      ...
      toast.success(
        isAr
          ? "تم الاتصال بحساب Google وتفويض صلاحية الاجتماعات!"
          : "Google Account successfully connected & Meet scopes authorized!"
      );
    } catch (err: any) {
      console.error(err);
      toast.error(
        isAr ? `فشل الاتصال بـ Google: ${err.message}` : `Google Connection failed: ${err.message}`
      );
    }
  };
  ...
  const handleCreateMeetRoom = async () => {
    if (!googleAccessToken) {
      toast.error(
        isAr
          ? "يرجى أولاً ربط وتفويض حساب Google لجدولة الاجتماعات"
          : "Please connect your Google Account first to authorize Meet API."
      );
      return;
    }
    ...
    const toastId = toast.loading(
      isAr
        ? "جاري الاتصال بـ Google API لإنشاء مساحة تفاوض حقيقية..."
        : "Contacting Google Meet API to provision a real-time negotiation space..."
    );
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success(
          isAr
            ? "تم إنشاء رابط Google Meet رسمي بنجاح!"
            : "Official Google Meet space created successfully!",
          { id: toastId }
        );
      ...
      toast.error(
        isAr
          ? `فشل تفعيل الغرفة من Google Meet: ${err.message}`
          : `Google Meet provisioning failed: ${err.message}`,
        { id: toastId }
      );
    } finally {
  ...
  const launchFloatingMeet = (explicitLink?: string) => {
    ...
    if (win) {
      ...
      toast.success(
        isAr
          ? "تم إطلاق نافذة الاجتماع الجانبية العائمة بنجاح! يمكنك الآن ترتيبها بجانب مدارج مدار الساعة."
          : "Floating Meet panel launched successfully side-by-side!"
      );
    } else {
      window.open(targetLink, "_blank");
      toast.info(
        isAr
          ? "تم فتح الاجتماع في تبويب جديد (تنبيه: نوصي بتمكين النوافذ المنبثقة من إعدادات المتصفح للاستمتاع بنافذة جانبية عائمة)"
          : "Opened in a new tab! (Tip: Allow popups for the side-by-side floating window experience)"
      );
    }
  };
  ...
  const analyzeTextWithGemini = async (inputText: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          isAr
            ? "عفواً، فشلت الخوادم في الاستجابة للذكاء الاصطناعي"
            : "AI Analysis route returned a failing response code."
        );
      }
      ...
      if (data.success) {
        ...
        setStatus("Completed");
        toast.success(isAr ? "اكتمل فحص الشروط بنجاح!" : "Legal terms analyzed successfully!", {
          id: loaderId,
        });
  ...
  const handleStartMic = () => {
    if (!recognitionRef.current) {
      toast.error(
        isAr
          ? "الميكروفون أو ميزة التعرف غير مدعومة في متصفحك الحالي"
          : "Web SpeechRecognition is not fully supported in your browser."
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        isAr
          ? "تم تفعيل الميكروفون الحقيقي وبث الاستماع الصوتي المباشر!"
          : "Native SpeechRecognition activated! Speak directly."
      );
  ...
  const handleStopMic = () => {
    ...
    const fullTranscript =
      transcriptLogs.length > 0
        ...
        : manualText ||
          (isAr
            ? "جلسة تفاوض حقيقية لتوريد أجهزة ومواصفات خوادم فنية بقيمة 85 ألف ريال مع صيانة SLAs لمدة 36 شهراً وشرط جزائي 1%."
            : "Annual Supply and warranty: Value of SAR 85,000, 36 months technical SLA coverage, late delivery penalty 1% with cap at 10% maximum.");
  ...
  const handleIngestManualText = () => {
    if (!manualText.trim()) {
      toast.error(
        isAr
          ? "يرجى كتابة أو لصق نصوص الشروط القانونية أولاً"
          : "Please paste legal text parameters first."
      );
      return;
    }
    ...
    const interval = setInterval(() => {
      setUploadProgress((prev) => {
        if (prev >= 100) {
          clearInterval(interval);
          toast.success(
            isAr ? "تم الانتهاء من قراءة البصمة الصوتية!" : "Acoustic signal digested successfully!"
          );
          // Send representation text of file to Gemini
          analyzeTextWithGemini(
            `Acoustic upload file import: ${file.name}. Core negotiation context includes 85,000 SAR value, 36 months comprehensive SLA, 1% delay cap penalty at 10% maximum and Riyadh jurisdiction.`
          );
          return 100;
        }
        return prev + 20;
  ...
  const handleBindToCLM = () => {
    ...
    extractedTerms.forEach((term) => {
      const isFirstParty =
        term.titleAr.includes("الأول") ||
        term.titleEn.toLowerCase().includes("first party") ||
        term.titleEn.toLowerCase().includes("employer");
      const isSecondParty =
        term.titleAr.includes("الثاني") ||
        term.titleEn.toLowerCase().includes("second party") ||
        term.titleEn.toLowerCase().includes("employee") ||
        term.titleEn.toLowerCase().includes("supplier");
      ...
      if (term.category === "Party") {
        ...
        (term.valueAr.includes("SCCA") ||
          term.valueEn.includes("SCCA") ||
          term.valueAr.includes("تحكيم") ||
          term.valueEn.includes("Arbitration"))
      ) {
        baseData.disputeResolution = "SCCA";
      } else if (
        term.category === "Condition" &&
        (term.valueAr.includes("المحاكم") || term.valueEn.includes("Court"))
      ) {
  ...
  const handleAddTask = () => {
    const newTask: NegotiationTask = {
      ...
      titleAr: isAr
        ? "مراجعة الشروط الإضافية مع المستشار القانوني"
        : "Review ad-hoc terms with legal consultant",
  ...
  const subPages = [
    {
      id: "schedule",
      titleAr: "📅 مخطط وجدول الاجتماعات",
      titleEn: "📅 Meetings & Scheduling Hub",
    },
    { id: "live", titleAr: "🎙️ الغرفة المباشرة والبث", titleEn: "🎙️ Live Meet Room & Transcript" },
    {
      id: "analyze",
      titleAr: "🧠 محلل الذكاء وصياغة CLM",
      titleEn: "🧠 Smart Terms & CLM Injector",
    },
    {
      id: "tasks",
      titleAr: "✅ مهام الامتثال والالتزامات",
      titleEn: "✅ Obligations Sync & Tasks",
    },
  ];
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-slate-200 pb-4 gap-4">
        <div>
          <span className="text-[10px] uppercase font-bold tracking-wider text-emerald-600 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-md">
            {isAr
              ? "قسم الاجتماعات والامتثال الذكي"
              : "Enterprise Conference & Smart Negotiation Core"}
          </span>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {isAr
              ? "جدولة، تتبع الحوار الصوتي المباشر، ومزامنة البنود مع محرك صياغة العقود CLM بدون بيانات وهمية"
              : "Connect real meetings, capture spoken audio transcripts, and inject real legal variables with zero mocks"}
          </p>
        </div>
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                <h2 className="text-sm font-bold text-slate-800 border-b border-slate-150 pb-2 flex items-center gap-2">
                  ...
                  <span>
                    {isAr ? "تخطيط وبدء غرفة اجتماع" : "Schedule & Provision Meeting Room"}
                  </span>
                </h2>
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 space-y-3 text-xs">
                  <h3 className="font-bold text-slate-700 flex items-center gap-1.5">
                    ...
                    <span>
                      {isAr ? "بروتوكول تفويض Google Meet" : "Google Meet Provisioning Policy"}
                    </span>
                  </h3>
                ...
                {googleAccessToken ? (
                  ...
                  >
                    ...
                    <span>
                      {isAr
                        ? "جدولة وإنشاء غرفة Google Meet"
                        : "Schedule & Launch Real Google Meet"}
                    </span>
                  </button>
                ) : (
            ...
            <div className="lg:col-span-7 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
              <h2 className="text-sm font-bold text-slate-800 border-b border-slate-150 pb-2 mb-4 flex items-center justify-between">
                <span className="flex items-center gap-2">
                  ...
                  <span>
                    {isAr
                      ? "قائمة الجلسات والاجتماعات المقررة"
                      : "Active Scheduled Negotiation Sessions"}
                  </span>
                </span>
              ...
              <div className="space-y-3">
                {localMeetings.map((meet) => (
                  <div
                    key={meet.id}
                    className="border border-slate-150 rounded-xl p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 hover:border-slate-300 transition-all bg-slate-50/40"
                  >
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto">
                      ...
                      <button
                        onClick={() => {
                          ...
                          toast.success(
                            isAr
                              ? "تم تهيئة الغرفة المفتوحة للبث المباشر والتفريغ"
                              : "Meeting room loaded inside Live transcription section!"
                          );
                        }}
                        ...
                        <div className="text-right flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-[10px] text-white font-black">
                              {isAr
                                ? "محاولة تضمين البث المباشر"
                                : "Forced Embedded Google Meet View"}
                            </p>
                            <p className="text-[8px] text-slate-300">
                              {isAr
                                ? "إذا ظهرت شاشة بيضاء أو رفض الاتصال، يرجى تفعيل اللوحة العائمة أدناه."
                                : "If refused/blank, use the floating window."}
                            </p>
                          </div>
                        </div>
                        <button
                    ...
                    <div className="absolute inset-0 w-full h-full flex flex-col items-center justify-center bg-gradient-to-b from-slate-900 to-slate-950 text-white p-6 space-y-4 md:space-y-6">
                      <div className="flex flex-col items-center space-y-2">
                        ...
                        <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">
                          ...
                          <span className="text-[10px] font-black tracking-wider uppercase font-mono">
                            {isAr ? "بث نشط ومتزامن" : "ACTIVE COGNITIVE SYNC"}
                          </span>
                        </div>
                        <h3 className="text-xs font-bold text-slate-100">
                          {isAr
                            ? "بوابة الغرفة المزدوجة لـ Google Meet"
                            : "Dual-Screen Companion Portal"}
                        </h3>
                      ...
                      <div className="flex flex-wrap items-center justify-center gap-3 w-full max-w-md">
                        ...
                        >
                          ...
                          <span>
                            {isAr
                              ? "إطلاق اللوحة العائمة الجانبية (موصى به)"
                              : "Launch Floating Side-Companion (Recommended)"}
                          </span>
                        </button>
                        <a
                          href={meetLink}
                      ...
                      <div className="pt-2">
                        ...
                        >
                          {isAr
                            ? "تجاوز الأمان ومحاولة التضمين المباشر داخل الصفحة (Iframe)"
                            : "Force direct in-page iframe embedding anyway"}
                        </button>
                      </div>
                    </div>
                  )
                  ...
                  <div className="space-y-3 text-slate-400">
                    ...
                    <div>
                      <h3 className="text-xs font-bold text-slate-300">
                        {isAr
                          ? "لم يتم ربط أي غرفة اجتماع جارية"
                          : "No active Google Meet session connected"}
                      </h3>
                      <p className="text-[10px] text-slate-500 mt-0.5">
                        {isAr
                          ? "يرجى إنشاء مساحة اجتماع في علامة التبويب الأولى أو تتبع تفريغ خارجي"
                          : "Configure and schedule a real meet room inside the scheduling panel."}
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-700">
                      {isAr
                        ? "تفريغ الصوت الفوري المباشر (Speech Recognition)"
                        : "Native Real-Time Voice Transcription"}
                    </span>
                  </div>
                ...
                <div className="h-14 bg-slate-950 border border-slate-900 rounded-lg overflow-hidden relative flex items-center justify-center">
                  {!isRecording ? (
                    <span className="text-[10px] text-slate-500 font-bold">
                      {isAr
                        ? "انقر 'بدء بث الميكروفون' للتحدث والتحويل المباشر"
                        : "Click 'Start Real-Time Microphone' to capture spoken words"}
                    </span>
                  ) : (
                  ...
                  {!isRecording ? (
                    ...
                    <>
                      ...
                      >
                        ...
                        <span>
                          {isPaused ? (isAr ? "استئناف" : "Resume") : isAr ? "إيقاف مؤقت" : "Pause"}
                        </span>
                      </button>
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col h-[320px]">
                <h3 className="text-xs font-black text-slate-800 border-b border-slate-150 pb-2 mb-3 flex items-center justify-between">
                  <span className="flex items-center gap-1.5">
                    ...
                    <span>
                      {isAr ? "سجل الحوار والتفريغ الفوري" : "Captured Transcription Log"}
                    </span>
                  </span>
                  <button
                  ...
                  {transcriptLogs.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center text-slate-400 space-y-2">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 mt-0.5">
                          {isAr
                            ? "تحدث بالميكروفون لتتبع الكلمات العربية والإنجليزية فورا"
                            : "Activate mic or select a file to translate voice patterns"}
                        </p>
                      </div>
                    </div>
                  ) : (
                    transcriptLogs.map((log, idx) => (
                      ...
                      >
                        ...
                        <p className="text-slate-800 font-bold text-right" dir="rtl">
                          {log.textAr}
                        </p>
                        <p className="text-slate-600 font-medium italic text-left" dir="ltr">
                          {log.textEn}
                        </p>
                      </div>
                    ))
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
                <h3 className="text-xs font-bold text-slate-800 border-b border-slate-150 pb-2 flex items-center gap-1.5">
                  ...
                  <span>
                    {isAr
                      ? "رفع تسجيل صوتي أو كتابة شروط يدوية"
                      : "Ingest Audio File or Manual Draft"}
                  </span>
                </h3>
                ...
                <div className="space-y-3">
                  <textarea
                    ...
                    placeholder={
                      isAr
                        ? "الصق أو اكتب الشروط والمسودة هنا لفرزها فورا عبر الذكاء الاصطناعي..."
                        : "Paste legal paragraphs or transcript here to analyze..."
                    }
                  ...
                  >
                    ...
                    <span>
                      {isAr ? "تحليل النص واستخراج البنود" : "Process Draft Terms with Gemini"}
                    </span>
                  </button>
                </div>
                ...
                <div className="border-t border-slate-100 pt-3">
                  ...
                  >
                    ...
                    <p className="text-[10px] font-bold text-slate-700">
                      {isAr ? "اسحب وألقِ تسجيل صوتي" : "Drag & Drop raw audio (WAV/MP3)"}
                    </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-200">
            ...
            <div className="lg:col-span-12 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3 relative overflow-hidden">
              ...
              <div className="flex items-center gap-1.5 border-b border-slate-100 pb-2">
                ...
                <h3 className="text-xs font-black text-slate-800">
                  {isAr
                    ? "الملخص التنفيذي القانوني الذكي (أداة Gemini 3.5)"
                    : "Cognitive Executive Summary (Gemini 3.5 Engine)"}
                </h3>
              </div>
              <p
                className="text-xs text-slate-800 leading-relaxed text-right font-semibold pr-3 border-r-2 border-emerald-500"
                dir="rtl"
              >
                {executiveSummaryAr}
              </p>
              ...
              <p
                className="text-xs text-slate-600 leading-relaxed text-left font-sans italic font-medium pl-3 border-l-2 border-emerald-500/50"
                dir="ltr"
              >
                {executiveSummaryEn}
              </p>
              ...
              <div>
                <h2 className="text-sm font-bold text-slate-800 border-b border-slate-150 pb-2 mb-4 flex items-center justify-between">
                  <span className="flex items-center gap-2">
                    ...
                    <span>
                      {isAr
                        ? "البنود القانونية المستخرجة وتدقيقها"
                        : "Extracted Legal Variables Audit"}
                    </span>
                  </span>
                  <span className="text-[10px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded font-bold">
                ...
                {extractedTerms.length === 0 ? (
                  <div className="text-center py-12 text-slate-400 space-y-2">
                    ...
                    <p className="text-xs font-bold">
                      {isAr ? "بانتظار قراءة أو تحليل الاجتماع" : "No variables extracted yet."}
                    </p>
                    <p className="text-[10px] text-slate-400">
                      {isAr
                        ? "ابدأ تسجيل الصوت أو اكتب مسودتك في الغرفة المباشرة"
                        : "Enter a session or paste legal draft to generate parameters."}
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {extractedTerms.map((term, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className="border border-slate-150 rounded-xl p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 hover:border-slate-300 transition-all bg-slate-50/40 relative overflow-hidden"
                        >
                          <div className="absolute top-0 right-0 w-1 h-full bg-emerald-500" />
                          <div className="space-y-1 flex-1">
                            <div className="flex items-center gap-2">
                              <span
                                className={`text-[9px] uppercase font-bold px-2 py-0.5 rounded border ${colors[term.category] || "bg-slate-50"}`}
                              >
                                {term.category}
                              </span>
                            ...
                            <p
                              className="text-[11px] text-slate-500 font-medium italic text-left"
                              dir="ltr"
                            >
                              {term.valueEn}
                            </p>
                ...
                <div className="border-t border-slate-100 pt-4 flex flex-col md:flex-row items-center justify-between gap-3 bg-slate-50/80 p-4 rounded-xl border border-slate-150">
                  <div className="text-right rtl:text-right ltr:text-left space-y-0.5">
                    <h4 className="text-xs font-bold text-slate-800">
                      {isAr ? "ربط وحقن البنود بمسودة CLM" : "Integrate directly with CLM module"}
                    </h4>
                    <p className="text-[9px] text-slate-450 text-slate-400 font-bold">
                      {isAr
                        ? "حقن هذه المتغيرات فورياً داخل قالب العقد القانوني النشط"
                        : "Instantly injects extracted parameters into your Contracts Editor"}
                    </p>
                  </div>
                  <button
            ...
            <div className="lg:col-span-5 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
              <h3 className="text-xs font-bold text-slate-800 border-b border-slate-150 pb-2 flex items-center gap-1.5">
                ...
                <span>
                  {isAr ? "منظور محاكاة العقد القانوني النشط" : "Active Legal Draft Live Render"}
                </span>
              </h3>
              ...
              <div className="bg-slate-50 border border-slate-150 rounded-xl p-4 font-serif text-[11px] leading-relaxed text-slate-700 h-[360px] overflow-y-auto space-y-3 shadow-inner">
                <div className="text-center font-bold text-xs text-slate-800 border-b border-slate-200 pb-2">
                  {isAr
                    ? "اتفاقية توريد وخدمات صيانة شبكات الخوادم"
                    : "Server Supplies & SLA Operational Agreement"}
                </div>
                ...
                {extractedTerms.map((term, i) => (
                  <div
                    key={i}
                    className="bg-white p-2.5 rounded border border-slate-200 font-sans text-[10px] space-y-1"
                  >
                    <div className="font-bold text-slate-800 flex items-center justify-between border-b border-slate-100 pb-1 flex-wrap">
                      ...
                      <span className="text-[9px] uppercase text-emerald-600 font-bold font-mono">
                        {term.category}
                      </span>
                    </div>
                    <p className="text-right text-slate-800 font-bold" dir="rtl">
                      {term.valueAr}
                    </p>
                    <p className="text-left text-slate-500 font-medium italic" dir="ltr">
                      {term.valueEn}
                    </p>
                  </div>
                ))}
                ...
                <p className="text-[10px] text-slate-400 text-center border-t border-slate-100 pt-3 italic">
                  {isAr
                    ? "خاضع للنظام والأنظمة القضائية التجارية للمملكة العربية السعودية"
                    : "Governed under the commercial regulatory guidelines of the Kingdom of Saudi Arabia"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-200">
            <div className="lg:col-span-8 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between space-y-6">
              <div>
                <h2 className="text-sm font-bold text-slate-800 border-b border-slate-150 pb-2 mb-4 flex items-center justify-between">
                  <span className="flex items-center gap-2">
                    ...
                    <span>
                      {isAr
                        ? "قائمة التكليفات والتعهدات والامتثال المالي"
                        : "Bilateral Obligations & Action Items Tracker"}
                    </span>
                  </span>
                ...
                {tasks.length === 0 ? (
                  <div className="text-center py-12 text-slate-400 space-y-2">
                    ...
                    <p className="text-xs font-bold">
                      {isAr ? "لا توجد التزامات معمدة حتى الآن" : "No parsed compliance tasks yet."}
                    </p>
                    <p className="text-[10px] text-slate-400">
                      {isAr
                        ? "قم بمعالجة التفريغ الصوتي لفرز التعهدات والغرامات تلقائياً"
                        : "Gemini extracts high-priority compliance obligations from transcripts instantly."}
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                      ...
                      >
                        <div className="flex items-start gap-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0 text-right rtl:text-right ltr:text-left">
                            <p
                              className={`text-xs font-bold leading-relaxed ${task.completed ? "line-through text-slate-400" : "text-slate-800"}`}
                            >
                            ...
                            <div className="flex items-center gap-3 mt-1.5 text-[9px] text-slate-500 font-bold flex-wrap">
                              ...
                              <span
                                className={`px-1.5 py-0.5 rounded uppercase text-[8px] font-bold border ${
                                  task.priority === "High"
              ...
              {tasks.length > 0 && (
                <div className="border-t border-slate-100 pt-4 flex flex-col md:flex-row items-center justify-between gap-3 bg-slate-50/80 p-4 rounded-xl border border-slate-150">
                  <div className="text-right rtl:text-right ltr:text-left space-y-0.5">
                    <h4 className="text-xs font-bold text-slate-800">
                      {isAr ? "مزامنة المهام مع لوحة الإدارة" : "Sync Actions with Workflows"}
                    </h4>
                    <p className="text-[9px] text-slate-400 font-bold">
                      {isAr
                        ? "تحويل شروط الامتثال والتكليفات فورياً لتراقبها كمسارات عمل تفاعلية"
                        : "Integrates obligations with global team workflows for active progress tracking"}
                    </p>
                  </div>
                  <button
            ...
            <div className="lg:col-span-4 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
              <h3 className="text-xs font-bold text-slate-800 border-b border-slate-150 pb-2 flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-emerald-600" />
                <span>
                  {isAr ? "مستوى الالتزام والصحة التنظيمية" : "Compliance & Risk Health Monitor"}
                </span>
              </h3>

              <div className="space-y-4 text-xs font-bold">
                <div className="bg-slate-50 border border-slate-150 p-4 rounded-xl space-y-2 text-center">
                  <span className="text-slate-450 text-[10px] text-slate-400 block uppercase">
                    {isAr ? "مؤشر صحة العقد" : "Contract Risk Rating"}
                  </span>
