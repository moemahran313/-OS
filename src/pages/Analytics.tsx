    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-20 text-center animate-pulse">
        ...
        <h2 className="text-2xl font-black text-zinc-900 mb-2">
          جاري تشغيل محرك التحليلات المتقدم...
        </h2>
        <p className="text-zinc-500 font-medium">
          نقوم بتحليل التدفق النقدي، سلوك العملاء، والتوقعات المستقبلية
        </p>
      </div>
    );
          ...
          <div className="flex flex-col md:flex-row gap-10">
            ...
            <div className="space-y-6">
              <h3 className="text-2xl font-black text-zinc-900 flex items-center gap-3">
                الملخص التنفيذي
                <span className="text-xs bg-zinc-100 text-zinc-500 px-3 py-1 rounded-full font-bold uppercase tracking-widest">
                  System Generated
                </span>
              </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="grid grid-cols-1 gap-4">
            {[
              ...
              },
              {
                ...
                unit: "أشهر",
                desc: "فترة استرداد التكلفة",
                icon: Clock,
                color: "text-amber-500",
              },
              {
                label: "Margin",
                value: data.unitEconomics.margin,
                unit: "%",
                desc: "هامش الربح الإجمالي",
                icon: DollarSign,
                color: "text-primary",
              },
            ].map((eco, idx) => (
              <div
                key={idx}
                className="bg-white p-6 rounded-[2rem] border border-zinc-100 shadow-sm flex items-center gap-4 group hover:border-zinc-300 transition-all"
              >
                <div
                  className={cn(
                    "w-12 h-12 rounded-xl bg-zinc-50 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform",
                    eco.color
                  )}
                >
                ...
                <div>
                  <div className="flex items-baseline gap-1">
                    ...
                    <span className="text-[10px] font-bold text-zinc-400">
                      {typeof eco.value === "number" ? eco.unit : ""}
                    </span>
                  </div>
        ...
        <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-6">
          {data.keyMetrics.map((kpi, i) => (
            <div
              key={i}
              className="bg-white p-10 rounded-[3rem] border border-zinc-100 shadow-sm flex flex-col justify-between group hover:shadow-2xl hover:shadow-zinc-100 transition-all"
            >
              <div className="flex justify-between items-start mb-8">
                <div>
                  <h3 className="text-sm font-black text-zinc-900 mb-1">{kpi.label}</h3>
                  <p className="text-[10px] text-zinc-400 font-medium max-w-[200px] leading-relaxed">
                    {kpi.description}
                  </p>
                </div>
                <div
                ...
                <div className="pt-6 border-t border-zinc-50">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {kpi.subDrivers.map((drive, di) => (
                      <span
                        key={di}
                        className="px-4 py-2 bg-zinc-50 text-zinc-600 rounded-xl text-[10px] font-bold border border-zinc-100/50"
                      >
                        {drive}
                      </span>
                    ))}
          ...
          <div className="relative z-10 space-y-12 h-full flex flex-col">
            <header className="flex justify-between items-start">
              <div>
                ...
                <p className="text-zinc-400 text-xs mt-1 font-medium italic">
                  Forecast Mode: Projected Revenue Growth (Upcoming 180 Days)
                </p>
              </div>
            ...
            <div className="flex-1 min-h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart
                  data={data.forecast.scenarios[0].data.map((point, i) => ({
                    label: point.label,
                    base: point.value,
            ...
            <footer className="pt-8 border-t border-white/5">
              <div className="flex flex-wrap gap-4 items-center">
                <span className="text-[10px] font-black text-zinc-500 uppercase">
                  عوامل الحسم المتغيرة:
                </span>
                {data.forecast.variables.map((v, idx) => (
                  <span
                    key={idx}
                    className="px-5 py-2 bg-white/5 border border-white/10 rounded-full text-[10px] font-bold text-zinc-300"
                  >
                    {v}
                  </span>
                ))}
                ...
                <div key={idx} className="space-y-4">
                  <div className="flex justify-between items-end">
                    <div>
                      ...
                      <span className="text-[10px] bg-amber-50 text-amber-600 px-2.5 py-1 rounded-lg font-bold mt-1 inline-block">
                        {bench.rating}
                      </span>
                    </div>
                    <span className="text-3xl font-black text-zinc-900 tracking-tighter">
                      {bench.current.toFixed(0)}%
                    </span>
                  </div>
                  <div className="h-4 bg-zinc-100 rounded-full relative overflow-hidden">
                    <div
                      className="absolute top-0 right-0 h-full bg-zinc-900 rounded-full"
                      style={{ width: `${bench.current}%` }}
                    />
                    <div
                      className="absolute top-0 right-0 h-full w-[2px] bg-emerald-500 z-10"
                      style={{ right: `${bench.industryAvg}%` }}
                    >
                      <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[8px] font-black text-emerald-600 whitespace-nowrap">
                        متوسط السوق ({bench.industryAvg}%)
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="flex flex-col md:flex-row gap-10">
          <div className="shrink-0 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-rose-600 text-xs font-bold mt-1">
                نظام رصد المخاطر المدعوم بالذكاء الاصطناعي
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
            {data.alerts.map((alert, i) => (
              <div
                key={i}
                className="bg-white p-6 rounded-[2rem] border border-rose-100 flex gap-5 group hover:shadow-xl hover:shadow-rose-100 transition-all"
              >
        ...
        <div className="lg:col-span-2 bg-white rounded-[3rem] border-2 border-zinc-100 p-12 space-y-10 shadow-sm">
          ...
          <div className="space-y-6">
            {data.actionPlan.map((rec, i) => (
              <div
                key={i}
                className="group cursor-default bg-zinc-50/50 p-6 rounded-[2rem] border border-zinc-100 hover:bg-white hover:border-zinc-300 transition-all"
              >
                  ...
                  <div className="flex gap-4 items-center">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-1">
                        ...
                        >
                          التأثير: {rec.impact === "high" ? "عالي" : "متوسط"}
                        </span>
                        <span className="text-[9px] bg-zinc-100 text-zinc-500 px-2.5 py-1 rounded-full font-black uppercase">
                          الجهد: {rec.effort === "low" ? "بسيط" : "متوسط"}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-zinc-200/50">
                  <div>
                    <span className="text-[9px] font-black text-zinc-400 uppercase block mb-1">
                      المفاضلة الاستراتيجية / Trade-off
                    </span>
                  ...
                  <div>
                    <span className="text-[9px] font-black text-zinc-400 uppercase block mb-1">
                      الأثر الثانوي / 2nd Order Effect
                    </span>
                    <p className="text-[11px] text-zinc-600 font-bold leading-relaxed">
                      {rec.secondOrderEffect}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-primary rounded-[3rem] p-12 text-zinc-950 flex flex-col justify-between shadow-2xl shadow-primary/20 scale-[1.02] relative z-10 border-4 border-white">
          <div className="space-y-8">
            ...
            <h3 className="text-3xl font-black leading-tight">
              لو كنت مكانك، هذا هو القرار الوحيد الذي سأبدأ به الآن:
            </h3>
            ...
            <div className="flex items-center gap-3">
              <div className="flex -space-x-2">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="w-8 h-8 rounded-full border-2 border-primary bg-zinc-900 flex items-center justify-center text-[10px] font-black text-primary"
                  >
                    CEO
                  </div>
                ))}
              </div>
              <span className="text-[10px] font-black uppercase tracking-normal text-zinc-900/60">
                بناءً على بروتوكول Mudarij الاستشاري
              </span>
            </div>
