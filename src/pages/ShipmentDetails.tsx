...
const statusSteps = [
  ...
  { id: "documents_ready", label: "جاهز للمستندات", description: "اكتمال الأوراق الجمركية", icon: FileText },
  { id: "in_transit", label: "في الطريق", description: "الشحنة مع الناقل الدولي", icon: Globe },
  { id: "at_customs", label: "في الجمارك", description: "فحص بضائع الجمارك السعودية", icon: ShieldCheck },
...
export default function ShipmentDetails() {
  ...
  useEffect(() => {
    ...
    const unsubShipment = onSnapshot(doc(db, "shipments", id), (snap) => {
      if (snap.exists()) {
        ...
        setShipment({ id: snap.id, ...data, documents: data.documents || [], comments: data.comments || [], events: data.events || [] });
        setLoading(false);
      } else {
        setShipment(null);
        setLoading(false);
    ...
    // Real-time Comments
    const unsubComments = onSnapshot(query(collection(db, `shipments/${id}/comments`), orderBy("createdAt", "desc")), (snap) => {
      setShipment((prev: any) => prev ? { ...prev, comments: snap.docs.map(d => ({ id: d.id, ...d.data() })) } : prev);
    });

    // Real-time Documents
    const unsubDocs = onSnapshot(collection(db, `shipments/${id}/documents`), (snap) => {
      setShipment((prev: any) => prev ? { ...prev, documents: snap.docs.map(d => ({ id: d.id, ...d.data() })) } : prev);
    });

    // Real-time Events
    const unsubEvents = onSnapshot(query(collection(db, `shipments/${id}/events`), orderBy("createdAt", "desc")), (snap) => {
      setShipment((prev: any) => prev ? { ...prev, events: snap.docs.map(d => ({ id: d.id, ...d.data() })) } : prev);
    });
  ...
  const shareEmail = () => {
    const subject = encodeURIComponent(`تفاصيل الشحنة: ${shipment?.alias || shipment?.supplierName}`);
  ...
  const trackRealTime = async (silent = false) => {
    if (!silent) toast.info("جاري التتبع الحي من الناقل...", { icon: <Activity className="animate-spin" /> });
    try {
      // Simulate real-time tracking since we don't have a backend tracking API
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      const trackingData = {
        status: ['SHIPPED', 'IN_TRANSIT', 'OUT_FOR_DELIVERY', 'DELIVERED'][Math.floor(Math.random() * 4)],
        location: `${shipment?.originPort || 'City'} -> ${shipment?.destinationPort || 'City'}`,
        lastUpdate: new Date().toISOString(),
        estimatedDelivery: shipment?.estimatedDeliveryDate ? new Date(shipment.estimatedDeliveryDate).toLocaleDateString('ar-SA') : 'غير محدد',
        events: [
          { status: "Departure from Origin Port", location: shipment?.originPort || "Origin", timestamp: new Date(Date.now() - 86400000 * 2).toISOString() },
          { status: "In Transit via Sea Freight", location: "Arabian Sea", timestamp: new Date(Date.now() - 86400000).toISOString() },
          { status: "Arrived at destination port", location: shipment?.destinationPort || "Destination", timestamp: new Date().toISOString() }
        ]
      };
  ...
  const statusHistory = shipment.statusHistory ? (typeof shipment.statusHistory === 'string' ? JSON.parse(shipment.statusHistory) : shipment.statusHistory) : ["planned"];
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate(-1)} className="p-3 bg-white border border-zinc-100 rounded-2xl hover:bg-zinc-50 transition-all text-zinc-400 hover:text-zinc-900">
             <ArrowRight className="w-5 h-5" />
          </button>
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-black text-zinc-900 tracking-tight">{shipment.alias || shipment.supplierName}</h1>
              <span className="bg-primary/10 text-primary px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest leading-none flex items-center h-6">{shipment.status}</span>
            </div>
        ...
        <div className="flex gap-3 w-full md:w-auto">
           ...
           <button onClick={() => trackRealTime()} className="flex-1 md:flex-none px-6 py-3 bg-zinc-900 text-white rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-zinc-800 transition-all shadow-xl shadow-zinc-900/10">
      ...
      {shipment.estimatedDeliveryDate && (
        ...
        >
          ...
          <div className="flex items-center gap-4 relative z-10">
             ...
             <div>
                <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1">الموعد المتوقع للتوصيل</p>
                <h2 className="text-3xl font-black text-emerald-950 flex items-center gap-3">
                  {new Date(shipment.estimatedDeliveryDate).toLocaleDateString('ar-SA', { day: 'numeric', month: 'long', year: 'numeric' })}
                  <span className="text-sm font-medium text-emerald-600 bg-white px-3 py-1 rounded-full border border-emerald-200">بعد {(new Date(shipment.estimatedDeliveryDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24) > 0 ? Math.ceil((new Date(shipment.estimatedDeliveryDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)) : 0} أيام</span>
                </h2>
             </div>
          </div>
        </motion.div>
      ...
      <AnimatePresence mode="wait">
        {shipment.status === "in_transit" && !trackingInfo ? (
          ...
          >
            ...
            <p className="text-sm text-zinc-500 font-medium tracking-tight">نقوم حالياً بجلب أحدث بيانات التتبع الحية لشحنتك برقم t{shipment.trackingNumber}</p>
          </motion.div>
        ) : trackingInfo && (
          <motion.div 
            key="tracking-data"
          ...
          >
               ...
               <div className="flex items-center gap-6">
                  ...
                  <div>
                     <div className="flex items-center gap-2 mb-1">
                        ...
                        <p className="text-[10px] font-black text-white/40 uppercase tracking-widest leading-none">تتبع حي مباشر ({shipment.carrier})</p>
                     </div>
                     <h3 className="text-2xl font-black text-white flex items-center gap-2">
                       {trackingInfo.status === 'SHIPPED' ? 'تم الشحن' : 
                        trackingInfo.status === 'IN_TRANSIT' ? 'قيد النقل' : 
                        trackingInfo.status === 'OUT_FOR_DELIVERY' ? 'خارج للتوصيل' : 
                        trackingInfo.status === 'DELIVERED' ? 'تم التوصيل' : trackingInfo.status}
                     </h3>
                  </div>
               </div>
               <div className="flex flex-col md:items-end gap-1">
                  <p className="text-[10px] font-black text-white/40 uppercase tracking-widest leading-none">الموقع الجغرافي الحالي</p>
               ...
               {trackingInfo.events.map((event: any, i: number) => (
                  <div key={i} className="relative group">
                     <div className="flex flex-col gap-2 p-4 bg-white/5 rounded-2xl border border-white/5 group-hover:bg-white/10 transition-colors h-full">
                        <span className="text-[9px] font-black text-white/40 uppercase tracking-wider">{new Date(event.timestamp).toLocaleDateString('ar-SA')}</span>
            ...
            <div className="mt-8 flex justify-between items-center text-[10px] font-bold text-white/40 uppercase tracking-widest">
               <div className="flex items-center gap-2">
                  ...
                  وقت آخر تحديث: {new Date(trackingInfo.lastUpdate).toLocaleTimeString('ar-SA')} ({new Date(trackingInfo.lastUpdate).toLocaleDateString('ar-SA')})
               </div>
                          ...
                          <div className="relative">
                            <motion.div 
                              ...
                              className={cn(
                                ...
                                isCompleted ? "bg-zinc-900 text-primary shadow-xl shadow-zinc-900/10" : "bg-white border-2 border-zinc-100 text-zinc-300",
                                isCurrent && "ring-8 ring-primary/10 shadow-[0_0_30px_rgba(16,185,129,0.3)] bg-zinc-900 text-primary scale-110",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] border border-zinc-100 shadow-sm overflow-hidden p-8">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-6">
                     <h3 className="text-sm font-black text-zinc-400 uppercase tracking-widest">مسار الشحنة</h3>
                     <div className="relative pl-8 border-r-2 border-zinc-100 space-y-8 pr-4">
                        <div className="relative">
                          ...
                          <p className="text-lg font-bold text-zinc-900">{shipment.originPort || "N/A"}</p>
                        ...
                        <div className="relative">
                          ...
                          <p className="text-lg font-bold text-zinc-900">{shipment.destinationPort || "N/A"}</p>
                  ...
                  <div className="bg-zinc-50 rounded-3xl p-6 space-y-4">
                     <h3 className="text-xs font-black text-zinc-400 uppercase tracking-widest">تفاصيل لوجستية</h3>
                     <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="col-span-2 pt-2 border-t border-zinc-200/50">
                           ...
                           <p className="text-sm font-medium text-zinc-700 leading-relaxed mt-1">{shipment.productDescription}</p>
                        </div>
                     </div>
                  </div>
               </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-zinc-100 shadow-sm p-8 space-y-6">
                ...
                {complianceReport?.riskFlags.length > 0 && (
                  <div className="space-y-2">
                     <p className="text-[10px] font-black text-rose-400 uppercase tracking-widest">تنبيهات حرجة</p>
                     <div className="grid gap-2">
                        {complianceReport.riskFlags.map((risk: string) => (
                           <div key={risk} className="bg-rose-500 text-white p-4 rounded-2xl flex items-center gap-3 shadow-lg shadow-rose-500/20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   ...
                   <div className="space-y-4">
                      <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">رفع المستندات المفقودة</p>
                    ...
                    <div key={comment.id} className="flex gap-4 group">
                       ...
                       <div className="flex-1 space-y-1">
                          <div className="flex items-center gap-2">
                             ...
                             <span className="text-[10px] text-zinc-400 font-bold tracking-widest uppercase">{new Date(comment.createdAt).toLocaleTimeString('ar-SA')}</span>
                          </div>
                          <div className="bg-zinc-50 p-4 rounded-2xl rounded-tr-none border border-zinc-100 group-hover:bg-zinc-100 transition-colors">
                             <p className="text-sm text-zinc-700 leading-relaxed font-medium">{comment.text}</p>
                          </div>
                          <div className="flex gap-4 mr-2 mt-1">
                             <button className="text-[10px] font-black text-zinc-400 uppercase hover:text-primary transition-colors">رد</button>
                             <button className="text-[10px] font-black text-zinc-400 uppercase hover:text-primary transition-colors">تفاعل</button>
                          </div>
                       </div>
                    </div>
                  ))}
            ...
            <div className="bg-white rounded-[2.5rem] border border-zinc-100 shadow-sm p-8 overflow-hidden relative group">
               ...
               <div className="space-y-8 relative pr-8 border-r-2 border-zinc-50 max-h-[500px] overflow-y-auto scrollbar-hide py-2">
                  {shipment.events?.length > 0 ? shipment.events.sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((event: any, i: number) => (
                    <div key={event.id} className="relative group/item">
                       ...
                       <div className="bg-zinc-50 border border-zinc-100 p-5 rounded-3xl group-hover/item:bg-white group-hover/item:border-primary group-hover/item:shadow-lg transition-all duration-300">
                          <div className="flex justify-between items-start mb-2">
                            <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em]">{event.type.replace('shipment.', '')}</p>
                            <div className="text-left">
                               <p className="text-[9px] font-black text-zinc-400 font-mono">{new Date(event.createdAt).toLocaleDateString('ar-SA')}</p>
                               <p className="text-[9px] font-medium text-zinc-400">{new Date(event.createdAt).toLocaleTimeString('ar-SA')}</p>
                            </div>
                          </div>
                          <p className="text-sm font-bold text-zinc-800 leading-relaxed mb-1">{event.description}</p>
                          {event.metadata && (
                            <div className="mt-2 p-2 bg-white/50 rounded-lg border border-zinc-100/50">
                               <pre className="text-[8px] font-mono text-zinc-400 truncate">{event.metadata}</pre>
                            </div>
                          )}
                       </div>
                    </div>
         ...
         <div className="space-y-8">
            ...
            <div className="bg-white rounded-3xl border border-zinc-100 p-6 shadow-sm">
               <h3 className="text-xs font-black text-zinc-400 uppercase tracking-widest mb-4 font-mono">الارتباط بنظام CRM</h3>
               {shipment.clientId ? (
                 <div className="space-y-3">
                   <div className="bg-amber-50 rounded-2xl p-4 flex items-center gap-4 border border-amber-100">
                      ...
                      <div className="flex-1 min-w-0">
                         <p className="text-xs font-black text-zinc-900 truncate">
                           {leads.find(l => l.id === (shipment as any).clientId)?.name || "عميل غير موجود"}
                         </p>
                         <p className="text-[10px] text-zinc-500 font-medium truncate">استباق المبيعات / Lead</p>
                      </div>
                   </div>
                   <div className="flex gap-2">
                     <Link to="/app/crm" className="flex-1 py-2 bg-zinc-100 text-zinc-900 rounded-lg text-[10px] font-black text-center uppercase tracking-widest hover:bg-zinc-200 transition-all">فتح الملف</Link>
                     <button 
                       onClick={async () => {
                         if (!id) return;
                         try {
                 ...
                 <div className="space-y-3">
                    <p className="text-[10px] text-zinc-400 font-medium">اختر عميلاً لربطه بهذه الشحنة:</p>
                    <select 
                    ...
                    >
                      ...
                      {leads.map(lead => (
                        <option key={lead.id} value={lead.id}>{lead.name} - {lead.company} ({lead.status})</option>
                      ))}
                    </select>
            ...
            {/* Broker Info */}
            <div className="bg-white rounded-3xl border border-zinc-100 p-6 shadow-sm">
               <h3 className="text-xs font-black text-zinc-400 uppercase tracking-widest mb-4">المخلص المعين</h3>
               <div className="bg-zinc-50 rounded-2xl p-4 flex items-center gap-4 border border-zinc-100">
                  ...
                  <div>
                     <p className="text-sm font-black text-zinc-900">{shipment.broker?.name || "نظام مدارج الآلي"}</p>
               ...
               <div className="mt-4 flex gap-2">
                  <button 
                    onClick={() => {
                      if (shipment.broker?.email) {
                        window.open(`mailto:${shipment.broker.email}?subject=بخصوص شحنة: ${shipment.alias || shipment.supplierName}`);
                      } else {
                        toast.info("لا يوجد بريد إلكتروني مسجل للمخلص، جاري تحويلك للمحادثة المباشرة...");
                        document.querySelector('.comment-textarea')?.scrollIntoView({ behavior: 'smooth' });
                      }
                    }}
                  ...
                  <button className="p-3 bg-zinc-100 text-zinc-600 rounded-xl hover:bg-zinc-200 transition-all"><MoreVertical className="w-4 h-4" /></button>
               </div>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-zinc-100 p-6 shadow-sm">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-xs font-black text-zinc-400 uppercase tracking-widest">المستندات المرفوعة</h3>
            ...
            <div className="bg-gradient-to-br from-zinc-900 to-zinc-950 rounded-[2rem] p-8 text-white relative overflow-hidden">
               ...
               <div className="relative z-10">
                  <Sparkles className={cn("w-8 h-8 text-primary mb-4", isAnalyzing && "animate-pulse")} />
                  ...
                  <AnimatePresence mode="wait">
                    {!aiResponse && !isAnalyzing && (
                      ...
                      >
                        <p className="text-white/60 text-xs font-medium leading-relaxed mb-6">
                          هل تود إجراء تحليل عميق لمتطلبات الاستيراد لهذه الشحنة بناءً على لوائح المملكة؟
                        </p>
                        <button 
                    ...
                    {isAnalyzing && (
                      ...
                      >
                         ...
                         <p className="text-xs font-bold text-white/50">جاري مراجعة الأنظمة الجمركية...</p>
                      </motion.div>
                    )}
         ...
         {showShareModal && (
           <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
             ...
             >
                ...
                <p className="text-sm text-zinc-500 font-medium mb-8">شارك رابط تتبع المباشر مع العميل أو فريق العمل.</p>
                ...
                <div className="space-y-4">
                   ...
                   >
                      <div className="flex items-center gap-4">
                         ...
                         <span className="text-sm font-bold text-zinc-900 tracking-tight">نسخ الرابط المباشر</span>
                      </div>
                   ...
                   >
                      <div className="flex items-center gap-4">
                         ...
                         <span className="text-sm font-bold text-zinc-900 tracking-tight">إرسال عبر البريد الإلكتروني</span>
                      </div>
                      <ChevronRight className="w-4 h-4 text-zinc-300" />
                   </button>
                </div>
