...
import { 
  Blocks, Search, Plus, Play, Cpu, Lock, CheckCircle2, XCircle, Clock, 
  Zap, ArrowRight, ArrowLeft, BellRing, Settings2, Smartphone, AlertTriangle, 
  GitPullRequest, Workflow, Database, Save, Send, X, ShieldAlert, FileText, Mail, 
  Download, Printer, RefreshCw, Sparkles, Building2, UserCheck, HelpCircle, ChevronRight
} from 'lucide-react';
...
export default function Workflows() {
  ...
  const [vatSteps, setVatSteps] = useState<WorkflowStep[]>([
    { id: 1, key: 'receive', title: 'استقبال المعاملات اليومية', desc: 'استيراد الفواتير من أنظمة المبيعات ودفاتر الأستاذ لشركة مدارج', icon: Blocks, status: 'not_started', statusText: 'بانتظار البدء' },
    { id: 2, key: 'validate', title: 'تدقيق ومطابقة الفواتير', desc: 'مطابقة العمليات الحسابية وصحة فحص ضريبة المدخلات والمخرجات (15%)', icon: FileText, status: 'not_started', statusText: 'بانتظار البدء' },
    { id: 3, key: 'zatca', title: 'فحص متطلبات هيئة ZATCA', desc: 'التحقق من الأرقام الضريبية (15 خانة) ورموز الاستجابة السريعة والقوانين المنظمة للربط', icon: ShieldAlert, status: 'not_started', statusText: 'بانتظار البدء' },
    { id: 4, key: 'prepare', title: 'تجهيز مسودة إقرار (Form 50)', desc: 'احتساب المخرجات والمشتريات وتوزيع البنود الضريبية آلياً', icon: GitPullRequest, status: 'not_started', statusText: 'بانتظار البدء' },
    { id: 5, key: 'approve', title: 'اعتماد الشريك المعتمد (SOCPA)', desc: 'إشعار الشريك لمراجعة المسودة وتعديلها رقمياً بنظام الموافقات', icon: UserCheck, status: 'not_started', statusText: 'بانتظار البدء' },
    { id: 6, key: 'submit', title: 'إرسال الإقرار إلى الهيئة', desc: 'محاكاة تقديم البيانات عبر واجهة ربط الهيئة العامة للزكاة والضريبة والجمارك', icon: Send, status: 'not_started', statusText: 'بانتظار البدء' },
    { id: 7, key: 'archive', title: 'الأرشفة والأمان الرقمي', desc: 'مزامنة الإقرار المعتمد وتشفير المستند النهائي ومنع التعديل مستقبلياً', icon: Lock, status: 'not_started', statusText: 'بانتظار البدء' },
  ]);

  // Audit automated steps definition
  const [auditSteps, setAuditSteps] = useState<WorkflowStep[]>([
    { id: 1, key: 'upload', title: 'رفع الكشوفات البنكية للمطابقة', desc: 'استيراد الحركات المالية البنكية ومطابقتها بمسيرات النقد الداخلي', icon: Database, status: 'not_started', statusText: 'بانتظار البدء' },
    { id: 2, key: 'ai_categorize', title: 'التصنيف التلقائي الذكي (AI)', desc: 'تحليل القيود البنكية وفرز المصروفات والأرباح بالاعتماد على الفرز المالي المتطور', icon: Cpu, status: 'not_started', statusText: 'بانتظار البدء' },
    { id: 3, key: 'missing_docs', title: 'كشف المستندات المفقودة', desc: 'رصد المعاملات والمدفوعات البنكية التي تفتقر لفاتورة ضريبية رسمية مبسطة', icon: AlertTriangle, status: 'not_started', statusText: 'بانتظار البدء' },
    { id: 4, key: 'accountant_review', title: 'التسويات المحاسبية والمراجعة', desc: 'تطبيق تسويات التدقيق والتحقق اليدوي للشريك المحاسبي لاعتبارات الوعاء الزكوي', icon: Settings2, status: 'not_started', statusText: 'بانتظار البدء' },
    { id: 5, key: 'audit_pack', title: 'تجهيز حزمة التدقيق والمصادقة', desc: 'توليد الرأي المحاسبي النهائي والتقرير التشغيلي مع استخلاص الرسوم البيانية والأهداف', icon: Zap, status: 'not_started', statusText: 'بانتظار البدء' },
  ]);

  // Current active workflow steps helper
  const currentSteps = activeTab === 'vat' ? vatSteps : (activeTab === 'audit' ? auditSteps : negotiationSteps);
  const updateSteps = activeTab === 'vat' ? setVatSteps : (activeTab === 'audit' ? setAuditSteps : setNegotiationSteps);
  ...
  // Drag and Touch Handlers on Canvas grid
  ...
  useEffect(() => {
    ...
    setLogs([
      `تم التغيير إلى مسار: ${tabLabel}`,
      "بانتظار الضغط على تشغيل الخطوة الأولى للبدء."
    ]);
  }, [activeTab]);
  ...
  const handleExecuteStep = async (stepIdx: number) => {
    ...
    newSteps[stepIdx] = { ...targetStep, status: 'running', statusText: 'جاري المعالجة الرقمية...' };
    ...
    try {
      if (activeTab === 'negotiation') {
        ...
        setStepPayload({
          ...
          findings: isSuccess ? undefined : ["الالتزام المالي يحتاج لربط مباشر بالفاتورة الإلكترونية الموحدة ZATCA."]
        });
        ...
        updatedSteps[stepIdx] = { 
          ...targetStep, 
          status: finalStatus, 
          statusText: finalStatus === 'completed' ? 'اكتمل بنجاح' : (finalStatus === 'warning' ? 'انتهى مع وجود ملاحظات' : 'رصدت تنبيهات امتثال حرجة!') 
        };
        updateSteps(updatedSteps);
  ...
  const runEntirePipeline = async () => {
    setLogs(prev => [`[${new Date().toLocaleTimeString('ar-SA')}] ⚡ تم وضع المسار بالكامل تحت وضع الأتمتة الفائقة والمحاكاة المنظمة.`, ...prev]);
  ...
  // Reset steps
  const resetPipeline = () => {
    const freshSteps = currentSteps.map(s => ({ ...s, status: 'not_started' as const, statusText: 'بانتظار البدء' }));
  ...
  const handleAskAdvisor = async (questionText: string) => {
    ...
    try {
      ...
      setChatHistory(prev => [...prev, { sender: 'ai', text: data.answer || "عذراً، لم أتمكن من العثور على رد كافٍ في الوقت الحالي." }]);
    } catch (err: any) {
      setChatHistory(prev => [...prev, { sender: 'ai', text: `❌ فشل الاتصال بالمساعد الذكي: ${err.message}. الرجاء فحص إعدادات مفتاح Gemini الخاص بك.` }]);
    } finally {
  ...
  const renderFormattedText = (txt: string) => {
    return txt.split('\n').map((line, idx) => {
      ...
      while ((match = boldRegex.exec(content)) !== null) {
        ...
        parts.push(<strong key={match.index} className="text-zinc-900 font-extrabold bg-zinc-100 px-1.5 py-0.5 rounded border border-zinc-200/50">{match[1]}</strong>);
      ...
      if (isHeader) {
        return <h4 key={idx} className="text-sm font-black text-zinc-900 mt-4 mb-2 border-b border-zinc-100 pb-1">{finalLine}</h4>;
      }
      if (isList) {
        return <li key={idx} className="list-disc list-inside text-xs font-semibold text-zinc-700 mr-2 my-1 leading-relaxed">{finalLine}</li>;
      }
      return <p key={idx} className="text-xs font-medium text-zinc-600 my-1 leading-relaxed">{finalLine || <br />}</p>;
    });
  };
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-4 bg-zinc-900 text-white p-8 rounded-3xl relative overflow-hidden shadow-2xl border border-zinc-800">
        ...
        <div className="relative z-10 space-y-2">
          ...
          <p className="text-zinc-300 max-w-2xl text-sm font-semibold">
            أتمت معالجة فحص ضريبة القيمة المضافة ومطابقة الأرصدة البنكية آلياً وفقاً لقواعد امتثال <strong className="text-indigo-300">هيئة الزكاة والضريبة والجمارك (ZATCA)</strong> وإرشادات المراجعة المعتمدة للشركات السعودية.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-3 space-y-4">
          ...
          <div className="bg-white border border-zinc-200 rounded-3xl p-4 shadow-sm space-y-2">
            <label className="text-xs font-black text-zinc-400 block uppercase tracking-wider mb-2">اختر محرك الأتمتة المالي</label>
            <button 
            ...
            >
              <div className={`p-2 rounded-xl shrink-0 ${activeTab === 'vat' ? 'bg-indigo-600 text-white' : 'bg-zinc-100 text-zinc-500'}`}>
            ...
            >
              <div className={`p-2 rounded-xl shrink-0 ${activeTab === 'audit' ? 'bg-indigo-600 text-white' : 'bg-zinc-100 text-zinc-500'}`}>
                <Database className="w-4 h-4" />
              </div>
              <div className="flex-1">
                <span className="block text-xs font-bold text-zinc-400">Audit Compliance Flow</span>
                <span className="font-extrabold text-sm block">2. مسار تدقيق المستندات والقوائم</span>
              </div>
            </button>
            ...
            {negotiationSteps.length > 0 && (
              ...
              >
                <div className={`p-2 rounded-xl shrink-0 ${activeTab === 'negotiation' ? 'bg-emerald-600 text-white' : 'bg-zinc-100 text-zinc-500'}`}>
                  <Workflow className="w-4 h-4" />
                </div>
                <div className="flex-1">
                  <span className="block text-xs font-bold text-zinc-400">Negotiation Compliance Flow</span>
                  <span className="font-extrabold text-sm block">3. تدقيق شروط ومخرجات المفاوضات</span>
                </div>
              </button>
            )}
            ...
            >
              <div className={`p-2 rounded-xl shrink-0 ${activeTab === 'custom' ? 'bg-indigo-650 text-white' : 'bg-zinc-100 text-zinc-500'}`}>
          ...
          {activeTab === 'custom' ? (
            <div className="bg-white border border-zinc-200 rounded-3xl p-4 shadow-sm space-y-4">
              <h3 className="text-xs font-black uppercase text-zinc-400 px-1">لوحة العُقد البرمجية (Palette Library)</h3>
              
              <div className="space-y-4">
                {/* Triggers Category */}
                <div>
                  <span className="text-[10px] font-black text-rose-500 uppercase tracking-widest px-1 block mb-2">1. مشغلات البداية / Triggers</span>
                  <div className="space-y-2">
                    <button 
                      onClick={() => addCustomNode('trigger', 'sales_trigger')}
                      className="w-full text-right p-3 rounded-2xl border border-zinc-100 hover:bg-rose-50/50 bg-white transition-all flex items-center gap-3 text-sm font-bold"
                    >
                      <div className="p-2 rounded-xl bg-rose-50 text-rose-600 shrink-0">
                        <Play className="w-3.5 h-3.5 fill-current" />
                      </div>
                      <div>
                        <span className="font-extrabold text-xs block text-zinc-800">حافز فواتير المبيعات</span>
                        <span className="text-[10px] text-zinc-410 block font-semibold">تشغيل فوري عند رصد فواتير</span>
                      </div>
                    </button>

                    <button 
                      onClick={() => addCustomNode('trigger', 'schedule_trigger')}
                      className="w-full text-right p-3 rounded-2xl border border-zinc-100 hover:bg-rose-50/50 bg-white transition-all flex items-center gap-3 text-sm font-bold"
                    >
                      <div className="p-2 rounded-xl bg-zinc-100 text-zinc-650 shrink-0">
                        <Clock className="w-3.5 h-3.5" />
                      </div>
                      <div>
                        <span className="font-extrabold text-xs block text-zinc-800">مجدول الفحص الزمني</span>
                        <span className="text-[10px] text-zinc-410 block font-semibold">تشغيل ذكي مجدول شهرياً</span>
                      </div>
                    </button>

                    <button 
                      onClick={() => addCustomNode('trigger', 'erp_trigger')}
                      className="w-full text-right p-3 rounded-2xl border border-zinc-100 hover:bg-rose-50/50 bg-white transition-all flex items-center gap-3 text-sm font-bold"
                    >
                      <div className="p-2 rounded-xl bg-zinc-100 text-zinc-650 shrink-0">
                      ...
                      <div>
                        <span className="font-extrabold text-xs block text-zinc-800">حافز قيود نظام ERP</span>
                        <span className="text-[10px] text-zinc-410 block font-semibold">عند استلام قيود ماليّة</span>
                      </div>
                    </button>
                  </div>
                </div>
                ...
                {/* Actions Category */}
                <div>
                  <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest px-1 block mb-2">2. إجراءات الأتمتة / Actions</span>
                  <div className="space-y-2 max-h-56 overflow-y-auto">
                    <button 
                      onClick={() => addCustomNode('action', 'zatca_verify')}
                      className="w-full text-right p-3 rounded-2xl border border-zinc-100 hover:bg-indigo-50/50 bg-white transition-all flex items-center gap-3 text-sm font-bold"
                    >
                      <div className="p-2 rounded-xl bg-indigo-50 text-indigo-600 shrink-0">
                        <ShieldAlert className="w-3.5 h-3.5" />
                      </div>
                      <div>
                        <span className="font-extrabold text-xs block text-zinc-800">تحقق ومطابقة ZATCA</span>
                        <span className="text-[10px] text-zinc-410 block font-semibold">التحقق من صحة الفاتورة والختم</span>
                      </div>
                    </button>

                    <button 
                      onClick={() => addCustomNode('action', 'slack_email_alert')}
                      className="w-full text-right p-3 rounded-2xl border border-zinc-100 hover:bg-indigo-50/50 bg-white transition-all flex items-center gap-3 text-sm font-bold"
                    >
                      <div className="p-2 rounded-xl bg-indigo-50 text-indigo-600 shrink-0">
                      ...
                      <div>
                        <span className="font-extrabold text-xs block text-zinc-800">إرسال التنبيه البريدي</span>
                        <span className="text-[10px] text-zinc-410 block font-semibold">إشعار بريدي أو Slack فوري</span>
                      </div>
                    </button>

                    <button 
                      onClick={() => addCustomNode('action', 'gosi_tax_audit')}
                      className="w-full text-right p-3 rounded-2xl border border-zinc-100 hover:bg-indigo-50/50 bg-white transition-all flex items-center gap-3 text-sm font-bold"
                    >
                      <div className="p-2 rounded-xl bg-indigo-50 text-indigo-600 shrink-0">
                      ...
                      <div>
                        <span className="font-extrabold text-xs block text-zinc-800">فحص التأمينات GOSI</span>
                        <span className="text-[10px] text-zinc-410 block font-semibold">مطابقة كشوف الرواتب والتأمينات</span>
                      </div>
                    </button>

                    <button 
                      onClick={() => addCustomNode('action', 'sms_notifier')}
                      className="w-full text-right p-3 rounded-2xl border border-zinc-100 hover:bg-indigo-50/50 bg-white transition-all flex items-center gap-3 text-sm font-bold"
                    >
                      <div className="p-2 rounded-xl bg-indigo-50 text-indigo-600 shrink-0">
                      ...
                      <div>
                        <span className="font-extrabold text-xs block text-zinc-800">إشعار العميل (SMS)</span>
                        <span className="text-[10px] text-zinc-410 block font-semibold">إرسال رابط التحقق الهاتفي</span>
                      </div>
                    </button>

                    <button 
                      onClick={() => addCustomNode('action', 'bank_match')}
                      className="w-full text-right p-3 rounded-2xl border border-zinc-100 hover:bg-indigo-50/50 bg-white transition-all flex items-center gap-3 text-sm font-bold"
                    >
                      <div className="p-2 rounded-xl bg-indigo-50 text-indigo-600 shrink-0">
                      ...
                      <div>
                        <span className="font-extrabold text-xs block text-zinc-800">مطابقة الكشوفات البنكية</span>
                        <span className="text-[10px] text-zinc-410 block font-semibold">تحليل كشف النقديات بدقة</span>
                      </div>
                    </button>
                  </div>
                </div>
                ...
                {/* Conditions Category */}
                <div>
                  <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest px-1 block mb-2">3. شروط ومنافذ منطقية / Logic</span>
                  <div className="space-y-2">
                    <button 
                      onClick={() => addCustomNode('condition', 'socpa_check')}
                      className="w-full text-right p-3 rounded-2xl border border-zinc-100 hover:bg-amber-50/50 bg-white transition-all flex items-center gap-3 text-sm font-bold"
                    >
                      <div className="p-2 rounded-xl bg-amber-50 text-amber-600 shrink-0">
                        <AlertTriangle className="w-3.5 h-3.5" />
                      </div>
                      <div>
                        <span className="font-extrabold text-xs block text-zinc-800">بوابة مستشار SOCPA</span>
                        <span className="text-[10px] text-zinc-410 block font-semibold">اشتراط توقيع الشريك للمرور</span>
                      </div>
                    </button>

                    <button 
                      onClick={() => addCustomNode('condition', 'balance_capacity_check')}
                      className="w-full text-right p-3 rounded-2xl border border-zinc-100 hover:bg-amber-50/50 bg-white transition-all flex items-center gap-3 text-sm font-bold"
                    >
                      <div className="p-2 rounded-xl bg-amber-50 text-amber-600 shrink-0">
                      ...
                      <div>
                        <span className="font-extrabold text-xs block text-zinc-800">ملاءة التغطيّة النقديّة</span>
                        <span className="text-[10px] text-zinc-410 block font-semibold">فحص رصيد الحساب قبل الصرف</span>
                      </div>
                    </button>
                  </div>
                </div>
            ...
            <div className="bg-white border border-zinc-200 rounded-3xl p-4 shadow-sm space-y-3">
              <h3 className="text-xs font-black uppercase text-zinc-400 px-1">خطوات المسار البرمجي الممنهجة</h3>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 border text-xs font-bold ${
                          ...
                          step.status === 'warning' ? 'bg-amber-50 text-amber-600 border-amber-200' :
                          step.status === 'error' ? 'bg-rose-50 text-rose-600 border-rose-200' :
                          step.status === 'running' ? 'bg-blue-50 text-blue-600 border-blue-200 animate-pulse' :
                          'bg-zinc-50 text-zinc-400 border-zinc-200'
                        }`}>
                          {step.status === 'completed' ? <CheckCircle2 className="w-4 h-4 text-emerald-500" /> : 
                           step.status === 'warning' ? <AlertTriangle className="w-4 h-4 text-amber-500" /> : 
                           step.status === 'error' ? <XCircle className="w-4 h-4 text-rose-500" /> : 
                           idx + 1}
                        </div>

                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <h4 className="text-xs font-black text-zinc-900 truncate leading-none">{step.title}</h4>
                          </div>
                          <p className="text-[10px] text-zinc-500 mt-1 truncate leading-none">{step.desc}</p>
                        </div>
                        ...
                        <button 
                          ...
                          className={`p-1.5 rounded-lg border hover:bg-indigo-600 hover:text-white transition-colors shrink-0 ${
                            step.status === 'completed' ? 'border-emerald-200 text-emerald-600 bg-emerald-50' : 'border-zinc-200 text-zinc-400 bg-zinc-50'
                          }`}
                        >
          ...
          <div className="bg-white border border-zinc-200 rounded-3xl p-6 shadow-sm min-h-[580px] flex flex-col justify-between">
            <div>
              ...
              <div className="border-b border-zinc-100 pb-4 mb-4 flex justify-between items-start">
                <div>
                  <span className="bg-zinc-100 text-zinc-600 text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-wider mb-1 inline-block">الخطوة {currentStepIndex + 1} من {currentSteps.length}</span>
                  <h2 className="text-lg font-black text-zinc-900 flex items-center gap-2 mt-1">
                    {currentSteps[currentStepIndex].title}
                  </h2>
                  <p className="text-xs font-medium text-zinc-500 mt-1">{currentSteps[currentStepIndex].desc}</p>
                </div>
                ...
                {stepPayload ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className={`p-4 rounded-2xl border flex items-start gap-3 ${
                      stepPayload.status === 'warning' ? 'bg-amber-50/50 border-amber-200 text-amber-900' :
                      stepPayload.status === 'error' ? 'bg-rose-50/50 border-rose-200 text-rose-950' :
                      'bg-emerald-50/50 border-emerald-200 text-emerald-900'
                    }`}>
                      <div className="mt-0.5">
                        {stepPayload.status === 'warning' ? <AlertTriangle className="w-5 h-5 text-amber-500" /> :
                         stepPayload.status === 'error' ? <XCircle className="w-5 h-5 text-rose-500" /> :
                         <CheckCircle2 className="w-5 h-5 text-emerald-500" />}
                      </div>
                      <div className="flex-1">
                        <h4 className="text-sm font-extrabold">منظومة الأتمتة المكتملة:</h4>
                        <p className="text-xs font-medium text-zinc-600 mt-1 leading-relaxed">{stepPayload.message}</p>
                    ...
                    {activeTab === 'vat' && currentStepIndex === 0 && (
                      ...
                      <div className="space-y-3">
                        <h3 className="text-xs font-black text-zinc-400 block uppercase">قائمة المعاملات المكتشفة في قواعد البيانات:</h3>
                        <div className="border border-zinc-100 rounded-2xl overflow-hidden shadow-inner max-h-[290px] overflow-y-auto">
                          <table className="w-full text-right text-xs">
                            <thead className="bg-zinc-50 border-b border-zinc-100 sticky top-0">
                              <tr>
                                <th className="p-3 font-extrabold text-zinc-500">المورد/العميل</th>
                                <th className="p-3 font-extrabold text-zinc-500">ميزة الضريبة</th>
                                <th className="p-3 font-extrabold text-zinc-500 text-left">القيمة الأساسية</th>
                                <th className="p-3 font-extrabold text-zinc-500 text-left">صافي الإجمالي</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-zinc-50 font-semibold">
                              {stepPayload.data?.transactions.map((tx: any) => (
                                <tr key={tx.id} className="hover:bg-zinc-50/50">
                                  <td className="p-3">
                                    <span className="block font-black text-zinc-800">{tx.supplierName || tx.buyerName}</span>
                                    <span className="text-[10px] text-zinc-400">مستند: {tx.docNumber} | {tx.date}</span>
                                  </td>
                                  <td className="p-3">
                                    <span className={`inline-flex px-1.5 py-0.5 rounded text-[10px] ${tx.type === 'sales' ? 'bg-blue-50 text-blue-600' : 'bg-amber-50 text-amber-600'}`}>
                                      {tx.type === 'sales' ? 'مبيعات' : 'مشتريات'}
                                    </span>
                                  </td>
                                  <td className="p-3 text-left font-mono">{tx.amountBeforeVat.toLocaleString()} SAR</td>
                                  <td className="p-3 text-left text-zinc-900 font-mono font-black">{tx.total.toLocaleString()} SAR</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                    ...
                    {activeTab === 'vat' && currentStepIndex === 1 && (
                      ...
                      <div className="space-y-2 bg-zinc-50 p-4 rounded-2xl border border-zinc-100 h-64 overflow-y-auto">
                        <span className="text-xs font-black text-zinc-400 block mb-2">أخطاء العمليات الحسابية المكتشفة:</span>
                        {stepPayload.details ? (
                          <div className="space-y-2">
                            ...
                            <div className="bg-emerald-50 text-emerald-700 p-3 rounded-xl border border-emerald-200 text-xs font-semibold">
                              ✓ تم التحقق من باقي مستندات الفواتير المتبقية بنسبة نجاح محاسبي 90%.
                            </div>
                          </div>
                        ) : (
                          <p className="text-xs font-medium text-zinc-500">لا توجد ملاحظات حسابية رياضية لمطابقتها.</p>
                        )}
                      </div>
                    )}
                    ...
                    {activeTab === 'vat' && currentStepIndex === 2 && (
                      ...
                      <div className="space-y-3">
                        <span className="text-xs font-black text-rose-500 block">شعبة تحليل امتثال الزكاة والفوترة الإلكترونية (ZATCA Rules):</span>
                        <div className="space-y-2 h-[260px] overflow-y-auto">
                          {stepPayload.nonCompliantList?.map((c: any, i: number) => (
                            <div key={i} className="bg-rose-50 border border-rose-100 p-3.5 rounded-2xl flex items-start gap-2 text-rose-950">
                              <AlertTriangle className="w-4 h-4 text-rose-600 shrink-0 mt-0.5" />
                              <div className="text-xs flex-1">
                                <h5 className="font-extrabold">المستند: {c.doc} - {c.entity}</h5>
                                <p className="text-xs text-rose-700 font-semibold mt-1">{c.reason}</p>
                              </div>
                            </div>
                          ))}
                          <div className="bg-emerald-50 border border-emerald-100 text-emerald-800 p-3 rounded-2xl text-xs font-semibold">
                            💡 توصية الغرفة الضريبية: قم بمراسلة الموردين فوراً لطلب تصحيح أرقام السجلات الضريبية وإعادة إصدارها بنسبة ضريبة 15% صحيحة لمنع الالتزامات المزدوجة.
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {activeTab === 'vat' && currentStepIndex === 3 && (
                      ...
                      <div className="bg-[#f0f4f8] p-5 rounded-2xl border-2 border-indigo-200 space-y-3">
                        <div className="text-center border-b border-indigo-100 pb-3">
                          ...
                          <h4 className="text-sm font-black text-indigo-900 mt-1">مسودة نموذج الإقرار الضريبي الرسمي لقيمة الضريبة المضافة (نموذج 50)</h4>
                          <span className="text-[10px] text-zinc-400 font-bold">الهيئة العامة للزكاة والضريبة والجمارك</span>
                        </div>
                        
                        <div className="divide-y divide-indigo-100 text-xs font-bold space-y-2">
                          <div className="flex justify-between items-center pt-2">
                            <span className="text-zinc-600">البند 1: المبيعات الخاضعة للنسبة الأساسية (15%)</span>
                            <div className="text-left">
                              <span className="block font-mono text-zinc-900">المبلغ الخاضع للضريبة: {stepPayload.form50?.sales_standard.amount.toLocaleString()} SAR</span>
                              <span className="block font-mono text-indigo-700">قيمة الضريبة: {stepPayload.form50?.sales_standard.vat.toLocaleString()} SAR</span>
                            </div>
                          </div>
                          
                          <div className="flex justify-between items-center pt-2">
                            <span className="text-zinc-600">البند 9: المشتريات الخاضعة للنسبة الأساسية (15%)</span>
                            <div className="text-left font-semibold">
                              <span className="block font-mono text-zinc-900">المبلغ الخاضع للضريبة: {stepPayload.form50?.purchases_standard.amount.toLocaleString()} SAR</span>
                              <span className="block font-mono text-rose-600">قيمة الضريبة: {stepPayload.form50?.purchases_standard.vat.toLocaleString()} SAR</span>
                            </div>
                          </div>
                          ...
                          <div className="flex justify-between items-center pt-2.5 text-sm font-black text-indigo-950 border-t-2 border-indigo-200">
                            ...
                            <span className="font-mono text-emerald-700 text-lg">{stepPayload.form50?.net_vat_due.toLocaleString()} SAR</span>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {activeTab === 'vat' && currentStepIndex === 5 && (
                      ...
                      <div className="bg-zinc-900 text-emerald-400 p-5 rounded-2xl border-2 border-emerald-500/20 font-mono text-xs space-y-2">
                        <div className="flex justify-between items-center border-b border-emerald-500/10 pb-2">
                          <span className="font-sans font-black text-zinc-300">طلب تقديم معتمد لـ ZATCA</span>
                          <span className="bg-emerald-500/20 px-2 py-0.5 rounded text-[10px] text-emerald-300 uppercase font-bold">Submitted</span>
                        </div>
                        <p>رقم المرجع: {stepPayload.zatcaReceipt?.referenceNumber}</p>
                        <p>وقت التقديم: {stepPayload.zatcaReceipt?.timestamp}</p>
                        <p className="break-all whitespace-pre-wrap">الختم الرقمي لمصلحة الضريبة: {stepPayload.zatcaReceipt?.digitalStamp}</p>
                        <div className="bg-zinc-800 text-zinc-400 p-2.5 rounded text-[10px] font-sans">
                          * تم توثيق المعاملة وإدراج التوقيع المشفر للهيئة عبر منصة Mudarij Sandbox بنجاح كامل.
                        </div>
                      </div>
                    )}
                    ...
                    {activeTab === 'audit' && currentStepIndex === 1 && (
                      ...
                      <div className="space-y-3">
                        <p className="text-xs font-black text-zinc-400 block uppercase">نتائج فرز وتصنيف بنود المصروفات بواسطة (AI Engine):</p>
                        <div className="h-44 bg-zinc-50 rounded-2xl border border-zinc-100 p-3">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={stepPayload.categories}>
                              <XAxis dataKey="name" tick={{ fontSize: 10, fill: '#6c757d', fontWeight: 'bold' }} />
                              <YAxis tick={{ fontSize: 10, fill: '#6c757d' }} />
                              <Tooltip />
                              <Bar dataKey="sum" fill="#4f46e5" radius={[5, 5, 0, 0]} name="قيمة التصنيف (SAR)" />
                            </BarChart>
                          </ResponsiveContainer>
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 text-center text-[10px] font-bold">
                          {stepPayload.categories?.map((cat: any, i: number) => (
                            <div key={i} className="bg-zinc-50 border border-zinc-100 p-1.5 rounded-lg">
                              <span className="block text-zinc-500">{cat.name}</span>
                              <span className="block text-zinc-900 mt-0.5">{cat.sum.toLocaleString()} SAR</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {activeTab === 'audit' && currentStepIndex === 2 && (
                      ...
                      <div className="space-y-3">
                        <span className="text-xs font-black text-rose-500 block">فجوات المستندات والمدفوعات المفقودة (Missing Docs Alert):</span>
                        <div className="border border-zinc-100 rounded-2xl overflow-hidden shadow-inner max-h-[250px] overflow-y-auto">
                          <table className="w-full text-right text-xs">
                            <thead className="bg-zinc-50 border-b border-zinc-100">
                              <tr>
                                <th className="p-3 font-extrabold text-zinc-500">تاريخ المعاملة البنكية</th>
                                <th className="p-3 font-extrabold text-zinc-500">حركة القيود المالية</th>
                                <th className="p-2 font-extrabold text-zinc-500 text-left">قيمة العجز المستندي</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-zinc-50 font-semibold text-rose-950">
                              {stepPayload.missingDocsList?.map((m: any, i: number) => (
                                <tr key={i} className="bg-rose-50/40 hover:bg-rose-50/80">
                                  ...
                                  <td className="p-3">
                                    <span className="font-extrabold text-zinc-800 block">{m.desc}</span>
                                    <span className="text-[10px] text-rose-600 block">⚠️ فاتورة إيصال مبسط مفقودة تماماً من النظام الدفتري</span>
                                  </td>
                                  <td className="p-3 text-left font-mono text-rose-600 font-extrabold">{m.amount.toLocaleString()} SAR</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                    ...
                    {activeTab === 'audit' && currentStepIndex === 4 && (
                      ...
                      <div className="border-4 border-zinc-900/10 p-6 rounded-2xl bg-white space-y-4 shadow-xl text-zinc-900 leading-relaxed text-xs relative overflow-hidden">
                        <div className="absolute top-2 left-2 rotate-12 bg-indigo-50 text-indigo-700 px-3 py-1 rounded border-2 border-indigo-200 uppercase font-black text-[9px] tracking-widest opacity-80 pointer-events-none">Draft Approved</div>
                        
                        <div className="flex justify-between items-start border-b border-zinc-200 pb-3">
                          <div>
                            <span className="bg-indigo-600 text-white font-bold text-[8px] px-2 py-0.5 rounded uppercase tracking-wider block mb-1 w-max">SOCPA Audit Standard</span>
                            <h3 className="text-sm font-black">{stepPayload.package?.clientName}</h3>
                            <p className="text-[10px] text-zinc-500">تقرير المراجعة الضريبة الخارجي لفترة: {stepPayload.package?.auditPeriod}</p>
                          </div>
                          <Building2 className="w-10 h-10 text-zinc-800" />
                        </div>

                        <div>
                          <h4 className="text-xs font-black text-zinc-900 mb-1">خلاصة رأي المحاسب المعتمد (Executive Opinion):</h4>
                          ...
                          <div className="mt-2 text-xs">
                            <span className="font-extrabold">رأي مدقق المدونة المحاسبية للشركة: </span>
                            <span className="text-amber-700 font-extrabold underline">{stepPayload.package?.auditOpinion}</span>
                          </div>
                        </div>

                        <div className="grid grid-cols-2 gap-2 text-center font-bold">
                          <div className="bg-zinc-50 border p-2 rounded-xl">
                            <span className="text-[10px] text-zinc-400">إجمالي الإيرادات في الأستاذ</span>
                            <span className="block text-sm font-mono text-zinc-900 font-black">{stepPayload.package?.financialMetrics?.revenue.toLocaleString()} SAR</span>
                          </div>
                          <div className="bg-zinc-50 border p-2 rounded-xl">
                            <span className="text-[10px] text-zinc-400">هامش الربح المحقق (Margins)</span>
                            <span className="block text-sm font-mono text-indigo-700 font-black">{stepPayload.package?.financialMetrics?.profitMargin}</span>
                          </div>
                        </div>

                        <div className="space-y-1.5 pt-2 border-t border-zinc-100">
                          <h5 className="font-black text-rose-600 text-[10px] uppercase">توصيات مراجع التدقيق الموصى بها مسبقاً (SOCPA Rules):</h5>
                          {stepPayload.package?.recommends?.map((rec: string, i: number) => (
                            <div key={i} className="flex gap-1.5 items-start font-semibold text-zinc-600">
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                    ...
                    <div>
                      <h3 className="font-black text-zinc-900">بانتظار تشغيل خطوة أتمتة مسار العمل</h3>
                      <p className="text-xs text-zinc-500 font-medium max-w-xs mt-1.5">
                        قم بالنقر على <strong className="text-indigo-600">تنفيذ واختبار الخطوة</strong> بالأعلى لمحاكاة جلب ومطابقة الفواتير، وفحص قواعد الامتثال مع الهيئة إلكترونياً.
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className="mt-8 border-t border-zinc-100 pt-4">
              <h3 className="text-[10px] font-black uppercase text-zinc-400 mb-2 flex items-center gap-1">
                <Database className="w-3.5 h-3.5" /> مسجل أحداث محرك الأتمتة والأحداث النشطة (Live Engine Logs)
              </h3>
              <div className="bg-zinc-950 text-zinc-400 p-4 rounded-2xl font-mono text-[10px] max-h-32 overflow-y-auto space-y-1 text-right" dir="rtl">
                {logs.map((log, i) => (
        ...
        <div className="xl:col-span-3 space-y-4">
          <div className="bg-zinc-50 border border-zinc-200 rounded-3xl p-5 shadow-sm space-y-4 h-[710px] flex flex-col justify-between">
            <div>
              ...
              <div className="border-b border-zinc-200/60 pb-3 flex justify-between items-center bg-zinc-50">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-sm font-black text-zinc-900 leading-none">مستشار الزكاة والضريبة والامتثال</h3>
                    <span className="text-[9px] text-zinc-400 font-bold block mt-1">SOCPA/ZATCA Advisor AI</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-3 overflow-y-auto h-[410px] scrollbar-thin scrollbar-thumb-zinc-200 p-1 space-y-3">
                ...
                {isAiLoading && (
                  <div className="bg-white border border-zinc-200 text-zinc-700 p-3 rounded-2xl text-xs text-right max-w-[90%] ml-auto shadow-sm flex items-center gap-2">
                    ...
                    <p className="text-[10px] text-zinc-450 font-bold">جاري مراجعة قواعد الضرائب والاعتمادات وحساب العميل الذكي...</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2 border-t border-zinc-200/60 pt-3">
              <span className="text-[9px] font-black uppercase text-zinc-400 block tracking-wider">استفسارات الفحص السريعة المحاسبية:</span>
              
              <div className="grid grid-cols-1 gap-1.5">
                {[
                  { text: "لماذا ارتفعت ضريبة القيمة المضافة هذا الشهر؟", icon: FileText, query: "Why did VAT increase this month?" },
                  { text: "أي الفواتير المحلية غير مطابقة لـ ZATCA؟", icon: AlertTriangle, query: "Which invoices are non-compliant?" },
                  { text: "أصدار مسودة تقرير التدقيق والامتثال الإداري", icon: Zap, query: "Prepare management report" },
                  { text: "ابحث عن المدفوعات المستندية البنكية المفقودة", icon: Search, query: "Find missing supplier invoices" }
                ].map((preset, i) => (
                  <button 
                    key={i}
              ...
              <div className="relative mt-2">
                <input 
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && aiQuery.trim() && handleAskAdvisor(aiQuery)}
