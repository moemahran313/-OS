...
export default function CRM() {
  ...
  const updateSort = (columnId: string, primaryField: SortField, primaryOrder: SortOrder, secondaryField: SortField, secondaryOrder: SortOrder) => {
    setColumnSort(prev => ({
      ...prev,
  ...
  const getSortedClients = (columnId: string) => {
    const colClients = clients.filter((l) => {
      ...
      const dateMatch = !filters.expectedCloseDate || (l.expectedCloseDate && String(l.expectedCloseDate).startsWith(filters.expectedCloseDate));

      if (!searchMatch) return false;

      const hasAdvancedFilters = filters.industry || filters.companySize || filters.expectedCloseDate;
    ...
    return [...colClients].sort((a, b) => {
      ...
      if (sort.primaryField === "expectedCloseDate") {
        pA = pA || "9999-12-31"; pB = pB || "9999-12-31";
      }
      ...
      if (sort.secondaryField === "expectedCloseDate") {
        sA = sA || "9999-12-31"; sB = sB || "9999-12-31";
      }
    ...
    try {
      ...
      const newClients = rows.slice(1).map(row => {
        ...
        return {
          ...
          status: ['new', 'contacted', 'won', 'lost', 'contracted'].includes(data.status?.toLowerCase()) ? data.status.toLowerCase() : 'new',
      ...
      alert("حدث خطأ أثناء استيراد الملف. يرجى التأكد من أن الهيكل صحيح (name, email, phone, company, status, value)");
    } finally {
      ...
      <header className="flex justify-between items-end">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            />
            <button
              className="flex items-center gap-2 bg-emerald-50 text-emerald-600 px-6 py-3.5 rounded-2xl font-bold hover:bg-emerald-100 transition-all text-sm border border-emerald-100 disabled:opacity-50"
            >
        ...
        <div className="flex gap-2 items-center flex-wrap">
          ...
          <div className="flex items-center gap-1 bg-zinc-100 p-1 rounded-lg">
            <button
              ...
              className={cn("px-3 py-1.5 text-xs font-bold rounded-md transition-all", filterLogic === "AND" ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-zinc-700")}
            >
              AND
            </button>
            <button
              onClick={() => setFilterLogic("OR")}
              className={cn("px-3 py-1.5 text-xs font-bold rounded-md transition-all", filterLogic === "OR" ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-zinc-700")}
            >
              OR
            </button>
          </div>
          
          {(filters.search || filters.industry || filters.companySize || filters.expectedCloseDate) && (
            <button 
              onClick={() => { setFilters({ industry: '', companySize: '', expectedCloseDate: '', search: '' }); setFilterLogic("AND"); }}
                ...
                <div className="flex items-center gap-1 relative">
                  <button
                    ...
                    className={cn(
                      ...
                      columnSort[column.id] && (columnSort[column.id].primaryField !== 'none' || columnSort[column.id].secondaryField !== 'none')
                      ...
                      >
                        ...
                        <div className="space-y-3">
                          <div className="space-y-1">
                            <label className="text-[10px] font-bold text-zinc-500">الفرز الأساسي</label>
                            <div className="flex gap-2">
                              <select 
                                value={columnSort[column.id]?.primaryField || 'none'} 
                                onChange={(e) => updateSort(column.id, e.target.value as any, columnSort[column.id]?.primaryOrder || 'desc', columnSort[column.id]?.secondaryField || 'none', columnSort[column.id]?.secondaryOrder || 'desc')}
                              ...
                              <select 
                                ...
                                onChange={(e) => updateSort(column.id, columnSort[column.id]?.primaryField || 'none', e.target.value as any, columnSort[column.id]?.secondaryField || 'none', columnSort[column.id]?.secondaryOrder || 'desc')}
                          ...
                          <div className="space-y-1">
                            <label className="text-[10px] font-bold text-zinc-500">الفرز الثانوي</label>
                            <div className="flex gap-2">
                              <select 
                                value={columnSort[column.id]?.secondaryField || 'none'} 
                                onChange={(e) => updateSort(column.id, columnSort[column.id]?.primaryField || 'none', columnSort[column.id]?.primaryOrder || 'desc', e.target.value as any, columnSort[column.id]?.secondaryOrder || 'desc')}
                                className="flex-1 bg-zinc-50 border border-zinc-200 rounded-md text-xs px-2 py-1"
                              >
                                <option value="none">بدون</option>
                                <option value="value">القيمة</option>
                              ...
                              <select 
                                ...
                                onChange={(e) => updateSort(column.id, columnSort[column.id]?.primaryField || 'none', columnSort[column.id]?.primaryOrder || 'desc', columnSort[column.id]?.secondaryField || 'none', e.target.value as any)}
                                className="w-20 bg-zinc-50 border border-zinc-200 rounded-md text-xs px-2 py-1"
                              >
                                <option value="desc">تنازلي</option>
                                <option value="asc">تصاعدي</option>
                        ...
                        <div className="mt-4 flex justify-end">
                          <button onClick={() => setMultiSortMenu(null)} className="text-xs font-bold text-primary px-3 py-1 bg-primary/10 rounded-md">إغلاق</button>
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-900/60 backdrop-blur-sm">
            ...
            >
              ...
              <div className="w-full md:w-1/3 bg-zinc-900 p-8 text-white space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-xs text-zinc-400 leading-relaxed font-medium">
                    سيتم تطبيق هذه التفضيلات تلقائياً على جميع فواتير هذا العميل
                    في مدارج OS.
                  </p>
                </div>
              ...
              <div className="flex-1 bg-white p-10 overflow-y-auto no-scrollbar relative flex flex-col">
                ...
                {activeTab === "details" ? (
                  <form onSubmit={handleSave} className="space-y-8 flex-1 overflow-y-auto pr-6 scrollbar-hide">
                    {!editingClient.id && (
                      <div className="space-y-1">
                        ...
                        <div className="relative">
                          ...
                          <div className="absolute left-4 top-1/2 -translate-y-1/2 p-1.5 bg-white rounded-lg shadow-sm text-zinc-400">
                            {(() => {
                              ...
                              if (t.includes("hr") || t.includes("موارد")) return <GraduationCap className="w-4 h-4 text-purple-500" />;
                              if (t.includes("ceo") || t.includes("رئيس") || t.includes("مدير عام")) return <Briefcase className="w-4 h-4 text-blue-500" />;
                              if (t.includes("tech") || t.includes("تقني") || t.includes("cto") || t.includes("it")) return <Globe className="w-4 h-4 text-green-500" />;
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2 mb-2">
                          {["التقنية", "الرعاية الصحية", "التجارة", "المقاولات", "التعليم", "المالية"].map((ind) => (
                            <button
                              key={ind}
                              type="button"
                              onClick={() => setEditingClient({ ...editingClient, industry: ind })}
                              className={cn(
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                          {[
                            ...
                            <button
                              key={size.id}
                              type="button"
                              onClick={() => setEditingClient({ ...editingClient, companySize: size.id })}
                              className={cn(
                  ...
                  <div className="flex-1 flex flex-col pt-4 space-y-6 overflow-y-auto scrollbar-hide">
                    ...
                    {shipments.filter(s => s.clientId === editingClient.id).length > 0 ? (
                      <div className="grid gap-4">
                        {shipments.filter(s => s.clientId === (editingClient as any).id).map(s => (
                          <div 
                            key={s.id} 
                          ...
                          >
                            ...
                            <span className={cn(
                              ...
                              s.status === 'cleared' ? 'bg-emerald-50 text-emerald-600' : 'bg-primary/10 text-primary'
                            )}>
                              {s.status}
                            </span>
                          </div>
                      ...
                      <div className="text-center py-20 bg-zinc-50 rounded-[2.5rem] border border-zinc-100 border-dashed">
                         ...
                         <p className="text-[10px] text-zinc-400 font-medium mt-1 uppercase tracking-tight">يرجى الربط من صفحة الموردين</p>
                         <button 
                           onClick={() => navigate('/app/suppliers')}
                    ...
                    editingClient.history.length === 0 ? (
                      <div className="text-center py-20 text-zinc-400">
                        ...
                        <p className="font-bold">
                          لا يوجد سجل تاريخي لهذا العميل حتى الآن.
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-6 relative before:absolute before:inset-y-0 before:right-[15px] before:w-[2px] before:bg-zinc-100 p-2">
                        {editingClient.history.map((item, idx) => (
                          <div
                            key={item.id}
                            className="relative rtl:pl-0 rtl:pr-10 text-right"
                          >
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-zinc-900/60 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-sm font-medium text-zinc-500 mb-6">
                قمت بنقل <strong className="text-zinc-900">{statusChangePrompt.client.name}</strong> متى ما أردت يمكنك إضافة ملاحظة سريعة توضح سبب أو تفاصيل هذه الخطوة.
              </p>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2">
        ...
        {(client.contractEndDate || client.projectCode || client.complianceRisk || client.conversionProbability) && (
          <div className="flex flex-wrap gap-2 mt-1 border-t border-zinc-50 pt-1.5">
            ...
            {client.complianceRisk && (
              <span className={cn("text-[9px] font-bold px-1.5 py-0.5 rounded flex items-center gap-1", 
                client.complianceRisk === 'low' ? 'bg-emerald-50 text-emerald-600' :
                client.complianceRisk === 'medium' ? 'bg-amber-50 text-amber-600' :
                client.complianceRisk === 'high' ? 'bg-rose-50 text-rose-600' :
                'bg-red-100 text-red-700'
              )}>
                <ShieldCheck className="w-2.5 h-2.5" />
                مخاطر: {client.complianceRisk === 'low' ? 'منخفضة' : client.complianceRisk === 'medium' ? 'متوسطة' : client.complianceRisk === 'high' ? 'مرتفعة' : 'حرجة'}
              </span>
            )}
          </div>
        )}
        ...
        <div className="relative" onClick={(e) => e.stopPropagation()}>
          ...
          <AnimatePresence>
            {showOptions && (
              ...
              >
                <button
                  onClick={(e) => { setShowOptions(false); onClick(e); }}
