...
export default function ProductContracts() {
  ...
  useEffect(() => {
    ...
    metaDesc.setAttribute('content', 'صياغة وتوليد وتوقيع العقود إلكترونياً بختم مرجعي موثق ومتوافق مع المحاكم السعودية. مكتبة من قوالب العقود المجهزة من مستشارين قانونيين.');
  }, []);
      ...
      <section className="relative pt-44 pb-24 md:pt-56 md:pb-36 overflow-hidden">
        ...
        <div className="container mx-auto px-6 max-w-7xl relative z-10 text-center">
          ...
          >
            ...
            <span className="text-sm font-semibold text-purple-200">العقود والاتفاقيات الذكية والتوقيع الإلكتروني</span>
          </motion.div>
          ...
          >
            مكتبة متكاملة من قوالب العقود المعدة من قبل مستشارين قانونيين، تعبئة تلقائية لمتغيرات العقود، إرسال فوري لروابط التوقيع الرقمي الآمن، وأرشيف سحابي مشفر يغنيك عن المستندات الورقية وتكاليف التوثيق التقليدي.
          </motion.p>
          ...
          >
            <Link to="/app/contracts" className="px-8 py-4 bg-primary text-white font-black rounded-2xl text-lg hover:scale-105 transition-all shadow-lg shadow-primary/20">
      ...
      >
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-black mb-4">مولّد العقود والاتفاقيات التفاعلي</h2>
            <p className="text-zinc-400 text-lg max-w-2xl mx-auto">اختر نوع العقد وقم بتعديل المتغيرات لتحديث صياغته القانونية فورياً.</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch">
            ...
            <div className="lg:col-span-5 bg-zinc-950/85 border border-white/10 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden backdrop-blur-3xl flex flex-col justify-between">
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-xs font-black text-zinc-400 mb-3 uppercase tracking-wider">اختر قالب العقد المعتمد</label>
                ...
                <div className="border-t border-white/5 pt-6 space-y-4">
                  <span className="text-xs font-black text-zinc-400 uppercase tracking-wider block">متغيرات وبنود العقد الذكية</span>
                  
                  {selectedTemplate === "employment" && (
                    <div className="grid grid-cols-1 gap-4">
                      <div>
                        <label className="block text-[10px] text-zinc-500 font-bold mb-1">الطرف الأول (المنشأة)</label>
                        <input 
                          type="text" 
                          value={contractVariables.firstParty} 
                          onChange={(e) => setContractVariables({...contractVariables, firstParty: e.target.value})}
                      ...
                      <div>
                        <label className="block text-[10px] text-zinc-500 font-bold mb-1">الطرف الثاني (الموظف)</label>
                        <input 
                          type="text" 
                          value={contractVariables.secondParty} 
                          onChange={(e) => setContractVariables({...contractVariables, secondParty: e.target.value})}
                          className="w-full px-4 py-2 text-xs bg-zinc-900 border border-white/5 rounded-xl text-white font-bold focus:border-purple-500 outline-none"
                        />
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] text-zinc-500 font-bold mb-1">الراتب الأساسي (ر.س)</label>
                          <input 
                            type="text" 
                            value={contractVariables.salary} 
                            onChange={(e) => setContractVariables({...contractVariables, salary: e.target.value})}
                            className="w-full px-4 py-2 text-xs bg-zinc-900 border border-white/5 rounded-xl text-white font-bold focus:border-purple-500 outline-none"
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] text-zinc-500 font-bold mb-1">مدة العقد</label>
                          <input 
                            type="text" 
                            value={contractVariables.duration} 
                            onChange={(e) => setContractVariables({...contractVariables, duration: e.target.value})}
                            className="w-full px-4 py-2 text-xs bg-zinc-900 border border-white/5 rounded-xl text-white font-bold focus:border-purple-500 outline-none"
                          />
                        </div>
                      </div>
                  ...
                  {selectedTemplate !== "employment" && (
                    <p className="text-zinc-500 text-xs font-bold leading-relaxed py-4 text-center">
                      القوالب الإضافية تتيح ربطاً ذكياً بمتغيرات المشاريع وعقود الفسح الجمركي تلقائياً.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="lg:col-span-7 bg-zinc-900 border border-white/10 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden backdrop-blur-3xl flex flex-col justify-between">
              <div className="p-6 bg-zinc-950 rounded-2xl border border-white/5 min-h-[380px] font-sans text-xs leading-relaxed text-zinc-400 space-y-4 shadow-inner overflow-y-auto max-h-[450px]">
                <div className="text-center font-black text-sm text-white mb-6">
                  {selectedTemplate === "employment" ? "عقد عمل موحد ومطابق للأنظمة" : "صياغة قانونية متوافقة مع القضاء السعودي"}
                </div>
                ...
                {selectedTemplate === "employment" ? (
                  <>
                    ...
                    <p>
                      <strong>البند الأول - موضوع العقد:</strong> يلتزم الطرف الثاني بموجب هذا العقد بالعمل تحت إدارة وإشراف الطرف الأول بمهنة تتطابق مع لوائح المنظمة وتوجيهات المدير التنفيذي وتحدد مدة هذا العقد بـ <span className="text-purple-400 font-bold">{contractVariables.duration}</span> تبدأ من تاريخ توقيع الطرفين.
                    </p>
                    <p>
                      <strong>البند الثاني - الأجر والبدلات:</strong> يلتزم الطرف الأول بصرف أجر شهري إجمالي وقدره <span className="text-emerald-400 font-bold">{contractVariables.salary} ر.س</span> يلتزم بتحويله بنهاية كل شهر ميلادي وفقاً لأنظمة برنامج حماية الأجور المعتمد في المملكة العربية السعودية.
                    </p>
                  </>
                ) : (
                  <p className="text-center text-zinc-500 py-12 font-bold">
                    جاري تحميل بنود ومواد {templates.find(t => t.id === selectedTemplate)?.name} المتكاملة...
                  </p>
                )}
              </div>
      ...
      >
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="text-center mb-20">
            <h2 className="text-3xl md:text-5xl font-black mb-4">التوقيع القانوني الأكثر أماناً وسهولة</h2>
            <p className="text-zinc-400 text-lg max-w-2xl mx-auto">صياغة وتوقيع وأرشفة العقود من شاشة واحدة تحافظ على سير الأعمال.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="p-8 rounded-[2rem] border border-white/5 bg-zinc-900/40 relative overflow-hidden group hover:border-white/10 transition-colors">
              ...
              <p className="text-zinc-400 text-sm font-medium leading-relaxed">
                يصدر النظام مع كل عقد موقّع إلكترونياً شهادة إثبات تتضمن الأرقام المرجعية، العناوين الرقمية (IP)، تأكيد النية، والتوثيق عبر رمز التحقق الذي يصل لأجهزة الموقعين لضمان حجية التوقيع.
              </p>
            </div>
            ...
            <div className="p-8 rounded-[2rem] border border-white/5 bg-zinc-900/40 relative overflow-hidden group hover:border-white/10 transition-colors">
              ...
              <p className="text-zinc-400 text-sm font-medium leading-relaxed">
                يمكن للطرف الآخر توقيع الاتفاقية أو العقد عبر الجوال أو الجهاز اللوحي مباشرة برسم التوقيع بخط اليد أو التوقيع بالاسم المطبوع دون الحاجة لطابعة أو ماسح ضوئي.
              </p>
            </div>
            ...
            <div className="p-8 rounded-[2rem] border border-white/5 bg-zinc-900/40 relative overflow-hidden group hover:border-white/10 transition-colors">
              ...
              <h3 className="text-xl font-black text-white mb-3">التحكم في تواريخ الانتهاء والتجديد</h3>
              <p className="text-zinc-400 text-sm font-medium leading-relaxed">
                نظام تنبيهات ذكي ينبه الأطراف المعنية تلقائياً قبل حلول تاريخ انتهاء العقد أو الحاجة لتجديده بـ 30 أو 60 يوماً لضمان قانونية استمرار الأعمال وعدم إيقاف الخدمات.
              </p>
            </div>
          </div>
        </div>
      ...
      >
        <div className="container mx-auto px-6 max-w-5xl text-center relative z-10">
          ...
          <p className="text-zinc-400 text-lg md:text-xl font-medium max-w-2xl mx-auto mb-10 leading-relaxed">
            التقِ بالجيل القادم من التواقيع الإلكترونية والعقود الذكية المعززة بحلول تشغيل مدارج OS المتطورة.
          </p>
