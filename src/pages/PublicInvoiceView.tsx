...
export default function PublicInvoiceView() {
  ...
  const [checkoutStep, setCheckoutStep] = useState<"select" | "details" | "3ds" | "success">(
    "select"
  );
  ...
  const handlePayTabsSubmit = (e: React.FormEvent) => {
    ...
    if (!cardNumber || !cardName || !cardExpiry || !cardCvv) {
      alert(
        invoice?.branding?.language === "en" ? "Please fill all fields" : "يرجى ملء كافة الحقول"
      );
      return;
    }
    setPaying(true);
  ...
  const handleVerifyOTP = () => {
    if (otpInput !== "1234") {
      setOtpError(
        invoice?.branding?.language === "en"
          ? "Invalid OTP. Use 1234 for testing."
          : "رمز تحقق خاطئ. استخدم 1234 للتجربة."
      );
      return;
    }
    setOtpError("");
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center font-sans tracking-tight">
        جاري التحميل...
      </div>
    );
  if (!invoice)
      ...
      <nav className="h-16 bg-white border-b border-zinc-100 flex items-center px-8 justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-black text-zinc-900 tracking-tight">
            بوابة الدفع الآمنة مدارج Secure Pay
          </span>
        </div>
      ...
      <main className="max-w-5xl mx-auto py-12 px-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div
            ...
            className="bg-white rounded-3xl shadow-xl shadow-zinc-200/50 border border-zinc-100 relative overflow-hidden flex justify-center"
          >
            <InvoicePrintTemplate invoice={invoice} />
          </div>
        </div>
        ...
        <div className="space-y-6">
              ...
              <div className="text-center p-6 bg-emerald-50 rounded-2xl border border-emerald-100 space-y-4 animate-in fade-in zoom-in duration-500">
                ...
                <div>
                  <h4 className="font-bold text-emerald-900">
                    {isEnglish ? "Payment Successful" : "تم السداد بنجاح"}
                  </h4>
                  <p className="text-xs text-emerald-600 mt-1">
                    {isEnglish
                      ? "Payment of the invoice balance has been confirmed."
                      : "تم استلام دفعتك وتأكيدها وتحديث حالة الفاتورة تلقائياً."}
                  </p>
                </div>
                <button
                  onClick={handleDownload}
                ...
                >
                  ...
                  {isEnglish ? "Download Receipt" : "تحميل إيصال الدفع"}
                </button>
              </div>
            ) : (
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    />
                    <span className="absolute left-4 inset-y-0 flex items-center text-xs font-black text-zinc-900">
                      {invoice.currency}
                    </span>
                  </div>
                </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-right">
                            <p className="text-xs font-bold text-zinc-900">
                              BenefitPay (مملكة البحرين)
                            </p>
                            <p className="text-[10px] text-zinc-500">
                              مسح سريع للرمز واستجابة فورية
                            </p>
                          </div>
                        </div>
                        <ChevronRight className="w-4 h-4 text-zinc-400 group-hover:translate-x-[-4px] transition-transform" />
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-[10px] text-zinc-500">
                              بصمة الإصبع أو الوجه بلمسة واحدة
                            </p>
                          </div>
                        </div>
                        <ChevronRight className="w-4 h-4 text-zinc-400 group-hover:translate-x-[-4px] transition-transform" />
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-[10px] text-zinc-500">
                              نظام الحوالات والمدفوعات السعودي
                            </p>
                          </div>
                        </div>
                        <ChevronRight className="w-4 h-4 text-zinc-400 group-hover:translate-x-[-4px] transition-transform" />
                                  ...
                                  purchase_units: [
                                    {
                                      amount: {
                                        currency_code:
                                          invoice.currency === "SAR" ? "USD" : invoice.currency,
                                        value: (invoice.currency === "SAR"
                                          ? Number(paymentAmount) / 3.75
                ...
                {checkoutStep === "details" && (
                  <div className="space-y-4 animate-in fade-in duration-300">
                    ...
                    >
                      ...
                      {isEnglish ? "Back to gateways" : "العودة لقائمة البوابات"}
                    </button>
                    ...
                    {selectedGateway === "paytabs" && (
                      <form onSubmit={handlePayTabsSubmit} className="space-y-4">
                        <div className="p-4 bg-zinc-50 border border-zinc-200 rounded-2xl flex items-center gap-3">
                          <div className="w-8 h-8 bg-[#E04F2F] text-white rounded-lg flex items-center justify-center font-bold text-xs shrink-0">
                            PT
                          </div>
                          <div className="text-right">
                            <p className="text-xs font-bold text-zinc-900">
                              بوابة دفع PayTabs الآمنة
                            </p>
                            <p className="text-[10px] text-zinc-500">
                              معالجة فورية تدعم بطاقات مدى السعودية
                            </p>
                          </div>
                        </div>
                        ...
                        >
                          ...
                          {paying
                            ? "جاري الاتصال بـ PayTabs..."
                            : `دفع ${Number(paymentAmount).toLocaleString()} ${invoice.currency}`}
                        </button>
                      </form>
                    )}
                    ...
                    {selectedGateway === "benefitpay" && (
                      <div className="space-y-4 text-center">
                        <div className="p-4 bg-[#005c8a]/10 border border-[#005c8a]/20 rounded-2xl flex items-center gap-3 text-right">
                          <div className="w-10 h-10 bg-[#005c8a] text-[#ffbf1f] rounded-xl flex items-center justify-center font-bold text-sm shrink-0">
                            BP
                          </div>
                          <div>
                            <p className="text-xs font-bold text-zinc-900">
                              محاكاة بنفت باي BenefitPay
                            </p>
                            <p className="text-[10px] text-zinc-500">
                              نظام الدفع الوطني - مملكة البحرين
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="bg-white border border-zinc-100 p-6 rounded-2xl space-y-4 shadow-sm">
                          <p className="text-xs text-zinc-500 font-bold leading-relaxed">
                            امسح رمز الاستجابة السريع (QR Code) أدناه عبر تطبيق BenefitPay في هاتفك
                            النقال لإتمام عملية الدفع مباشرة.
                          </p>
                        ...
                        <div className="bg-zinc-50 border border-zinc-200 rounded-2xl p-4 space-y-3 shadow-inner">
                          <div className="flex justify-between text-xs font-bold border-b border-zinc-200/50 pb-2">
                            ...
                            <span className="text-zinc-900">
                              {invoice.zatcaConfig?.sellerName || "المتجر الشريك / Merchant"}
                            </span>
                          </div>
                          <div className="flex justify-between text-xs font-bold border-b border-zinc-200/50 pb-2">
                          ...
                          <div className="flex justify-between text-xs font-black">
                            ...
                            <span className="text-emerald-600 font-mono text-sm">
                              {Number(paymentAmount).toLocaleString()} {invoice.currency}
                            </span>
                          </div>
                        </div>
                    ...
                    {selectedGateway === "sadad" && (
                      <form onSubmit={handleSadadSubmit} className="space-y-4">
                        <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl flex items-center gap-3">
                          <div className="w-10 h-10 bg-emerald-500 text-white font-bold text-xs rounded-xl flex items-center justify-center">
                            سداد
                          </div>
                          <div className="text-right">
                            <p className="text-xs font-bold text-emerald-900">
                              حساب سداد الإلكتروني (SADAD)
                            </p>
                            <p className="text-[10px] text-emerald-600">
                              اقتطاع مباشر وآمن من حسابك البنكي
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-3">
                          <div>
                            <label className="text-[10px] font-bold text-zinc-500 uppercase block mb-1">
                              اسم المستخدم لحساب سداد / Sadad Username
                            </label>
                            <input
                              type="text"
                              required
                      ...
                      <div className="p-6 space-y-4 text-center">
                        ...
                        <div className="space-y-1">
                          <h4 className="font-bold text-sm text-zinc-900">
                            التحقق الثنائي للعملية
                          </h4>
                          <p className="text-[10px] text-zinc-500 leading-relaxed">
                            لقد أرسلنا رمز التحقق المكون من 4 أرقام عبر رسالة نصية قصيرة (SMS) إلى
                            رقم جوالك المسجل لدى البنك.
                          </p>
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-[10px] text-zinc-400 font-black">
                            تلميح للتجربة: أدخل الرمز{" "}
                            <span className="text-zinc-900 font-mono bg-zinc-100 px-1 py-0.5 rounded">
                              1234
                            </span>
                          </p>
                        </div>
                ...
                <div className="pt-6 border-t border-zinc-50 space-y-4">
                  <div className="flex flex-col gap-2 bg-zinc-50 p-4 rounded-2xl border border-zinc-100">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-bold text-emerald-600">
                        {(invoice.paidAmountHalalas / 100).toLocaleString() || 0} {invoice.currency}
                      </span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span className="text-zinc-500 font-bold">المتبقي / Remaining</span>
                      <span className="font-bold text-zinc-900">
                        {(invoice.remainingBalanceHalalas / 100).toLocaleString() || 0}{" "}
                        {invoice.currency}
                      </span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h5 className="text-[11px] font-bold text-zinc-900 leading-none mb-1">
                        دفع مشفر وآمن
                      </h5>
                      <p className="text-[10px] text-zinc-400 font-medium">
                        تتم معالجة بياناتك بأمان تام عبر معايير التشفير والامتثال PCI-DSS.
                      </p>
                    </div>
                  </div>
          ...
          <div className="p-6 bg-white rounded-2xl border border-zinc-100 flex justify-between items-center">
            ...
            <CheckCircle2
              className={cn(
                "w-6 h-6",
                invoice.status === "paid" ? "text-emerald-500" : "text-zinc-200"
              )}
            />
          </div>
        </div>
