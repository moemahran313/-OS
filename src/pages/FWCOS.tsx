...
const GCC_COUNTRIES = [{ code: "KSA", name: "السعودية" }];

const INITIAL_RULES = [
  {
    id: 1,
    type: "iqama",
    title: "انتهاء الإقامة/الهوية",
    description: "تنبيه عند اقتراب انتهاء بطاقة الهوية أو الإقامة",
    impact: "High",
    thresholdDays: 60,
    countries: GCC_COUNTRIES.map((c) => ({ ...c, active: true, thresholdDays: 60 })),
  },
  {
    id: 2,
    type: "wps",
    title: "حماية الأجور (WPS)",
    description: "تأخر الرواتب يرفع نسبة الخطورة بشكل تلقائي",
    impact: "Critical",
    thresholdDays: 15,
    countries: GCC_COUNTRIES.map((c) => ({ ...c, active: true, thresholdDays: 15 })),
  },
  {
    id: 3,
    type: "visa",
    title: "صلاحية التأشيرات والجوازات",
    description: "يجب تجديد التأشيرات والجوازات المرفقة قبل انتهاء صلاحيتها",
    impact: "Medium",
    thresholdDays: 45,
    countries: GCC_COUNTRIES.map((c) => ({ ...c, active: true, thresholdDays: 45 })),
  },
  {
    id: 4,
    type: "contract",
    title: "انتهاء العقود",
    description: "تنبيه عند اقتراب نهاية العقد لاتخاذ قرار التجديد أو التصفية",
    impact: "Medium",
    thresholdDays: 30,
    countries: GCC_COUNTRIES.map((c) => ({ ...c, active: true, thresholdDays: 30 })),
  },
];
...
const RiskGauge = ({ score, size = 48 }: { score: number; size?: number }) => {
  ...
  const colorClass =
    score > 66 ? "text-rose-500" : score > 33 ? "text-amber-500" : "text-emerald-500";

  return (
    <div
      className="relative flex flex-col items-center justify-end font-black flex-shrink-0"
...
export default function FWCOS() {
  ...
  const [activeTab, setActiveTab] = useState<
    "dashboard" | "workers" | "tasks" | "rules" | "settings" | "zatca" | "gosi"
  >("dashboard");
  ...
  const handleSaveWorker = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        userId: uid,
        documents:
          typeof editingWorker.documents === "string"
            ? editingWorker.documents
            : JSON.stringify(editingWorker.documents || []),
      };
  ...
  const computedWorkers: ComputedWorker[] = useMemo(() => {
    ...
    return workers.map((w) => {
      ...
      if (isWpsActive) {
        if (w.wpsStatus === "violation") {
          riskScore += 50;
          isCritical = true;
          alerts.push({
            ruleType: "wps",
            ruleTitle: "حماية الأجور",
            country: w.country,
            text: "مخالفة WPS",
          });
        } else if (w.wpsStatus === "delayed") {
          riskScore += 20;
          alerts.push({
            ruleType: "wps",
            ruleTitle: "حماية الأجور",
      ...
      if (isIqamaActive) {
        ...
        if (iqamaDays < 0) {
          ...
          alerts.push({
            ruleType: "iqama",
            ruleTitle: "الإقامة/الهوية",
            country: w.country,
            text: "الوحدة منتهية",
            daysRemaining: Math.floor(iqamaDays),
          });
        } else if (
          iqamaDays <= (iqamaCountryRule?.thresholdDays || iqamaRule?.thresholdDays || 60)
        ) {
          riskScore += 20;
          isExpiring = true;
          alerts.push({
      ...
      if (isContractActive && w.contractEndDate) {
        ...
        if (contractDays < 0) {
          ...
          alerts.push({
            ...
            country: w.country,
            text: "عقد منتهي",
            daysRemaining: Math.floor(contractDays),
          });
        } else if (
          contractDays <= (contractCountryRule?.thresholdDays || contractRule?.thresholdDays || 30)
        ) {
          riskScore += 10;
          isExpiring = true;
          alerts.push({
        ...
        w.documents?.forEach((doc) => {
          if (doc.expiryDate) {
            ...
            if (docDays < 0) {
              ...
              alerts.push({
                ruleType: "visa",
                ruleTitle: `مستند (${doc.type})`,
                country: w.country,
                text: "مستند منتهي",
                daysRemaining: Math.floor(docDays),
              });
            } else if (
              docDays <= (visaCountryRule?.thresholdDays || visaRule?.thresholdDays || 30)
            ) {
              riskScore += 15;
              isExpiring = true;
              alerts.push({
  ...
  const dashboardWorkers = useMemo(() => {
    return selectedCountryObj === "ALL"
      ? computedWorkers
      : computedWorkers.filter((w) => w.country === selectedCountryObj);
  ...
  const filteredWorkersList = useMemo(() => {
    ...
    if (workerFilters.search) {
      list = list.filter(
        (w) =>
          w.name.includes(workerFilters.search) ||
          w.passportNumber?.includes(workerFilters.search) ||
          w.visaNumber?.includes(workerFilters.search)
      );
    }
    ...
    if (workerFilters.expiryStatus !== "ALL") {
      if (workerFilters.expiryStatus === "expired") {
        list = list.filter((w) =>
          w.alerts.some((a) => a.daysRemaining !== undefined && a.daysRemaining < 0)
        );
    ...
    if (workerFilters.contractExpiryStart) {
      ...
      list = list.filter(
        (w) => w.contractEndDate && new Date(w.contractEndDate).getTime() >= start
      );
    }
  ...
  const nationalities = useMemo(
    () => Array.from(new Set(computedWorkers.map((w) => w.nationality))),
    [computedWorkers]
  );
  const roles = useMemo(
    () => Array.from(new Set(computedWorkers.map((w) => w.role))),
    [computedWorkers]
  );
  ...
  const filteredLogs = useMemo(() => {
    ...
    if (auditFilterAction !== "ALL")
      logs = logs.filter((l) => l.action.includes(auditFilterAction));
    return logs;
  ...
  return (
    <div className="space-y-8 max-w-full mx-auto h-[calc(100vh-6rem)] overflow-y-auto pb-10">
      ...
      <header className="flex justify-between items-end">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <span className="bg-rose-100 text-rose-700 font-bold px-3 py-1 rounded-full text-xs flex items-center gap-1.5 border border-rose-200">
              <ShieldAlert className="w-3.5 h-3.5" /> FWC-OS (Beta)
            </span>
          ...
          <h1 className="text-3xl font-black text-zinc-900 tracking-tight">
            نظام الامتثال وإدارة الوافدين
          </h1>
          <p className="text-zinc-500 mt-1 font-medium max-w-2xl">
            إدارة دقيقة لوثائق وإقامات العمالة مع سجل تدقيق وتنبيهات مخصصة لدول مجلس التعاون.
          </p>
        </div>
        <button
        ...
        return (
          ...
          >
            ...
            >
              <div
                className={cn(
                  "w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm border",
                  isUrgent ? "text-rose-500 border-rose-100" : "text-amber-500 border-amber-100"
                )}
              >
              ...
              <div>
                ...
                <p className="text-sm font-medium opacity-80">
                  يوجد {workers.length} موظفين تنتهي عقودهم أو إقاماتهم خلال أقل من {period} يوماً.
                  يرجى مراجعة الحالة واتخاذ اللازم.
                </p>
              </div>
            </div>
            <button
      ...
      <div className="flex gap-2 border-b border-zinc-200 pb-2 overflow-x-auto no-scrollbar">
        {(["dashboard", "workers", "tasks", "rules", "zatca", "gosi", "settings"] as const).map(
          (tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={cn(
                "px-5 py-2.5 rounded-[1.2rem] text-sm font-bold transition-all whitespace-nowrap",
                activeTab === tab
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    sum.wpsIssues > 0 || sum.expiringDocs > 0
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div className="bg-white p-5 rounded-[2rem] border border-zinc-200/50 shadow-sm flex flex-col justify-between hover:-translate-y-1 hover:shadow-md transition-all cursor-default">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-[10px] font-bold text-zinc-400 bg-zinc-50 px-2 py-1 rounded-lg">
                  العمالة ({selectedCountryObj})
                </span>
              </div>
              <div>
                <h3 className="text-3xl font-black text-zinc-900">{dashboardWorkers.length}</h3>
              </div>
            </div>
              ...
              <div>
                <h3 className="text-3xl font-black text-emerald-600">
                  {dashboardWorkers.length > 0
                    ? (
                        (dashboardWorkers.filter(
                          (w) =>
                            w.nationality?.includes("سعودي") ||
                            w.nationality?.toLowerCase().includes("saudi")
                        ).length /
            ...
            <div className="bg-white p-5 rounded-[2rem] border border-zinc-200/50 shadow-sm flex flex-col justify-between hover:-translate-y-1 hover:shadow-md transition-all cursor-default">
              ...
              <div>
                <h3 className="text-3xl font-black text-zinc-900">
                  {
                    dashboardWorkers.filter(
                      (w) => w.alerts.length > 0 && !w.alerts.some((a) => a.ruleType === "wps")
                    ).length
                  }
                </h3>
              </div>
            ...
            <div className="bg-white p-5 rounded-[2rem] border border-zinc-200/50 shadow-sm flex flex-col justify-between ring-1 ring-rose-100 hover:-translate-y-1 hover:shadow-md transition-all cursor-default">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-[10px] font-bold text-rose-500 bg-rose-50 px-2 py-1 rounded-lg border border-rose-100">
                  مخالفات الأجور (WPS)
                </span>
              </div>
              <div>
                <h3 className="text-3xl font-black text-rose-600">
                  {dashboardWorkers.filter((w) => w.wpsStatus !== "compliant").length}
                </h3>
              </div>
            </div>
            <div className="bg-zinc-900 text-white p-5 rounded-[2rem] shadow-xl flex flex-col justify-between hover:-translate-y-1 hover:shadow-2xl transition-all cursor-default">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-[10px] font-bold text-zinc-300 bg-white/10 px-2 py-1 rounded-lg">
                  المخاطرة بصفتها الحالية
                </span>
              </div>
              <div>
                <h3 className="text-3xl font-black text-white flex items-baseline gap-1">
                  {Math.floor(
                    dashboardWorkers.reduce((acc, w) => acc + w.riskScore, 0) /
                      (dashboardWorkers.length || 1)
                  )}{" "}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-[2rem] p-6 border border-zinc-200/50 shadow-sm flex flex-col">
              <h3 className="text-lg font-black text-zinc-900 mb-6 flex items-center gap-2">
                <AlertTriangle className="w-5 h-5 text-amber-500" /> تنبيهات متقدمة (
                {selectedCountryObj})
              </h3>
              <div className="space-y-4 flex-1">
                {dashboardWorkers
                  ...
                  .map((worker) => (
                    <div
                      key={worker.id}
                      className="flex items-start justify-between p-4 rounded-2xl bg-zinc-50 border border-zinc-100"
                    >
                        ...
                        <div>
                          ...
                          <div className="flex flex-col gap-1 mt-1">
                            {worker.alerts.map((a, i) => (
                              <div
                                key={i}
                                className="text-[10px] bg-rose-50 border border-rose-100 flex items-center gap-2 px-2 py-1 rounded w-fit"
                              >
                                ...
                                {a.daysRemaining !== undefined && (
                                  <span className="bg-rose-200 text-rose-800 px-1 py-0.5 rounded text-[9px] font-black">
                                    {Math.abs(a.daysRemaining)} يوم
                                  </span>
                                )}
                              </div>
                            ))}
                ...
                {dashboardWorkers.filter((w) => w.status !== "safe").length === 0 && (
                  <p className="text-sm text-zinc-400 font-bold">لا توجد تنبيهات لهذه المنطقة.</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            ...
            <div className="bg-white border border-zinc-200/50 rounded-[2rem] shadow-sm overflow-hidden p-6">
              <h3 className="font-black text-xl text-zinc-900 mb-4 flex items-center gap-2">
                <Activity className="w-5 h-5 text-blue-500" /> مهام وتكليفات PRO
              </h3>
              <div className="divide-y divide-zinc-100">
                {generatedTasks.length === 0 && (
                  <p className="text-zinc-500 text-sm">لا توجد مهام نشطة أو تنبيهات.</p>
                )}
                  ...
                  <div key={task.id} className="py-4 flex items-center justify-between">
                    <div>
                      <h4 className="font-bold text-zinc-900 text-sm mb-1">
                        {task.type}{" "}
                        <span className="text-zinc-400 font-medium text-[10px] bg-zinc-100 px-1 rounded mx-1">
                          {task.workerName}
                        </span>
                      </h4>
                      <span
                    ...
                    <div className="flex items-center gap-2">
                      {task.rawStatus !== "in_progress" && task.rawStatus !== "completed" && (
                        <button
                          ...
                          className="px-3 py-1.5 text-[10px] font-bold bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100"
                        >
                          بدأ العمل
                        </button>
                      )}
                      {task.rawStatus !== "completed" && (
                        <button
                          ...
                          className="p-1.5 text-[10px] font-bold bg-emerald-50 text-emerald-700 rounded-lg hover:bg-emerald-100"
                        >
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-[2rem] shadow-xl overflow-hidden p-6 text-zinc-300">
              <div className="flex justify-between items-start mb-4">
                <h3 className="font-black text-xl text-white flex items-center gap-2">
                  <History className="w-5 h-5 text-zinc-400" /> سجل التدقيق
                </h3>
              </div>
              <div className="flex gap-2 mb-4">
                <select
                  ...
                  className="bg-zinc-800 border border-zinc-700 text-zinc-200 text-xs px-2 py-1.5 rounded-lg outline-none flex-1"
                >
              ...
              <div className="space-y-4">
                {filteredLogs.length === 0 ? (
                  <p className="text-sm font-medium text-zinc-600">
                    لم يتم تسجيل أي عمليات بناءاً على التصفية.
                  </p>
                ) : null}
                {filteredLogs.map((log, i) => (
                  <div
                    key={i}
                    className="flex gap-4 p-3 bg-zinc-800/50 rounded-xl border border-zinc-800"
                  >
                    <div
                      className={cn(
                        "w-8 h-8 rounded-full flex items-center justify-center shrink-0",
                        log.action.includes("مكتملة")
                    ...
                    <div>
                      <p className="text-xs text-zinc-100 leading-snug font-medium">
                        <span className="font-bold text-white mb-1 block">{log.user}</span> قام
                        بتحديث مهمة{" "}
                        <span className="text-blue-400">
                          "{log.type} - {log.workerName}"
                        </span>{" "}
                        إلى الحالة{" "}
                        <span
                          className={cn(
      ...
      {activeTab === "workers" && (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-white border border-zinc-200/50 rounded-[2rem] shadow-sm overflow-hidden flex flex-col h-[calc(100vh-14rem)]"
        >
          <div className="p-6 border-b border-zinc-100 flex flex-col gap-4 shrink-0">
            <div className="flex justify-between items-center">
              ...
              <div className="relative hidden md:block">
                ...
                <input
                  ...
                  onChange={(e) => setWorkerFilters({ ...workerFilters, search: e.target.value })}
                  placeholder="بحث في السجل..."
                  className="pr-9 pl-4 py-2 bg-zinc-50 border border-zinc-200 rounded-xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary/20"
                />
              </div>
            </div>
            {/* Advance Filters */}
            <div className="flex flex-col gap-3 w-full">
              <div className="flex flex-wrap gap-2 items-center w-full z-20 relative">
                <div className="w-full md:w-64">
                  <Select
                    ...
                    onChange={(selected) =>
                      setWorkerFilters({
                        ...workerFilters,
                        nationalities: selected ? selected.map((s) => s.value) : [],
                      } as any)
                    }
                    styles={{
                      control: (base) => ({
                ...
                <select
                  value={workerFilters.wpsStatus}
                  onChange={(e) =>
                    setWorkerFilters({ ...workerFilters, wpsStatus: e.target.value } as any)
                  }
                  className="bg-white border border-zinc-200 text-zinc-700 text-xs font-bold px-3 py-[9px] rounded-lg outline-none cursor-pointer hover:bg-zinc-50 transition-all"
                >
                ...
                <select
                  value={workerFilters.expiryStatus}
                  onChange={(e) =>
                    setWorkerFilters({ ...workerFilters, expiryStatus: e.target.value } as any)
                  }
                  className="bg-white border border-zinc-200 text-zinc-700 text-xs font-bold px-3 py-[9px] rounded-lg outline-none cursor-pointer hover:bg-zinc-50 transition-all"
                >
              ...
              <div className="flex flex-wrap gap-4 items-center bg-zinc-50 p-3 rounded-xl border border-zinc-100 mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    type="date"
                    value={workerFilters.contractExpiryStart}
                    onChange={(e) =>
                      setWorkerFilters({
                        ...workerFilters,
                        contractExpiryStart: e.target.value,
                      } as any)
                    }
                    className="bg-white border border-zinc-200 text-zinc-700 text-xs font-bold px-2 py-1 rounded-md outline-none"
                    placeholder="من"
                  />
                  <span className="text-zinc-400 font-medium text-xs">-</span>
                  <input
                    type="date"
                    value={workerFilters.contractExpiryEnd}
                    onChange={(e) =>
                      setWorkerFilters({
                        ...workerFilters,
                        contractExpiryEnd: e.target.value,
                      } as any)
                    }
                    className="bg-white border border-zinc-200 text-zinc-700 text-xs font-bold px-2 py-1 rounded-md outline-none"
                    placeholder="إلى"
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    type="date"
                    value={workerFilters.iqamaExpiryStart}
                    onChange={(e) =>
                      setWorkerFilters({
                        ...workerFilters,
                        iqamaExpiryStart: e.target.value,
                      } as any)
                    }
                    className="bg-white border border-zinc-200 text-zinc-700 text-xs font-bold px-2 py-1 rounded-md outline-none"
                    placeholder="من"
                  ...
                  <input
                    type="date"
                    value={workerFilters.iqamaExpiryEnd}
                    onChange={(e) =>
                      setWorkerFilters({ ...workerFilters, iqamaExpiryEnd: e.target.value } as any)
                    }
                    className="bg-white border border-zinc-200 text-zinc-700 text-xs font-bold px-2 py-1 rounded-md outline-none"
                    placeholder="إلى"
                  />
          ...
          <div className="overflow-auto flex-1">
            <table className="w-full text-right text-sm">
              <thead className="bg-zinc-50 border-b border-zinc-100 text-zinc-500 font-bold sticky top-0 z-10">
                <tr>
                  <th className="p-4">العامل والدور</th>
                  <th className="p-4">الجنسية</th>
                  <th className="p-4">أرقام الهوية والتأشيرة</th>
                  <th className="p-4">تنبيهات</th>
              ...
              <tbody className="divide-y divide-zinc-100 font-medium">
                {filteredWorkersList.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-zinc-400 font-bold">
                      لا يوجد عمال يطابقون هذه الفلاتر المطروحة.
                    </td>
                  </tr>
                ) : null}
                {filteredWorkersList.map((w) => (
                  <tr
                    key={w.id}
                    className="hover:bg-zinc-50/50 transition-colors whitespace-nowrap align-top"
                  >
                    <td className="p-4 align-top">
                      <div className="flex flex-col">
                        <span className="font-bold text-zinc-900">{w.name}</span>
                        <span className="text-[10px] text-zinc-400 flex items-center gap-1">
                          <MapPin className="w-3 h-3" /> {w.country} • {w.role}
                        </span>
                      </div>
                    </td>
                    <td className="p-4 text-xs font-bold text-zinc-600 align-top">
                      {w.nationality}
                    </td>
                    <td className="p-4 font-mono text-zinc-500 text-xs flex flex-col gap-1 align-top">
                      <span>{w.visaNumber || "بدون تأشيرة"}</span>
                      <span>{w.passportNumber || "بدون جواز"}</span>
                    </td>
                    <td className="p-4 text-xs text-zinc-600 align-top">
                      {w.alerts.length > 0 ? (
                        <div className="flex flex-col gap-1.5 min-w-[200px] whitespace-normal">
                          {w.alerts.map((a, i) => (
                            <div
                              key={i}
                              className={cn(
                                "p-2 rounded-xl border flex flex-col gap-1.5 shadow-sm",
                                a.daysRemaining !== undefined && a.daysRemaining < 0
                                  ? "bg-rose-50 border-rose-100"
                                  : "bg-amber-50 border-amber-100",
                                a.ruleType === "wps" && "bg-rose-50 border-rose-100"
                              )}
                            >
                                ...
                                <span
                                  className={cn(
                                    a.daysRemaining !== undefined && a.daysRemaining < 0
                                      ? "text-rose-700"
                                      : "text-amber-700",
                                    a.ruleType === "wps" && "text-rose-700"
                                  )}
                                >
                                  {a.ruleType} • {a.country}
                                </span>
                                {a.daysRemaining !== undefined && (
                                  <span
                                    className={cn(
                                      "px-1.5 py-0.5 rounded bg-white/50 text-[9px]",
                                      a.daysRemaining < 0
                                  ...
                                  >
                                    {a.daysRemaining === 0
                                      ? "اليوم"
                                      : a.daysRemaining < 0
                                        ? `منذ ${Math.abs(a.daysRemaining)} يوم`
                                        : `متبقي ${a.daysRemaining} يوم`}
                                  </span>
                                )}
                              </div>
                              <span
                        ...
                        <span className="px-2 py-0.5 rounded font-bold uppercase bg-emerald-50 text-emerald-600">
                          سليم
                        </span>
                      )}
                    </td>
                    <td className="p-4 align-top">
                      <div className="flex justify-center">
                        <RiskGauge score={w.riskScore} size={40} />
                      </div>
                    </td>
                    <td className="p-4 text-left align-top">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => setEditingWorker(w as any)}
                          className="text-[10px] font-black bg-blue-50 text-blue-700 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition-all shadow-sm"
                        >
                          تعديل
                        </button>
                        <button
                          onClick={() => setEditingWorker(w as any)}
                          className="text-[10px] font-black bg-white border border-zinc-200 text-zinc-600 px-3 py-1.5 rounded-lg hover:bg-zinc-50 transition-all shadow-sm flex items-center gap-1"
                        >
                          <FileText className="w-3 h-3" /> وثائق
                        </button>
                        <button
                          onClick={() => deleteWorker(w.id)}
                          className="text-[10px] font-black bg-rose-50 border border-rose-100 text-rose-600 px-3 py-1.5 rounded-lg hover:bg-rose-100 transition-all shadow-sm flex items-center gap-1"
                        >
        ...
        >
          <div className="bg-zinc-900 text-white rounded-[2rem] p-8 shadow-xl">
            <h2 className="text-2xl font-black mb-2 flex items-center gap-3">
              محرك الامتثال الإقليمي
            </h2>
            <p className="text-zinc-400 text-sm max-w-xl leading-relaxed">
              تحكم بحدود التنبيهات (Warning Thresholds) لتتناسب مع سرعة الإجراءات في شركتك، وسيقوم
              النظام بتحديث درجات المخاطرة أتماتيكياً.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {rules.map((rule) => (
              <div
                key={rule.id}
                className="bg-white p-6 rounded-[2rem] border border-zinc-200 shadow-sm flex flex-col hover:border-blue-200 hover:shadow-md transition-all"
              >
                <div className="flex justify-between items-start mb-4">
                ...
                <div className="bg-amber-50 border border-amber-100 p-3 rounded-xl mb-4 flex 1 justify-between items-center">
                  <label className="text-xs font-bold text-amber-900 flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5" /> حد التنبيه (أيام):
                  </label>
                  <input
                    type="number"
                    min="0"
                    ...
                    onChange={(e) =>
                      setRules((prev) =>
                        prev.map((r) =>
                          r.id === rule.id
                            ? { ...r, thresholdDays: parseInt(e.target.value) || 0 }
                            : r
                        )
                      )
                    }
                    className="w-16 px-2 py-1 bg-white border border-amber-200 rounded-lg text-xs font-bold outline-none focus:ring-2 focus:ring-amber-500/20 text-center"
                  />
                </div>

                <div className="w-full h-px bg-zinc-100 mb-4" />
                <h5 className="text-[10px] font-bold text-zinc-400 mb-3 uppercase tracking-widest">
                  الدول المطبقة
                </h5>
                <div className="space-y-2 mb-4">
                  {rule.countries.map((c: any) => (
                    <div
                      key={c.code}
                      className="flex flex-col gap-2 bg-zinc-50 border border-zinc-100 p-3 rounded-xl"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Globe className="w-3.5 h-3.5 text-zinc-400" />
                          <span className="text-xs font-bold text-zinc-700">{c.name}</span>
                        </div>
                        <button
                          onClick={() => {
                            setRules((prev) =>
                              prev.map((r) =>
                                r.id === rule.id
                                  ? {
                                      ...r,
                                      countries: r.countries.map((ct: any) =>
                                        ct.code === c.code ? { ...ct, active: !ct.active } : ct
                                      ),
                                    }
                                  : r
                              )
                        ...
                        >
                          {c.active ? (
                            ...
                            <ToggleLeft className="w-5 h-5 text-zinc-300 hover:text-zinc-400" />
                          )}
                        </button>
                      </div>
      ...
      {activeTab === "zatca" && (
        ...
        >
          <div className="bg-emerald-900 text-white rounded-[2rem] p-8 shadow-xl">
            <h2 className="text-2xl font-black mb-2 flex items-center gap-3">
              التحقق عبر ZATCA (هيئة الزكاة والضريبة والجمارك)
            </h2>
            <p className="text-emerald-100 text-sm max-w-xl leading-relaxed">
              تحقق فورياً من السجلات التجارية وشهادات الإمتثال للمقاولين وموردي العمالة بشكل مباشر
          ...
          <div className="bg-white p-8 rounded-[2rem] border border-zinc-200 shadow-sm flex flex-col items-center justify-center max-w-2xl mx-auto">
            ...
            <div className="w-full flex gap-3">
              {zatcaValidationType === "cr" && (
                <input
                  type="text"
                  value={zatcaCR}
                  onChange={(e) => setZatcaCR(e.target.value)}
                  placeholder="رقم السجل التجاري (10 أرقام)"
                  className="flex-1 px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-emerald-500/20"
                />
              )}
              {zatcaValidationType === "vat" && (
                <input
                  type="text"
                  value={zatcaVat}
                  onChange={(e) => setZatcaVat(e.target.value)}
                  placeholder="الرقم الضريبي (15 رقم ويبدأ بـ 3)"
                  className="flex-1 px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-emerald-500/20"
                />
              )}
              {zatcaValidationType === "cert" && (
            ...
            {zatcaResult && (
              <motion.div
                ...
                className={cn(
                  "w-full mt-6 p-6 rounded-2xl border flex flex-col gap-4",
                  zatcaResult.valid
                ...
                <div className="flex items-center gap-4">
                  <div
                    className={cn(
                      ...
                      zatcaResult.valid ? "bg-emerald-100/50" : "bg-rose-100/50"
                    )}
                  >
                    {zatcaResult.valid ? (
                  ...
                  <div>
                    <h4 className="font-black text-xl mb-1 truncate">
                      {zatcaResult.valid
                        ? "البيانات صحيحة ومطابقة"
                        : "البيانات غير صحيحة أو توجد مخالفات"}
                    </h4>
                    <p className="text-sm font-medium opacity-80">
                      تم التحقق في: {new Date(zatcaResult.checkedAt).toLocaleString("ar-SA")}
                    </p>
                  </div>
                </div>

                {!zatcaResult.valid &&
                  zatcaResult.validationErrors &&
                  zatcaResult.validationErrors.length > 0 && (
                    <div className="bg-rose-100/50 p-4 rounded-xl border border-rose-200/50">
                      <ul className="list-disc list-inside text-sm font-bold text-rose-800 space-y-1">
                        {zatcaResult.validationErrors.map((err: string, idx: number) => (
                          <li key={idx}>{err}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div className="bg-white/60 p-4 rounded-xl border border-emerald-200/50 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mt-2">
                    {Object.entries(zatcaResult.details).map(([key, value]) => (
                      <div key={key} className="flex flex-col">
                        <span className="text-[10px] font-black uppercase text-emerald-600/70">
                          {key}
                        </span>
              ...
              <div className="border border-zinc-200 p-6 rounded-2xl bg-zinc-50 flex flex-col items-center justify-center text-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500 font-medium">
                    لتوثيق الفواتير وسجلات الامتثال
                  </p>
                </div>
                <label className="bg-zinc-900 w-full text-white font-bold text-xs py-2 px-4 rounded-xl cursor-pointer hover:bg-zinc-800 transition">
                  رفع المفتاح
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.files?.length) {
                        toast.success(
                          "تم رفع المفتاح الخاص بنجاح وسيتم تخزينه محلياً بشكل آمن (Encrypted Vault)."
                        );
                      }
                    }}
                  />
            ...
            <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <p className="text-sm font-bold text-emerald-900">
                  حالة الربط مع منصة فاتورة (ZATCA)
                </p>
              </div>
              <span className="text-xs font-black bg-emerald-200/50 text-emerald-800 px-3 py-1 rounded-full">
                متصل (Online)
              </span>
            </div>
          </div>
      ...
      {activeTab === "gosi" && (
        ...
        >
          <div className="bg-teal-900 text-white rounded-[2rem] p-8 shadow-xl">
            <h2 className="text-2xl font-black mb-2 flex items-center gap-3">
              لوحة التأمينات الاجتماعية (GOSI)
            </h2>
            <p className="text-teal-100 text-sm max-w-xl leading-relaxed">
              متابعة دقيقة لاشتراكات التأمينات لكل موظف والتأكد من مطابقتها للراتب الأساسي المُسجل،
              ...
              <table className="w-full text-right" dir="rtl">
                ...
                <tbody className="divide-y divide-zinc-50 flex-1 h-full">
                  {computedWorkers.map((w) => {
                    ...
                    const baseSalary = (w as any).baseSalaryHalalas
                      ? (w as any).baseSalaryHalalas / 100
                      : 4000;
                    ...
                    return (
                      <tr key={w.id} className="hover:bg-zinc-50 transition-colors">
                        ...
                        <td className="p-4 text-xs font-medium text-zinc-600">
                          {w.nationality || "غير محدد"}
                        </td>
                        <td className="p-4 text-sm font-bold text-zinc-800">
                          {baseSalary.toLocaleString()} ر.س
                        </td>
                        <td className="p-4">
                          <div className="font-black text-sm text-teal-700">
                            {expectedGosi.toLocaleString()} ر.س
                          </div>
                          <div className="text-[10px] text-zinc-400 mt-1">
                            صاحب العمل: {(baseSalary * gosiCompanyRate).toLocaleString()} ر.س{" "}
                            {isSaudi
                              ? `+ الموظف: ${(baseSalary * gosiEmployeeRate).toLocaleString()} ر.س`
                              : ""}
                          </div>
                        </td>
                        <td className="p-4">
                  ...
                  {computedWorkers.length === 0 && (
                    <tr>
                      <td colSpan={5} className="p-8 text-center text-zinc-400 text-sm font-bold">
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-[2rem] border border-zinc-200 shadow-sm flex flex-col gap-6">
              <div>
                ...
                <p className="text-xs text-zinc-500 font-medium">
                  قم بضبط لهجة الردود ونماذج الذكاء الاصطناعي المفضلة للتحليلات.
                </p>
              </div>
              <div className="space-y-4 flex-1">
                <div className="space-y-2">
                  ...
                  <select
                    value={aiSettings.dialect}
                    onChange={(e) => setAiSettings({ ...aiSettings, dialect: e.target.value })}
                    className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none cursor-pointer"
                  >
                ...
                <div className="space-y-2">
                  <label className="text-sm font-bold text-zinc-700">
                    توجيه مخصص (Custom Prompt)
                  </label>
                  <textarea
                    value={aiSettings.customPrompt}
                    onChange={(e) => setAiSettings({ ...aiSettings, customPrompt: e.target.value })}
                    rows={3}
                    placeholder="اكتب تعليمات إضافية لتوجيه الذكاء الاصطناعي..."
                    className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm resize-none outline-none font-medium"
                  ></textarea>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-[2rem] border border-zinc-200 shadow-sm flex flex-col gap-6">
              <div>
                ...
                <p className="text-xs text-zinc-500 font-medium">
                  تحكم في وتيرة الإشعارات وتنبيهات العقود والمستندات للعمالة.
                </p>
              </div>
              <div className="space-y-4 flex-1">
                <div className="space-y-2">
                  <label className="text-sm font-bold text-zinc-700">
                    إضافة فترة تنبيه جديدة (بالأيام)
                  </label>
                  <div className="flex gap-2">
                    <input
                      type="number"
                      min="1"
                      value={notificationSettings.reminderPeriod}
                      onChange={(e) =>
                        setNotificationSettings({
                          ...notificationSettings,
                          reminderPeriod: parseInt(e.target.value) || 0,
                        })
                      }
                      className="w-24 px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none text-center"
                    />
                    <button
                      onClick={() => {
                        if (
                          notificationSettings.reminderPeriod > 0 &&
                          !notificationSettings.reminderPeriods.includes(
                            notificationSettings.reminderPeriod
                          )
                        ) {
                          setNotificationSettings({
                            ...notificationSettings,
                            reminderPeriods: [
                              ...notificationSettings.reminderPeriods,
                              notificationSettings.reminderPeriod,
                            ].sort((a, b) => b - a),
                          });
                        }
                      }}
                      className="px-4 py-2 bg-blue-50 text-blue-600 font-bold rounded-xl hover:bg-blue-100 transition whitespace-nowrap text-sm"
                    >
                      إضافة الفترة
                    </button>
                  </div>
                ...
                <div className="space-y-3">
                  <label className="text-sm font-bold text-zinc-700">
                    فترات التنبيه النشطة (أيام قبل الانتهاء)
                  </label>
                  <div className="flex flex-wrap gap-2">
                    {notificationSettings.reminderPeriods.length === 0 && (
                      <span className="text-xs text-zinc-400 font-medium">
                        لا توجد فترات تنبيه تم إعدادها.
                      </span>
                    )}
                    {notificationSettings.reminderPeriods.map((period) => (
                      <div
                        key={period}
                        className="flex items-center gap-2 bg-zinc-100 px-3 py-1.5 rounded-lg border border-zinc-200"
                      >
                        ...
                        <button
                          onClick={() =>
                            setNotificationSettings({
                              ...notificationSettings,
                              reminderPeriods: notificationSettings.reminderPeriods.filter(
                                (p) => p !== period
                              ),
                            })
                          }
                  ...
                  <p className="text-[10px] text-zinc-400 mt-2 leading-relaxed">
                    ستصلك إشعارات وتنبيهات في لوحة القيادة (Dashboard) وفي واجهة النظام عند وصول أي
                    عقد أو إقامة إلى هذه الفترات المتبقية قبل الانتهاء.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-end">
            <button
              ...
              className="px-8 py-3 bg-zinc-900 text-white font-bold rounded-xl shadow-lg hover:scale-105 transition-transform flex items-center gap-2"
            >
        ...
        {editingWorker && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-900/60 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-white w-full max-w-5xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row h-[90vh] min-h-[600px] border border-white/20"
              dir="rtl"
            >
              <div className="w-full md:w-1/3 bg-zinc-50 p-8 border-l border-zinc-100 flex flex-col overflow-y-auto">
                <div className="flex justify-between items-start mb-6">
                  <div className="w-16 h-16 bg-blue-100 text-blue-700 rounded-2xl flex items-center justify-center font-black text-2xl shadow-sm border border-blue-200">
                    {editingWorker.name ? (
                      editingWorker.name.charAt(0)
                    ) : (
                  ...
                  <button
                    ...
                    className="p-2 bg-white rounded-full border border-zinc-200 text-zinc-400 hover:text-zinc-600 md:hidden"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
                <h2 className="text-xl font-black text-zinc-900 mb-1">
                  {editingWorker.name || "عامل جديد"}
                </h2>
                <div className="p-4 bg-white rounded-2xl border border-zinc-100 mt-4">
                  <h3 className="text-xs font-bold text-zinc-900 mb-3 flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-primary" /> حالة الامتثال الأساسية
                  </h3>
                  <div className="space-y-3">
                    <div>
                      <label className="text-[10px] font-bold text-zinc-500">
                      ...
                      <select
                        value={editingWorker.wpsStatus}
                        onChange={(e) =>
                          setEditingWorker({ ...editingWorker, wpsStatus: e.target.value as any })
                        }
                        className="w-full mt-1 p-2 bg-zinc-50 border border-zinc-200 rounded-lg text-sm outline-none"
                      >
                        <option value="compliant">ملتزم</option>
                        <option value="delayed">متأخر الدفع</option>
                        <option value="violation">مخالفة مرصودة</option>
                      </select>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 flex flex-col bg-white overflow-hidden">
                <div className="p-6 border-b border-zinc-100 flex justify-between items-center bg-white z-10 sticky top-0 hidden md:flex">
                  <h3 className="font-black text-lg text-zinc-900">
                    {isAddingNew ? "إنشاء ملف عامل جديد" : "تحديث بيانات العامل والمستندات"}
                  </h3>
                  <button
                    onClick={() => setEditingWorker(null)}
                    className="p-2 bg-zinc-50 rounded-xl border border-zinc-200 text-zinc-400 hover:text-zinc-600 transition-all"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
                  ...
                  <form id="worker-form" onSubmit={handleSaveWorker} className="space-y-6">
                    <div>
                      <h4 className="text-sm font-black text-zinc-900 mb-4 flex items-center gap-2">
                        <Users className="w-4 h-4 text-blue-500" /> المعلومات والتفاصيل التعاقدية
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-zinc-500">اسم العامل</label>
                          <input
                            required
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEditingWorker({ ...editingWorker, nationality: e.target.value })
                            }
                            className="w-full px-4 py-2.5 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none"
                          />
                        </div>
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-zinc-500">
                            رقم التأشيرة (Visa Number)
                          </label>
                          <input
                            value={editingWorker.visaNumber || ""}
                            onChange={(e) =>
                              setEditingWorker({ ...editingWorker, visaNumber: e.target.value })
                            }
                            className="w-full px-4 py-2.5 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none"
                          />
                        </div>
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEditingWorker({ ...editingWorker, passportNumber: e.target.value })
                            }
                            className="w-full px-4 py-2.5 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none"
                          />
                        </div>
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-zinc-500">بداية العقد</label>
                          <input
                            type="date"
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-zinc-500">نهاية العقد</label>
                          <input
                            type="date"
                            value={editingWorker.contractEndDate}
                            onChange={(e) =>
                              setEditingWorker({
                                ...editingWorker,
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-zinc-500">
                            انتهاء الإقامة (Iqama)
                          </label>
                          <input
                            type="date"
                            value={editingWorker.iqamaExpiry}
                            onChange={(e) =>
                              setEditingWorker({ ...editingWorker, iqamaExpiry: e.target.value })
                            }
                            className="w-full px-4 py-2.5 bg-zinc-50 border border-zinc-200 rounded-xl text-sm font-bold outline-none"
                          />
                        </div>
                  ...
                  <div>
                    <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 mb-6">
                      <div>
                        <h4 className="text-sm font-black text-zinc-900 flex items-center gap-2">
                          <File className="w-4 h-4 text-blue-500" /> إدارة مرفقات الوثائق وصلاحيتها
                        </h4>
                      </div>
                      <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (file && editingWorker) {
                              const type = (
                                document.getElementById("docUploadType") as HTMLSelectElement
                              ).value as any;
                              ...
                              setEditingWorker({
                                ...editingWorker,
                                documents: [...(editingWorker.documents || []), newDoc],
                              });
                            }
                        ...
                        <button
                          ...
                          type="button"
                          className="text-[11px] font-bold bg-zinc-900 text-white px-4 py-2 rounded-xl flex items-center gap-2 shadow-sm hover:scale-105 transition-all"
                        >
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {editingWorker.documents?.map((doc) => (
                        <div
                          key={doc.id}
                          className="flex flex-col sm:flex-row items-start lg:items-center justify-between p-4 border border-zinc-200 bg-white rounded-xl gap-4 hover:border-blue-200 hover:shadow-sm transition-all group"
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <h5 className="text-sm font-bold text-zinc-900 line-clamp-1">
                                {doc.name}
                              </h5>
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className="text-[9px] font-black uppercase tracking-wider bg-zinc-100 text-zinc-500 px-1.5 py-0.5 rounded">
                                  {doc.type}
                                </span>
                          ...
                          <div className="flex items-end gap-3 w-full lg:w-auto shrink-0">
                            ...
                            <div className="flex gap-1">
                              <button
                                ...
                                className="p-2 text-zinc-400 hover:text-blue-500 bg-zinc-50 rounded-lg"
                                title="عرض المستوى"
                              >
                              ...
                              <button
                                ...
                                className="p-2 text-rose-400 hover:text-rose-500 bg-rose-50 rounded-lg"
                                title="حذف المستند"
                              >
                      ...
                      {(!editingWorker.documents || editingWorker.documents.length === 0) && (
                        <div className="text-center py-8 text-zinc-400 text-xs font-bold">
                          لا توجد مرفقات مرتبطة بهذا العامل
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="p-6 border-t border-zinc-100 bg-zinc-50 flex gap-3 shrink-0">
                  <button
                    form="worker-form"
                    type="submit"
                    className="flex-1 bg-zinc-900 text-white py-3 rounded-xl font-bold shadow-lg hover:scale-[1.01] transition-all text-sm flex items-center justify-center gap-2"
                  >
                    <CheckCircle2 className="w-5 h-5" /> التحديث واعتماد البيانات
                  </button>
                  <button
                    onClick={() => setEditingWorker(null)}
                    type="button"
                    className="px-6 py-3 bg-white border border-zinc-200 text-zinc-600 rounded-xl font-bold hover:bg-zinc-100 transition-all text-sm"
                  >
                    إلغاء
                  </button>
                </div>
