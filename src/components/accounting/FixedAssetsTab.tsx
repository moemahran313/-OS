...
import { 
  Briefcase, Plus, Calendar, FileText, Settings, Play, ArrowUpRight, 
  Trash2, ShieldCheck, CheckCircle2, ChevronRight 
} from "lucide-react";
...
export default function FixedAssetsTab({
  ...
  const [assets, setAssets] = useState<FixedAsset[]>([
    { id: "asset-1", name: "مبنى الإدارة الرئيسي - الرياض", category: "Buildings", purchaseDate: "2024-01-01", cost: 1200000, residualValue: 200000, usefulLifeYears: 25, depreciationMethod: "Straight-Line", accumulatedDepreciation: 96000 },
    { id: "asset-2", name: "خط إنتاج التعبئة والتغليف المطور", category: "Machinery", purchaseDate: "2025-01-01", cost: 350000, residualValue: 30000, usefulLifeYears: 8, depreciationMethod: "Declining-Balance", accumulatedDepreciation: 87500 },
    { id: "asset-3", name: "خوادم الحوسبة السحابية والمعدات الشبكية", category: "IT Equipment", purchaseDate: "2025-06-01", cost: 80000, residualValue: 5000, usefulLifeYears: 4, depreciationMethod: "Straight-Line", accumulatedDepreciation: 18750 },
    { id: "asset-4", name: "أسطول سيارات توزيع المواد المبردة", category: "Vehicles", purchaseDate: "2025-03-01", cost: 240000, residualValue: 40000, usefulLifeYears: 5, depreciationMethod: "Straight-Line", accumulatedDepreciation: 40000 },
  ]);
  ...
  const handlePostDepreciation = () => {
    ...
    if (onPostJournal) {
      ...
      onPostJournal({
        ...
        lines: [
          { accountCode: "508001", accountName: "مصروف إهلاك الأصول الثابتة", debit: totalDepValue, credit: 0 },
          { accountCode: "108001", accountName: "مجمع إهلاك الأصول الثابتة المتراكم", debit: 0, credit: totalDepValue },
        ]
      });
    }
    ...
    <div className="space-y-6 text-right">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-850 p-5 rounded-2xl shadow-sm">
          <span className="text-[10px] text-zinc-400 font-bold uppercase">القيمة الشرائية الكلية للأصول (Total Cost)</span>
          <h4 className="text-xl font-black text-zinc-900 dark:text-zinc-100 font-mono mt-2">
            {totalCost.toLocaleString()} ر.س
          </h4>
          <p className="text-[10px] text-zinc-400 font-bold mt-1">مسجل لـ {assets.length} أصول رئيسية ومستدامة</p>
        </div>

        <div className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-850 p-5 rounded-2xl shadow-sm">
          <span className="text-[10px] text-zinc-400 font-bold uppercase">إجمالي مجمع الإهلاك (Accumulated Dep.)</span>
          <h4 className="text-xl font-black text-rose-600 font-mono mt-2">
            {totalAccDep.toLocaleString()} ر.س
          </h4>
          <p className="text-[10px] text-zinc-400 font-bold mt-1">يمثل نسبة إهلاك تراكمية بنسبة {((totalAccDep / totalCost) * 100).toFixed(1)}%</p>
        </div>

        <div className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-850 p-5 rounded-2xl shadow-sm">
          <span className="text-[10px] text-zinc-400 font-bold uppercase">القيمة الدفترية الحالية (Net Book Value)</span>
          <h4 className="text-xl font-black text-emerald-600 dark:text-emerald-400 font-mono mt-2">
            {totalBookValue.toLocaleString()} ر.س
          </h4>
          <p className="text-[10px] text-zinc-400 font-bold mt-1">القيمة المحاسبية الصافية المتبقية بالأستاذ العام</p>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center bg-zinc-50 dark:bg-zinc-950 p-4 rounded-2xl border border-zinc-150 dark:border-zinc-850">
        <div>
          <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-100">سجل الأصول والترحيلات الجارية</h3>
          <p className="text-[10px] text-zinc-400 font-bold">يمكنك إضافة الأصول هنا وتشغيل عملية إهلاك شهري فوري</p>
        </div>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-850 rounded-3xl shadow-sm overflow-hidden">
        <table className="w-full text-xs text-right">
          ...
          <tbody>
            {assets.map((asset) => (
              <tr key={asset.id} className="border-b border-zinc-50 dark:border-zinc-850 hover:bg-zinc-50/20">
                <td className="p-3">
                  <div className="font-black text-zinc-850 dark:text-zinc-150">{asset.name}</div>
                  <div className="text-[10px] text-zinc-400 mt-0.5">رمز: {asset.id.toUpperCase()}</div>
                </td>
                <td className="p-3">
                  <span className="px-2.5 py-0.5 rounded-full text-[9px] font-black bg-zinc-100 text-zinc-650 dark:bg-zinc-800 dark:text-zinc-300">
                    {asset.category === "Buildings" ? "عقارات ومباني" :
                     asset.category === "Machinery" ? "آلات ومعدات" :
                     asset.category === "IT Equipment" ? "أجهزة تقنية" : "سيارات ونقل"}
                  </span>
                </td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            ...
            >
              <div>
                ...
                <p className="text-[10px] text-zinc-400 font-bold">تم احتساب قيم الإهلاك بناءً على المعايير المحاسبية المعتمدة للشركة</p>
              </div>
                  ...
                  <tbody>
                    {depreciationReport.map((item) => (
                      <tr key={item.id} className="border-b border-zinc-50 dark:border-zinc-850">
                        ...
                        <td className="p-3 text-left font-mono">{item.bookValueBefore.toLocaleString()} ر.س</td>
                    ...
                    <tr className="bg-zinc-50 dark:bg-zinc-950 font-black">
                      ...
                      <td className="p-3 text-left font-mono text-indigo-600 dark:text-indigo-400">
                        {depreciationReport.reduce((sum, i) => sum + i.monthlyDep, 0).toLocaleString()} ر.س
                      </td>
                      <td className="p-3"></td>
                    </tr>
                  </tbody>
              ...
              <div className="bg-zinc-50 dark:bg-zinc-950 p-4 rounded-xl border border-zinc-100 dark:border-zinc-850 space-y-2">
                <span className="text-[10px] text-zinc-400 font-black block">صيغة القيد المزدوج التلقائي المقترحة للأستاذ العام:</span>
                <div className="flex justify-between items-center text-xs font-bold border-b border-zinc-100 dark:border-zinc-850 pb-1.5 mt-2">
                  <span className="text-zinc-700 dark:text-zinc-300">حـ/ مصروف إهلاك الأصول الثابتة (مدين)</span>
                  <span className="font-mono text-emerald-600">
                    {depreciationReport.reduce((sum, i) => sum + i.monthlyDep, 0).toLocaleString()} ر.س
                  </span>
                </div>
                <div className="flex justify-between items-center text-xs font-bold">
                  <span className="text-zinc-700 dark:text-zinc-300">حـ/ مجمع إهلاك أصول الأستاذ (دائن)</span>
                  <span className="font-mono text-rose-600">
                    {depreciationReport.reduce((sum, i) => sum + i.monthlyDep, 0).toLocaleString()} ر.s
                  </span>
                </div>
              </div>
      ...
      {showAddAsset && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          ...
          >
            ...
            <form onSubmit={handleAddAsset} className="space-y-3 text-xs font-bold text-zinc-700 dark:text-zinc-300">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) => setNewAsset(p => ({ ...p, category: e.target.value as any }))}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    value={newAsset.cost || ""}
                    onChange={(e) => setNewAsset(p => ({ ...p, cost: parseFloat(e.target.value) || 0 }))}
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => setNewAsset(p => ({ ...p, residualValue: parseFloat(e.target.value) || 0 }))}
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-150 dark:border-zinc-800 rounded-xl px-4 py-3 focus:outline-none font-mono"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => setNewAsset(p => ({ ...p, usefulLifeYears: parseInt(e.target.value) || 5 }))}
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-150 dark:border-zinc-800 rounded-xl px-4 py-3 focus:outline-none font-mono"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) => setNewAsset(p => ({ ...p, depreciationMethod: e.target.value as any }))}
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-150 dark:border-zinc-800 rounded-xl px-4 py-3 focus:outline-none cursor-pointer"
                  >
