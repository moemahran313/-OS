...
import { 
  CreditCard, Plus, Upload, CheckCircle2, AlertTriangle, ArrowRightLeft, 
  Search, RefreshCw, Layers, Check, ShieldAlert, Sparkles 
} from "lucide-react";
...
export default function BankingTab() {
  const [bankAccounts] = useState<BankAccount[]>([
    { id: "bank-1", name: "الحساب الجاري الرئيسي - الراجحي", bankName: "Al Rajhi Bank", accountNo: "SA8080000001092837482", currency: "SAR", ledgerBalance: 1450000, statementBalance: 1450000 },
    { id: "bank-2", name: "المصروفات النثرية والتشغيلية - الأهلي", bankName: "SNB", accountNo: "SA4030000003091827364", currency: "SAR", ledgerBalance: 45000, statementBalance: 48500 },
    { id: "bank-3", name: "حساب التسهيلات والاعتمادات - الرياض", bankName: "Riyadh Bank", accountNo: "SA2010000009081726354", currency: "SAR", ledgerBalance: 820000, statementBalance: 820000 },
  ]);

  const [activeBankId, setActiveBankId] = useState("bank-1");

  const [statementLines, setStatementLines] = useState<StatementLine[]>([
    { id: "stmt-1", date: "2026-05-01", description: "SAR 5000 - دفع رواتب موظفين فرعيين", amount: -5000, matched: false },
    { id: "stmt-2", date: "2026-05-03", description: "فاتورة مبيعات عميل - مجموعة الشايع", amount: 45000, matched: false },
    { id: "stmt-3", date: "2026-05-05", description: "رسوم تشغيل بوابة الدفع مدى", amount: -350, matched: false },
    { id: "stmt-4", date: "2026-05-07", description: "توريدات خامات ومواد بناء للمستودع", amount: -15000, matched: false },
    { id: "stmt-5", date: "2026-05-10", description: "دفعة سداد من المراعي", amount: 89000, matched: false },
  ]);

  const [ledgerLines, setLedgerLines] = useState<LedgerLine[]>([
    { id: "ledg-1", date: "2026-05-01", refNo: "JV-2026-009", description: "مسودة قيد سداد أجور", amount: -5000, matched: false },
    { id: "ledg-2", date: "2026-05-03", refNo: "INV-2026-001", description: "فاتورة رقم 001 - الشايع", amount: 45000, matched: false },
    { id: "ledg-3", date: "2026-05-08", refNo: "PV-2026-022", description: "أمر صرف توريد مستودعات", amount: -15000, matched: false },
    { id: "ledg-4", date: "2026-05-10", refNo: "INV-2026-002", description: "سداد فاتورة المراعي", amount: 89000, matched: false },
  ]);
    ...
    setTimeout(() => {
      ...
      const updatedStatements = statementLines.map(stmt => {
        ...
        if (match) {
          ...
          match.matched = true;
          newAudit.push(`[تطابق تلقائي] تم مطابقة الحركة البنكية بقيمة (${stmt.amount.toLocaleString()} ر.س) مع القيد المحاسبي (${match.refNo}) بنجاح.`);
        }
        return stmt;
      });
  ...
  const handleManualMatch = (stmtId: string, ledgId: string) => {
    ...
    setStatementLines(prev => prev.map(s => s.id === stmtId ? { ...s, matched: true, matchedWith: ledg.refNo } : s));
            ...
            >
              <div className="flex justify-between items-start">
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${
                  isSelected ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-900 dark:text-emerald-400" : "bg-zinc-100 text-zinc-500"
                }`}>
                  <CreditCard className="w-4.5 h-4.5" />
                </div>
                <span className="text-[10px] text-zinc-400 font-bold font-mono">{account.accountNo.substring(0, 10)}...</span>
              </div>
              <h4 className="text-xs font-black text-zinc-800 dark:text-zinc-200 mt-4">{account.name}</h4>
              ...
              <div className="flex justify-between items-center pt-3 border-t border-zinc-100 dark:border-zinc-850 mt-4 text-xs">
                <div>
                  <span className="text-[9px] text-zinc-400 font-bold block">رصيد الأستاذ (Ledger)</span>
                  <span className="font-mono font-black text-zinc-800 dark:text-zinc-200">{account.ledgerBalance.toLocaleString()} ر.س</span>
                </div>
                <div className="text-left">
                  <span className="text-[9px] text-zinc-400 font-bold block">رصيد كشف الحساب</span>
                  <span className="font-mono font-black text-emerald-600 dark:text-emerald-400">{account.statementBalance.toLocaleString()} ر.s</span>
                </div>
              </div>
            </div>
          );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-850 p-6 rounded-3xl shadow-sm space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
              ...
              <p className="text-[10px] text-zinc-400 font-bold">مطابقة العمليات الواردة في كشف الحساب البنكي مع قيود اليومية العامة</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center text-[10px] font-black uppercase text-zinc-400">
                ...
                <span className="font-mono">{statementLines.filter(s => s.matched).length} / {statementLines.length} مطابقة</span>
              </div>
                ...
                {statementLines.map(stmt => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`font-mono font-black ${stmt.amount < 0 ? "text-rose-600" : "text-emerald-600"}`}>
                        {stmt.amount > 0 ? "+" : ""}{stmt.amount.toLocaleString()} ر.س
                      </span>
                    </div>
                    ...
                    {stmt.matched ? (
                      ...
                      <div className="flex justify-between items-center pt-2 border-t border-zinc-100 dark:border-zinc-850">
                        <span className="text-[9px] text-zinc-400 font-bold">بانتظار ربط الحركة</span>
                        <div className="flex gap-1">
                          {ledgerLines.filter(l => !l.matched && l.amount === stmt.amount).map(matchLedg => (
                            <button
                              key={matchLedg.id}
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center text-[10px] font-black uppercase text-zinc-400">
                ...
                <span className="font-mono">{ledgerLines.filter(l => l.matched).length} / {ledgerLines.length} مسواة</span>
              </div>
                ...
                {ledgerLines.map(ledg => (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center">
                      <p className="font-bold text-zinc-850 dark:text-zinc-150">{ledg.description}</p>
                      <span className="font-mono text-[9px] text-indigo-500 font-black">{ledg.refNo}</span>
                    </div>
                    ...
                    {ledg.matched ? (
                      ...
                      <span className="text-[9px] text-zinc-400 font-bold block pt-1">حركة غير مستقرة</span>
                    )}
                  </div>
                ))}
              </div>
        ...
        <div className="lg:col-span-4 bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-850 p-5 rounded-3xl shadow-sm space-y-5 flex flex-col justify-between">
          <div className="space-y-4">
            <div>
              <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-100">سجل عمليات التدقيق والمطابقة</h3>
              <p className="text-[10px] text-zinc-400 font-bold">سجل تتبع الحركات المسواة بنجاح والتسويات المحاسبية</p>
            </div>
            ...
            <div className="space-y-2 max-h-[250px] overflow-y-auto pr-1">
              {reconcileAudit.length === 0 ? (
                ...
                reconcileAudit.map((log, index) => (
                  <div key={index} className="text-[10px] text-zinc-600 dark:text-zinc-300 font-bold border-r-2 border-emerald-500 pr-2.5 py-1">
                    {log}
                  </div>
                ))
              )}
          ...
          <div className="bg-zinc-50 dark:bg-zinc-950 p-4 rounded-xl border border-zinc-100 dark:border-zinc-850 space-y-3 mt-4">
            <span className="text-[10px] text-zinc-400 font-black block">حالة الحساب البنكي النشط</span>
            <div className="flex justify-between text-xs font-bold">
              <span className="text-zinc-500">فروقات غير مسواة:</span>
              <span className="font-mono text-rose-500 font-black">
                {activeAccount ? (activeAccount.statementBalance - activeAccount.ledgerBalance).toLocaleString() : 0} ر.س
              </span>
            </div>
            <div className="w-full bg-zinc-200 dark:bg-zinc-800 h-1.5 rounded-full">
              <div className="bg-emerald-500 h-1.5 rounded-full" style={{ width: reconciledCount > 0 ? "80%" : "30%" }}></div>
            </div>
            <span className="text-[9px] text-zinc-400 font-bold block">تمت مطابقة {reconciledCount} حركات تجارية بالكامل.</span>
          </div>
        </div>
      </div>
    </div>
