...
import { 
  Sparkles, CheckCircle2, AlertTriangle, ArrowRightLeft, Send, 
  Bot, HelpCircle, ArrowUpRight, ShieldCheck, HeartPulse, ShieldAlert 
} from "lucide-react";
...
export default function CopilotTab({
  ...
  const handleSendMessage = (textToSend?: string) => {
    ...
    setTimeout(() => {
      ...
      if (normQuery.includes("صحة") || normQuery.includes("health") || normQuery.includes("score")) {
        responseText = "بناءً على تتبع وتدقيق أرصدة الأستاذ العام والذمم المدينة والدائنة الحالية، قمت بحساب مؤشر صحة العمل المالي (Business Health Score) لمنشأتكم. النتيجة جيدة جداً بنسبة 86%، ولكن هناك نقاط حرجة تتعلق بعمر بعض فواتير العملاء المتأخرة:";
        healthScoreBreakdown = {
          overall: 86,
          metrics: [
            { name: "اتجاه نمو الإيرادات (Revenue growth)", score: 92, description: "نمو ثابت ومطابق للأهداف السنوية بنسبة ارتفاع +12%." },
            { name: "متوسط أعمار ديون العملاء (AR Aging)", score: 68, description: "هناك تراكم للفواتير غير المسددة لأكثر من 90 يوماً مما يضغط على رأس المال العامل." },
            { name: "نسبة كفاية السيولة النقدية (Cash Ratio)", score: 89, description: "توفر ممتاز للنقدية وصندوق الطوارئ لتغطية الالتزامات قصيرة الأجل." },
            { name: "معدل تكلفة الاستحواذ (CAC to LTV)", score: 95, description: "تحسن في كفاءة التسويق والحصول على عملاء ذوي ولاء مرتفع بإنفاق منخفض." }
          ]
        };
      } else if (normQuery.includes("قيد") || normQuery.includes("journal") || normQuery.includes("دفعنا") || normQuery.includes("استلمنا")) {
        responseText = "قمت بتحليل العملية المالية التي ذكرتها، وصيغت لك القيد المزدوج المتوازن تلقائياً لتسجيله بالأستاذ العام للشركة:";
        suggestedJournal = {
          description: "إثبات مصروف إيجار المكاتب نقداً بموجب إشعار Copilot AI",
          lines: [
            { accountCode: "502005", name: "حـ/ إيجارات المكاتب والفروع (مدين)", debit: 12000, credit: 0 },
            { accountCode: "101001", name: "حـ/ صندوق النقدية في الخزينة (دائن)", debit: 0, credit: 12000 }
          ]
        };
      } else if (normQuery.includes("أخطاء") || normQuery.includes("anomalies") || normQuery.includes("شذوذ")) {
        responseText = "أجريت مراجعة شاملة لدفتر اليومية المساعد لكافة الفروع. لم يتم العثور على أخطاء عدم توازن (الدائن والمدين متطابقان). ومع ذلك، هناك تحذير واحد: تم رصد معاملتين متطابقتين بالقيمة (45,000 ر.س) لصالح عميل واحد خلال فترة 24 ساعة، يرجى مراجعتها لمنع تكرار الفوترة المزدوجة.";
      } else {
        responseText = "قمت بفحص طلبك المالي. بصفتي خبيراً محاسبياً قانونياً لـ Madarij ERP، يمكنني مساعدتك بشكل مباشر بالضغط على أحد الإجراءات السريعة في الأسفل لتحليل الأصول، مراجعة الأخطاء المفتوحة، أو توليد قيود اليومية من لغتك مباشرة.";
      }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-right">
      ...
      <div className="lg:col-span-2 bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-850 p-5 rounded-3xl shadow-sm flex flex-col justify-between h-[520px]">
        ...
        <div className="flex items-center gap-2 pb-3 border-b border-zinc-100 dark:border-zinc-850">
          ...
          <div>
            <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-100">المستشار والمساعد المالي المدعوم بالذكاء الاصطناعي</h3>
            <p className="text-[9px] text-zinc-400 font-bold">بوابة الاستعلام السريع والتدقيق المالي الذاتي اللحظي</p>
          </div>
        </div>
          ...
          {messages.map((msg) => (
            ...
            >
              <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 text-[10px] font-black ${
                msg.sender === "user" ? "bg-zinc-100 text-zinc-650" : "bg-emerald-50 text-emerald-600 dark:bg-emerald-950/30"
              }`}>
                {msg.sender === "user" ? "أنا" : <Bot className="w-4 h-4" />}
              </div>

              <div className={`p-3.5 rounded-2xl font-bold space-y-3 ${
                msg.sender === "user" 
                ...
                {msg.healthScoreBreakdown && (
                  <div className="space-y-3 pt-2 bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-100 dark:border-zinc-850">
                    <div className="flex justify-between items-center pb-2 border-b border-zinc-50 dark:border-zinc-850">
                      <span className="text-xs font-black text-zinc-800 dark:text-zinc-200">النتيجة الإجمالية لصحة الأعمال (Score)</span>
                      ...
                      {msg.healthScoreBreakdown.metrics.map((m, idx) => (
                        <div key={idx} className="space-y-1">
                          <div className="flex justify-between text-[10px] font-bold">
                            ...
                            <span className="font-mono text-zinc-900 dark:text-zinc-100 font-black">{m.score}%</span>
                          </div>
                          <div className="w-full bg-zinc-100 dark:bg-zinc-850 h-1 rounded-full">
                            <div className="bg-emerald-500 h-1 rounded-full" style={{ width: `${m.score}%` }}></div>
                          </div>
                ...
                {msg.suggestedJournal && (
                  <div className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-150 dark:border-zinc-850 space-y-3">
                    <div className="text-[10px] text-zinc-400 font-black uppercase">معاينة القيد المقترح:</div>
                    <p className="text-[10px] font-black text-zinc-800 dark:text-zinc-200">{msg.suggestedJournal.description}</p>
                    <div className="space-y-1.5 border-t border-zinc-100 dark:border-zinc-850 pt-2 text-[10px]">
                      {msg.suggestedJournal.lines.map((l: any, idx: number) => (
                        <div key={idx} className="flex justify-between items-center font-bold">
                          <span>{l.accountCode} — {l.name}</span>
                          <span className={`font-mono ${l.debit > 0 ? "text-emerald-600" : "text-rose-600"}`}>
                    ...
                    <button
                      onClick={() => {
                        if (onPostJournal && msg.suggestedJournal) {
                          ...
                          alert("تم ترحيل القيد المحاسبي المولد بالذكاء الاصطناعي للأستاذ العام بنجاح!");
                        }
                      }}
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-850 p-5 rounded-3xl shadow-sm space-y-4">
        <div>
          <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-100">تحليلات وإجراءات تشغيل ذكية مقترحة</h3>
          <p className="text-[10px] text-zinc-400 font-bold">نقرات تشغيل سريعة لتسريع أعمال الرقابة المالية ومراجعة الدفاتر</p>
        </div>
        ...
        <div className="bg-zinc-50 dark:bg-zinc-950 p-4 rounded-xl border border-zinc-100 dark:border-zinc-850 text-[10px] text-zinc-500 leading-relaxed font-bold space-y-1">
          <span className="font-black text-zinc-700 dark:text-zinc-300 block mb-1">الامتثال والأمان الضريبي:</span>
          <p>مساعد الذكاء الاصطناعي معزول تماماً محلياً ويقوم فقط بمراجعة الأرصدة التراكمية، ولا يشارك أي قيم سرية خارج الحاوية المالية الخاصة بـ Madarij OS.</p>
        </div>
      </div>
    </div>
  );
