...
export default function PayrollAudit({ run, onClose }: PayrollAuditProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden flex flex-col max-h-[85vh]">
        <div className="p-8 pb-6 border-b border-zinc-100 flex justify-between items-start bg-zinc-50" dir="rtl">
          <div>
          ...
          <button onClick={onClose} className="p-2 bg-white rounded-full border border-zinc-200 text-zinc-400 hover:text-zinc-600 shadow-sm"><X className="w-5 h-5"/></button>
        </div>
              ...
              {logs.map((log, idx) => (
                <div key={idx} className="relative rtl:pl-0 rtl:pr-10 text-right">
                  <div className="absolute right-0 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-slate-50 text-slate-500 border-4 border-white flex items-center justify-center shadow-sm">
                    {log.action.toLowerCase().includes('lock') || log.action.toLowerCase().includes('إقفال') ? <Lock className="w-3.5 h-3.5" /> : <History className="w-3.5 h-3.5" />}
                  </div>
                  <div className="bg-zinc-50 border border-zinc-100 p-4 rounded-2xl">
                    <div className="flex justify-between items-center mb-1 gap-2">
                      ...
                      <span className="text-[10px] text-zinc-400 font-bold whitespace-nowrap" dir="ltr">
