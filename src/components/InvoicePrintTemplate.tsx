...
export default function InvoicePrintTemplate({ invoice }: InvoicePrintTemplateProps) {
  ...
  const getInvoiceTitle = () => {
    switch (invoice.type) {
      case "simplified":
        return getLabel("فاتورة ضريبية مبسطة", "Simplified Tax Invoice");
      case "credit_note":
        return getLabel("إشعار دائن", "Credit Note");
      case "debit_note":
        return getLabel("إشعار مدين", "Debit Note");
      case "standard":
      default:
        return getLabel("فاتورة ضريبية", "Tax Invoice");
    }
  };
    ...
    >
        ...
        <div className="flex-1 flex flex-col items-start gap-6">
          {invoice.branding?.logo ? (
            <img
              ...
              alt="Logo"
              className={cn("object-contain", isMinimal ? "h-10" : isClassic ? "h-16" : "h-14")}
            />
          ) : (
            <h2 className="text-xl font-black text-zinc-900 tracking-tight">
              {invoice.zatcaConfig?.sellerName || "مدارجOS"}
            </h2>
          )}
          ...
          <div className={cn("flex flex-col gap-1", isMinimal ? "text-zinc-500" : "text-zinc-600")}>
            ...
            <p className="text-sm font-bold tracking-widest text-zinc-400">#{invoice.number}</p>
          </div>
        </div>
          ...
          <React.Fragment key={sectionId}>
                ...
                <div className="flex flex-col gap-4 border-l border-zinc-200/60 pl-6">
                  ...
                  <div className="flex flex-col gap-1">
                    <h3 className="font-black text-zinc-900 text-lg">
                      {invoice.zatcaConfig?.sellerName || "اسم المنشأة"}
                    </h3>

                    <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 mt-2 text-[11px]">
                      <span className="text-zinc-500">
                        {getLabel("الرقم الضريبي", "VAT Number")}
                      </span>
                      <span className="font-bold text-zinc-900">
                        {invoice.zatcaConfig?.sellerVat || "---"}
                      </span>

                      <span className="text-zinc-500">
                        {getLabel("تاريخ الإصدار", "Issue Date")}
                      </span>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 mt-2 text-[11px]">
                      ...
                      <span className="text-zinc-500">
                        {getLabel("تاريخ الاستحقاق", "Due Date")}
                      </span>
                ...
                <table className="w-full text-sm text-left rtl:text-right border-collapse">
                  <thead>
                    ...
                    >
                      <th
                        className={cn(
                          "py-4 px-4 font-bold w-1/2",
                          isClassic ? "px-0" : "rounded-r-xl"
                        )}
                      >
                        {getLabel("الوصف / التفاصيل", "Description / Details")}
                      </th>
                      <th className="py-4 px-2 font-bold text-center">
                        {getLabel("الكمية", "QTY")}
                      </th>
                      <th className="py-4 px-2 font-bold text-center">
                        {getLabel("سعر الوحدة", "Unit Price")}
                      </th>
                      <th
                        className={cn(
                          "py-4 px-4 font-bold text-left rtl:text-left",
                          isClassic ? "px-0" : "rounded-l-xl"
                        )}
                      >
                        {getLabel("المجموع", "Total")}
                      </th>
                    </tr>
                  </thead>
                  <tbody className={cn(!isClassic && "divide-y divide-zinc-100")}>
                    {invoice.lineItems.map((item, idx) => {
                      const unitPrice =
                        item.unitPriceHalalas / 100 || item.totalHalalas / 100 / item.quantity;
                      return (
                        <tr
                          key={idx}
                          className={cn(
                            "group transition-colors",
                            !isMinimal && "hover:bg-zinc-50/50"
                          )}
                        >
                                ...
                                {item.customFields.map(
                                  (cf, cfi) =>
                                    cf.key && (
                                      <div
                                        key={cfi}
                                        className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-zinc-100/80 text-[10px]"
                                      >
                          ...
                          <td
                            className={cn(
                              "py-4 align-top text-left rtl:text-left font-black text-zinc-900",
                              isClassic ? "px-0" : "px-4"
                            )}
                          >
                            {(item.totalHalalas / 100).toLocaleString(undefined, {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            })}
                          </td>
            ...
            {sectionId === "terms" && invoice.paymentTerms && (
              <section className="text-xs text-zinc-500 mt-2">
                <div className="font-bold text-zinc-900 mb-2 uppercase tracking-widest">
                  {getLabel("شروط الدفع", "Payment Terms")}
                </div>
            ...
            {sectionId === "notes" && invoice.notes && (
              <section className="text-xs text-zinc-500 mt-2">
                <div className="font-bold text-zinc-900 mb-2 uppercase tracking-widest">
                  {getLabel("ملاحظات إضافية", "Additional Notes")}
                </div>
      ...
      >
        ...
        <div className="flex-1 max-w-[250px] mb-8">
          <div className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-12">
            {getLabel("ختم المنشأة والتوقيع", "Company Stamp & Signature")}
          </div>
          ...
          <div className="flex flex-col gap-4 text-sm">
            <div className="flex justify-between items-center text-zinc-600 px-4">
              ...
              <span className="font-black">
                {(invoice.subtotalHalalas / 100).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}{" "}
                <span className="text-[10px] ml-1 opacity-60 font-black">{invoice.currency}</span>
              </span>
            </div>
            <div className="flex justify-between items-center text-zinc-600 px-4">
              <span className="font-bold">
                {getLabel("ضريبة القيمة المضافة (15%)", "VAT (15%)")}
              </span>
              <span className="font-black">
                {(invoice.vatAmountHalalas / 100).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}{" "}
                <span className="text-[10px] ml-1 opacity-60 font-black">{invoice.currency}</span>
            ...
            >
              ...
              <div className="flex justify-between items-end relative z-10 w-full">
                <div className="text-[11px] uppercase tracking-widest font-black opacity-90">
                  {getLabel("الإجمالي المستحق", "Total Due")}
                </div>
                <div className="text-3xl font-black text-left rtl:text-left whitespace-nowrap tracking-tight">
                  {(invoice.totalAmountHalalas / 100).toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}{" "}
                  <span className="text-sm ml-1 opacity-80 font-bold">{invoice.currency}</span>
                </div>
              </div>
                ...
                <div className="flex flex-col gap-2 mt-4 pt-4 border-t border-white/20 relative z-10">
                  <div className="flex justify-between w-full opacity-90">
                    <span className="text-[10px] font-bold uppercase">
                      {getLabel("المبلغ المدفوع", "Amount Paid")}
                    </span>
                    <span className="text-sm font-bold">
                      -{(invoice.paidAmountHalalas / 100).toLocaleString()}{" "}
                      <span className="text-[10px] opacity-70 ml-1">{invoice.currency}</span>
                    </span>
                  </div>
                  <div className="flex justify-between w-full">
                    <span className="text-[11px] font-black uppercase text-amber-300">
                      {getLabel("الرصيد المتبقي", "Remaining Balance")}
                    </span>
                    <span className="text-lg font-black text-amber-300">
                      {(invoice.remainingBalanceHalalas / 100).toLocaleString()}{" "}
                      <span className="text-[10px] opacity-70 ml-1">{invoice.currency}</span>
                    </span>
                  </div>
                </div>
      ...
      <div className="mt-16 text-center text-[10px] text-zinc-400 font-bold tracking-widest uppercase border-t border-zinc-100 pt-6">
        {getLabel(
          "تم إصدار هذه الفاتورة آلياً ومطابقة لمتطلبات هيئة الزكاة والضريبة والجمارك.",
          "This tax invoice is issued electronically and complies with ZATCA regulations."
        )}
      </div>
    </div>
  );
