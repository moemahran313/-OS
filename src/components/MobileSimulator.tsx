...
const INITIAL_CHATS = [
  {
    ...
    messages: [
      ...
      {
        id: 2,
        text: "أهلاً مروان، كشف الحساب جاهز في لوحة معلوماتك",
        sender: "system",
        time: "09:15",
      },
  ...
  },
  {
    ...
    messages: [
      {
        id: 1,
        text: "نحتاج تفعيل الدفع عبر آبل باي لتسهيل التحصيل",
        sender: "client",
        time: "24/05",
      },
    ],
...
const INITIAL_CLIENTS = [
  {
    id: 1,
    name: "ماجد بن خالد",
    company: "مؤسسة النور والظلال",
    phone: "+966 55 532 9911",
  ...
  },
  {
    id: 2,
    name: "ليلى حسن",
    company: "حلول البريد السريع",
    phone: "+966 50 321 4455",
    email: "laila@fastpost.sa",
    area: "جدة، الحمراء",
    balance: "3,120 ر.س",
  },
  {
    ...
    company: "مكتب آفاق التطوير",
    phone: "+966 53 777 8899",
    email: "shammari@dh.com",
...
export default function MobileSimulator() {
  ...
  const [customPushBody, setCustomPushBody] = useState(
    "تم رصد تسجيل دخول لحسابك من متصفح سفاري - لندن"
  );
  ...
  useEffect(() => {
    if (
      settings?.language &&
      (settings.language === "ar" || settings.language === "en" || settings.language === "fr")
    ) {
  ...
  const handleSendMessage = (e?: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      triggerNotification(
        "رسالة جديدة من " + (currentChat?.name || "العميل"),
  ...
  const triggerNotification = (title: string, body: string) => {
    ...
    setPushAlertQueue((prev) => [
      `${new Date().toLocaleTimeString()}: [Push] ${title} - ${body}`,
      ...prev,
    ]);
  ...
  const handleCreateMobileInvoice = (e: React.FormEvent) => {
    ...
    triggerNotification(
      "🧾 تم إنشاء الفاتورة المصغرة",
      `فاتورة جديدة للعميل ${newInv.clientName} بمبلغ ${newInv.amount}`
    );
  };
  ...
  const deviceTranslations: Record<string, Record<string, string>> = {
    ar: {
      ...
      notifDesc:
        "قم بصياغة وإطلاق إشعارات فورية تجريبية للتحقق من مظهر الإشعارات وتنبيهات الأمان على أجهزة الموظفين والإداريين.",
      ...
      blueprintHubDesc:
        "تلقي رسائل المبيعات والدعم فوراً، والرد عليها بواسطة قوالب سريعة مبرمجة لعملاء الخليج (موافقة مع متطلبات هيئة الاتصالات والفواتير المباشرة بنقرة واحدة كملفات PDF متكاملة).",
      blueprintDashTitle: "📊 لوحة متابعة ومراقبة فورية للرواتب والامتثال",
      blueprintDashDesc:
        "كشف تتبع فوري لمعدلات التوطين بنظام نطاقات، حالة مسيرات الرواتب بوزارة الموارد البشرية ومستحقات الموردين وتواريخ الرواتب WPS.",
      blueprintSecureTitle: "🔒 أمان ودعم بخصائص عتاد الجوال",
      blueprintSecureDesc:
        "الامتثال الكامل لتشريعات حماية البيانات الشخصية لوائح نظام PDPL عبر FaceID وبصمات الأصابع محلياً، وميزة العمل دون اتصال بالإنترنت ومزامنة البيانات SQLite.",
    ...
    en: {
      ...
      notifDesc:
        "Draft and dispatch simulated push alerts to verify live security notifications on native companion devices.",
      ...
      langSelectDesc:
        "Choose default language inside your simulated device to update directions and text strings.",
      ...
      blueprintSub:
        "Replicating Mudarij OS core functionalities on native mobile companion clients:",
      blueprintHubTitle: "💬 WhatsApp Sales Hub Integration",
      blueprintHubDesc:
        "Allows on-the-go agents to view integrated chat histories, receive push client messages, use customized GCC templates, and dispatch fully ZATCA-compliant PDF invoices in 1-click.",
      blueprintDashTitle: "📊 Multi-KPI Dashboard on the Go",
      blueprintDashDesc:
        "Track real-time Saudi Nitaqat localization statuses, view pending invoices, monitor supplier shipments, and receive countdown warnings for WPS payroll deadlines to avoid lockouts.",
      blueprintSecureTitle: "🔒 Native Security & Biometric FaceID",
      blueprintSecureDesc:
        "Fully compliant with local PDPL policies utilizing hardware biometric MFA (Apple FaceID / Android Biometrics) with offline cache storage logic via unified SQLite engines.",
    ...
    fr: {
      ...
      notifDesc:
        "Rédigez et envoyez des alertes push test pour vérifier les notifications de sécurité.",
      ...
      blueprintHubDesc:
        "Suivi des opportunités, synchronisation des chats et émission instantanée de documents approuvés.",
      blueprintDashTitle: "📊 Tableau de Bord Intuitif Mobile",
      blueprintDashDesc:
        "Suivi immédiat des scores d'emploi locaux, conformité WPS et obligations fiscales.",
      blueprintSecureTitle: "🔒 Sécurité Biométrique Forte FaceID",
      blueprintSecureDesc:
        "Authentification via la reconnaissance faciale de l'appareil, stockage local résilient en mode hors connexion.",
  ...
  return (
    <div
      className="bg-zinc-50 rounded-3xl p-6 border border-zinc-200 shadow-sm"
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 mb-8 border-b border-zinc-200">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-1 font-medium italic">
            {t(
              "settings.mobile.subtitle",
              "محاكاة فورية للبيئة المحمولة بنظام iOS و Android للاتصال بالعملاء وخدمة الدعم مباشرة."
            )}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-white p-1 rounded-xl border border-zinc-200 shadow-sm">
          <button
            ...
            onClick={() => {
              setDeviceOS("ios");
              toast.success(
                settings.language === "ar" ? "تم تفعيل واجهة iOS بنجاح" : "Apple iOS UI mode active"
              );
            }}
            ...
            onClick={() => {
              ...
              toast.success(
                settings.language === "ar"
                  ? "تم تفعيل واجهة Google Android"
                  : "Google Android UI mode active"
              );
            }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 space-y-6">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  onClick={() => {
                    if (mobileLang === "ar") {
                      triggerNotification(
                        "🚨 محاولة احتيال محتملة",
                        "تم حظر محاولة دفع مشبوهة بمبلغ 5,000 ر.س من متصفح غير مدعوم."
                      );
                    } else {
                      triggerNotification(
                        "🚨 Potential Fraud Blocked",
                        "Suspicious payment attempt of 5,000 SAR has been blocked from an untrusted source."
                      );
                    }
                  }}
          ...
          <div className="bg-white p-5 rounded-2xl border border-zinc-200 shadow-sm space-y-3">
            <h4 className="font-bold text-xs text-zinc-950 uppercase tracking-wide">
              {dt.langSelectTitle}
            </h4>
              ...
              {[
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    toast.success(
                      lg.code === "ar"
                        ? "تم تبديل لغة صندوق الإيعاز"
                        : "Device viewport switch done"
                    );
                  }}
            ...
            <p className="text-[10px] text-zinc-400 mt-2">{dt.langSelectDesc}</p>
          </div>
          ...
          <div className="bg-zinc-900 text-zinc-100 p-4 rounded-2xl border border-zinc-800 shadow-inner font-mono text-[10px] space-y-2 h-44 overflow-y-auto">
            ...
            {pushAlertQueue.length === 0 ? (
              ...
              pushAlertQueue.map((log, idx) => (
                <div
                  key={idx}
                  className="leading-tight py-0.5 border-b border-zinc-800/40 text-left ltr:text-left rtl:text-right"
                >
                  {log}
                </div>
              ))
          ...
          <div className="bg-gradient-to-br from-indigo-950 to-slate-900 text-white p-5 rounded-2xl border border-indigo-900/30 shadow-md space-y-4">
            <div>
              ...
              <p className="text-[10px] text-zinc-400 mt-1 leading-relaxed">{dt.blueprintSub}</p>
            </div>
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-3">
                      ...
                      <div className="flex flex-col h-full bg-white rounded-2xl border border-zinc-200 shadow-xs overflow-hidden -mx-1">
                        ...
                        <div className="bg-zinc-50 px-3 py-2 border-b border-zinc-200 flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2 text-right">
                            ...
                            />
                            <div>
                              <h4 className="font-bold text-[11px] text-zinc-900Leading-none">
                                {selectedChat?.name}
                              </h4>
                        ...
                        <div className="flex-1 min-h-[220px] p-2.5 overflow-y-auto bg-zinc-50 space-y-2 flex flex-col justify-end">
                          {selectedChat?.messages.map((msg, i) => (
                            ...
                            >
                              <p>{msg.text}</p>
                              <span className="text-[7px] text-zinc-400 block mt-1 text-left">
                                {msg.time}
                              </span>
                            </div>
                          ))}
                        ...
                        <form
                          ...
                          className="p-2 bg-white border-t border-zinc-200 flex gap-1.5"
                        >
                          <input
                            type="text"
                            value={replyText}
                ...
                {activeScreen === "crm" && (
                  <div className="space-y-3">
                    <div className="bg-white p-3 rounded-xl border border-zinc-200">
                      <h4 className="font-bold text-[11px] text-zinc-600 mb-2">
                        {dt.clientsCount}
                      </h4>
                    ...
                    <div className="space-y-2">
                      {clients.map((cl) => (
                        <div
                          key={cl.id}
                          className="bg-white p-3 rounded-xl border border-zinc-100 flex flex-col gap-2.5 shadow-xs text-right"
                        >
                ...
                {activeScreen === "invoice" && (
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <h5 className="text-[10px] font-bold text-zinc-500 uppercase">
                        الفواتير المنشأة الجاهزة
                      </h5>
                      {simulatedInvoices.length === 0 ? (
                        <p className="text-[9px] text-zinc-400 italic text-center">
                          لم تصدر فواتير جوال بعد.
                        </p>
                      ) : (
                        simulatedInvoices.map((inv) => (
                          <div
                            key={inv.id}
                            className="bg-white p-2.5 rounded-lg border border-zinc-100 flex items-center justify-between shadow-xxs"
                          >
                            <div>
                              <p className="text-[9px] font-bold text-zinc-900 leading-none">
                                {inv.clientName}
                              </p>
                              <p className="text-[7px] text-zinc-400 font-mono mt-0.5">
                                {inv.number} • {inv.item}
                              </p>
                            </div>
                    ...
                    <div className="bg-white p-4 rounded-xl border border-zinc-200 space-y-3">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between p-2 bg-zinc-50 rounded-lg">
                          <div>
                            <p className="text-[10px] font-bold text-zinc-900">
                              سماح بـ Push Notifications
                            </p>
                            <p className="text-[8px] text-zinc-500">
                              استلام تنبيهات الأمان والدخول
                            </p>
                          </div>
                        ...
                        <div className="flex items-center justify-between p-2 bg-zinc-50 rounded-lg">
                          <div>
                            <p className="text-[10px] font-bold text-zinc-900">
                              تنبيهات Unusual Logins
                            </p>
                            <p className="text-[8px] text-zinc-500">
                              إشعار عند تسجيل الدخول من جهاز غير معتمد
                            </p>
                          </div>
                      ...
                      <div className="bg-amber-50 rounded-lg p-2.5 border border-amber-200 text-amber-900 text-[8px] leading-relaxed">
                        ⚠️ <strong>حماية إدارية:</strong> تطبيق الجوال متصل برصد الأخطاء الفوري
                        للمواقع (trusted IP) ومزود ببصمة الوجه لتفويض المعاملات.
                      </div>
                    </div>
                  </div>
                )}
