      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className={`p-5 rounded-2xl border ${stats.nitaqatColor}`}>
          ...
          <p className="text-xs font-bold opacity-80">
            نسبة التوطين: {stats.saudizationPct.toFixed(1)}%
          </p>

          <div className="mt-4 flex max-w-full gap-1 h-2 rounded-full overflow-hidden bg-white/50">
            {stats.saudiCount > 0 && (
              <div
                style={{ width: `${stats.saudizationPct}%` }}
                className="bg-emerald-500 h-full"
              ></div>
            )}
            {stats.expatCount > 0 && (
              <div
                style={{ width: `${100 - stats.saudizationPct}%` }}
        ...
        <div className="p-5 rounded-2xl border border-zinc-200 bg-zinc-50">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-sm font-black ${stats.docsExpired > 0 ? "text-rose-600" : "text-slate-400"}`}
              >
                {stats.docsExpired}
              </span>
            </div>
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-sm font-black ${stats.docsExpiringSoon > 0 ? "text-amber-600" : "text-slate-400"}`}
              >
                {stats.docsExpiringSoon}
              </span>
            </div>
          </div>
        </div>
