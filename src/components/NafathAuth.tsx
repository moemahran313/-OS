...
export default function NafathAuth({
  onVerified,
  className,
}: {
  onVerified?: () => void;
  ...
  if (isVerified && !isOpen) {
    return (
      <div
        className={`flex items-center gap-2 bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg border border-emerald-100 ${className}`}
      >
                ...
                <div className="flex flex-col items-center text-center space-y-4">
                  {status === "waiting" && (
                    <>
                      <div className="w-20 h-20 bg-emerald-50 rounded-full flex items-center justify-center relative">
                        ...
                        <img
                          src="https://upload.wikimedia.org/wikipedia/ar/thumb/5/5f/Nafath_app_logo.svg/512px-Nafath_app_logo.svg.png"
                        ...
                        />
                        {!document.querySelector('img[src*="Nafath_app_logo"]') && (
                      ...
                      <div className="mt-8 pt-4 border-t border-slate-100 w-full flex flex-col gap-2">
                        ...
                        <button
                          ...
                          className="w-full py-2 bg-emerald-50 text-emerald-600 rounded-xl text-xs font-bold border border-emerald-100 hover:bg-emerald-100 transition-colors"
                        >
                          Simulate Success (Accept in App)
                        </button>
                        <button
                          onClick={() => setStatus("failed")}
                          className="w-full py-2 bg-rose-50 text-rose-600 rounded-xl text-xs font-bold border border-rose-100 hover:bg-rose-100 transition-colors"
                        >
                  ...
                  {status === "success" && (
                    ...
                    >
                      ...
                      <p className="text-sm font-medium text-slate-500">
                        تم توثيق هويتك إلكترونياً واعتماد التوقيع.
                      </p>
                    </motion.div>
                  )}
                  {status === "failed" && (
                    <div className="flex flex-col items-center text-rose-600">
                      ...
                      <button
                        onClick={() => setIsOpen(false)}
                        className="mt-4 px-4 py-2 bg-slate-100 text-slate-800 font-bold rounded-xl text-sm"
                      >
                        إلغاء
                      </button>
                    </div>
