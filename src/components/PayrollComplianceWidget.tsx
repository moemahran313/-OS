...
export default function PayrollComplianceWidget({ runs }: { runs: any[] }) {
  ...
  useEffect(() => {
    if (complianceData.isLockdown) {
      ...
      if (!localStorage.getItem(lockKey)) {
        ...
        toast.error(`تم تفعيل وضع Emergency Lockdown لأنظمة الرواتب بسبب تجاوز مهلة WPS للمسير ${complianceData.period}`, {
          duration: 10000
        });
      }
    }
  ...
  return (
    <div className={`p-6 rounded-3xl border shadow-sm flex flex-col gap-4 relative overflow-hidden transition-all ${complianceData.isLockdown ? 'bg-rose-50 border-rose-200' : 'bg-white border-zinc-100'}`}>
      ...
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-3">
          <div className={`p-3 rounded-2xl ${complianceData.isLockdown ? 'bg-rose-100 text-rose-600' : 'bg-emerald-50 text-emerald-600'}`}>
            {complianceData.isLockdown ? <AlertOctagon className="w-6 h-6" /> : <ShieldCheck className="w-6 h-6" />}
          </div>
          <div>
        ...
        <div className="text-left shrink-0 max-w-[150px] w-full">
          {complianceData.isGenerated ? (
            ...
            <div className="flex flex-col gap-1.5 items-end">
              <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-[10px] font-bold ${complianceData.isLockdown ? 'bg-rose-500 text-white animate-pulse' : 'bg-amber-100 text-amber-700'}`}>
                {complianceData.isLockdown ? 'متأخر (إيقاف خدمات)' : `متبقي ${complianceData.daysLeft} يوم`}
              </span>
              {!complianceData.isLockdown && (
                <div className="w-full bg-zinc-200 h-1.5 rounded-full overflow-hidden flex justify-end">
                  <div 
                    className={`h-full transition-all duration-500 ${
                      complianceData.daysLeft > 15 ? 'bg-emerald-500' :
                      complianceData.daysLeft > 5 ? 'bg-amber-400' :
                      'bg-rose-500 text-white'
                    }`} 
                    style={{ width: `${Math.max(0, Math.min(100, (complianceData.daysLeft / 30) * 100))}%` }} 
                  />
                </div>
              )}
            </div>
          ...
          {complianceData.isLockdown ? (
            <div className="mt-2 p-4 bg-rose-100 rounded-xl border border-rose-200">
              ...
              <p className="text-xs font-medium text-rose-700 mb-4 leading-relaxed">
                تم تجاوز المدة النظامية (30 يوم) لرفع مسير شهر {complianceData.period}. قد تواجه الشركة تجميداً للخدمات الحكومية.
              </p>
              <button onClick={handleWhatsAppCFO} className="w-full bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold py-2.5 rounded-lg flex items-center justify-center gap-2 transition-colors">
            ...
            <div className="mt-2 bg-zinc-50 p-4 rounded-xl">
              ...
              <ul className="space-y-2 text-xs font-medium text-zinc-600">
                <li className="flex items-center gap-2">
                  {complianceData.hasRun ? <Check className="w-4 h-4 text-emerald-500" /> : <HelpCircle className="w-4 h-4 text-amber-500" />}
