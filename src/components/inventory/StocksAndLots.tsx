...
import { 
  Layers, Search, Filter, Plus, Info, RefreshCw, Barcode, CheckCircle2,
  Calendar, ShieldAlert, Tag, LayoutGrid, MapPin, Truck, AlertTriangle
} from "lucide-react";
...
const INITIAL_SERIAL_NUMBERS = [
  { serial: "SN-9823102", productId: "TAB-101", status: "In Stock", warehouseId: "Main", purchaseDate: "2026-01-10", saleDate: null, warrantyMonths: 24, customer: null, supplier: "مجموعة الرياض للتأثيث" },
  { serial: "SN-9823103", productId: "TAB-101", status: "In Stock", warehouseId: "Main", purchaseDate: "2026-01-10", saleDate: null, warrantyMonths: 24, customer: null, supplier: "مجموعة الرياض للتأثيث" },
  { serial: "SN-1029311", productId: "CHR-202", status: "Sold", warehouseId: "Riyadh", purchaseDate: "2026-02-14", saleDate: "2026-05-10", warrantyMonths: 12, customer: "الشركة الوطنية للحلول", supplier: "مجموعة الرياض للتأثيث" }
];

// Default Batch / Lot Tracking list
const INITIAL_BATCHES = [
  { batchNo: "LOT-26A-01", productId: "TAB-101", prodDate: "2026-01-15", expiryDate: "2027-01-15", warehouseId: "Main", supplier: "مجموعة الرياض للتأثيث", initialQty: 100, remainingQty: 45, status: "Healthy" },
  { batchNo: "LOT-26B-02", productId: "TAB-101", prodDate: "2026-02-10", expiryDate: "2026-12-10", warehouseId: "Riyadh", supplier: "مجموعة الرياض للتأثيث", initialQty: 50, remainingQty: 15, status: "Expiring Soon" },
  { batchNo: "LOT-26C-03", productId: "TAB-101", prodDate: "2026-03-05", expiryDate: "2027-09-05", warehouseId: "Jeddah", supplier: "مجموعة الرياض للتأثيث", initialQty: 40, remainingQty: 8, status: "Healthy" }
];

export default function StocksAndLots({ 
  items, 
  warehouses, 
  onUpdateProduct 
}: StocksAndLotsProps) {
  ...
  const handleRunFefoPicker = (prodId: string) => {
    ...
    if (sorted.length > 0) {
      ...
      toast.success(`توصية FEFO: الدفعة المناسبة للصرف هي (${sorted[0].batchNo}) بصلاحية حتى ${sorted[0].expiryDate}`);
    } else {
      setFefoSuggestedBatch(null);
  ...
  return (
    <div className="space-y-6">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-zinc-50 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500 font-bold border-b border-zinc-100 dark:border-zinc-800 uppercase tracking-wider text-[11px]">
                  ...
                  {warehouses.map(wh => (
                    <th key={wh.id} className="p-4 text-center">{wh.nameAr}</th>
                  ))}
              ...
              <tbody>
                {filteredItems.length === 0 ? (
                  <tr>
                    <td colSpan={warehouses.length + 5} className="p-12 text-center text-zinc-400 font-bold">
                      لا توجد مواصفات مخزنية مسجلة حالياً.
                    </td>
                  </tr>
                ) : (
                  filteredItems.map(item => {
                    const totalQty = Object.values(item.warehouseQuantities || {}).reduce((a: any, b: any) => Number(a) + Number(b), 0) as number;
                    const minStock = item.minStock || 15;
                    const maxStock = item.maxStock || 200;
                    
                    return (
                      <tr key={item.id} className="border-b border-zinc-50 dark:border-zinc-800 hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-all">
                        <td className="p-4">
                          <span className="font-black text-zinc-900 dark:text-zinc-100 block">{item.nameAr}</span>
                          <span className="text-[10px] text-zinc-400 font-mono block">{item.sku}</span>
                        </td>
                        <td className="p-4">
                          {item.binLocation ? (
                            ...
                            <button
                              onClick={() => {
                                ...
                                setBinAisle(""); setBinRack(""); setBinShelf(""); setBinCode("");
                              }}
                        ...
                        {warehouses.map(wh => {
                          ...
                          return (
                            <td key={wh.id} className="p-4 text-center font-mono font-black text-zinc-800 dark:text-zinc-200">
                              <span className={qty === 0 ? "text-zinc-300" : "text-zinc-900 dark:text-zinc-100"}>
                                {qty}
                              </span>
                            </td>
                          );
                        ...
                        <td className="p-4 text-center font-mono">
                          <span className={`px-2.5 py-1 rounded-full font-black text-xs ${totalQty === 0 ? "bg-red-50 text-red-700" : totalQty < minStock ? "bg-amber-50 text-amber-700" : "bg-emerald-50 text-emerald-700"}`}>
                            {totalQty}
                          </span>
                        </td>
                        ...
                        <td className="p-4 space-y-0.5 text-[10px] text-zinc-500">
                          <div className="flex justify-between">
                            ...
                            <span className="font-mono text-zinc-800 dark:text-zinc-200">{minStock}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>الحد الأقصى:</span>
                            <span className="font-mono text-zinc-800 dark:text-zinc-200">{maxStock}</span>
                          </div>
                          {totalQty < minStock && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm space-y-4">
            ...
            <form onSubmit={handleRegisterSerial} className="space-y-4 text-xs font-bold">
              <div>
                <label className="block text-zinc-400 mb-1">الرقم التسلسلي الفريد (cannot duplicate)</label>
                <input
                  type="text"
                  required
                  value={newSerial}
              ...
              <div>
                ...
                >
                  ...
                  {items.map(item => (
                    <option key={item.id} value={item.id}>{item.nameAr} ({item.sku})</option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                >
                  ...
                  {warehouses.map(wh => (
                    <option key={wh.id} value={wh.id}>{wh.nameAr}</option>
                  ))}
                </select>
              </div>
          ...
          <div className="lg:col-span-2 bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
            <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-100 mb-4 border-b pb-2">قاعدة بيانات الأرقام التسلسلية المسجلة</h3>
            <div className="overflow-x-auto text-xs font-bold text-right">
              <table className="w-full">
                <thead>
                  <tr className="bg-zinc-50 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500 font-bold border-b border-zinc-100 dark:border-zinc-800 uppercase tracking-wider text-[11px]">
                ...
                <tbody>
                  {serials.map((s, idx) => {
                    const matchItem = items.find(i => i.id === s.productId || i.sku === s.productId);
                    return (
                      <tr key={idx} className="border-b border-zinc-50 dark:border-zinc-800">
                        <td className="p-3 font-mono font-black text-indigo-600 dark:text-indigo-400">{s.serial}</td>
                        <td className="p-3">
                          <span className="block">{matchItem ? matchItem.nameAr : "طاولة مكتبية فاخرة"}</span>
                          <span className="text-[10px] text-zinc-400 font-mono block">{matchItem ? matchItem.sku : "TAB-101"}</span>
                        </td>
                        <td className="p-3 text-zinc-600 dark:text-zinc-400">{s.warehouseId}</td>
                        <td className="p-3 space-y-1">
                          <span className={`px-2 py-0.5 rounded text-[9px] font-black inline-block ${s.status === "In Stock" ? "bg-emerald-50 text-emerald-700" : "bg-zinc-50 text-zinc-600"}`}>
                            {s.status === "In Stock" ? "متوفر" : "مباع"}
                          </span>
                          <span className="block text-[9px] text-zinc-400 font-black">ضمان: {s.warrantyMonths} شهر</span>
                        </td>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-emerald-900/10 via-teal-900/5 to-transparent p-5 rounded-[2rem] border border-emerald-500/20 shadow-sm">
            ...
            <p className="text-xs text-zinc-500 mb-4">
              خوارزمية الصرف الذكي (FEFO) - تقوم بالبحث في جميع دفعات الإنتاج وتقترح الدفعة التي تقترب نهاية صلاحيتها لتجنب الهدر المالي للمواد.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
              ...
              {fefoSuggestedBatch ? (
                <div className="md:col-span-2 bg-white dark:bg-zinc-900 border border-emerald-200 dark:border-emerald-900/30 p-4 rounded-2xl flex items-center justify-between gap-4">
                  <div>
                    <span className="text-[9px] font-black text-emerald-600 uppercase block">الدفعة الموصى بصرفها حالياً (FEFO)</span>
                    <h4 className="text-sm font-mono font-black text-indigo-600 mt-1">رقم الدفعة: {fefoSuggestedBatch.batchNo}</h4>
                    <p className="text-[11px] text-zinc-400 mt-0.5">تاريخ انتهاء الصلاحية: <span className="font-mono text-red-500 font-bold">{fefoSuggestedBatch.expiryDate}</span></p>
                  </div>
                  <div className="text-left font-bold">
                    <span className="text-xs text-zinc-400">الكمية المتاحة:</span>
                    <span className="text-xl font-mono text-zinc-950 dark:text-white block font-black">{fefoSuggestedBatch.remainingQty} حبة</span>
                  </div>
                </div>
              ) : (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
              <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-100 mb-4 border-b pb-2">سجل دفعات الإنتاج وتواريخ انتهاء الصلاحية</h3>
              <div className="overflow-x-auto text-xs font-bold text-right">
                <table className="w-full">
                  <thead>
                    <tr className="bg-zinc-50 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500 font-bold border-b border-zinc-100 dark:border-zinc-800 uppercase tracking-wider text-[11px]">
                  ...
                  <tbody>
                    {batches.map((b, idx) => {
                      const matchItem = items.find(i => i.id === b.productId || i.sku === b.productId);
                      return (
                        <tr key={idx} className="border-b border-zinc-50 dark:border-zinc-800">
                          <td className="p-3 font-mono font-black text-indigo-600 dark:text-indigo-400">{b.batchNo}</td>
                          <td className="p-3">
                            <span className="block">{matchItem ? matchItem.nameAr : "طاولة مكتبية فاخرة"}</span>
                            <span className="text-[10px] text-zinc-400 font-mono block">{matchItem ? matchItem.sku : "TAB-101"}</span>
                          </td>
                          <td className="p-3 text-zinc-600 dark:text-zinc-400">{warehouses.find(w => w.id === b.warehouseId)?.nameAr || b.warehouseId}</td>
                          <td className="p-3 font-mono space-y-0.5">
                            <span className="block text-zinc-800 dark:text-zinc-200">{b.expiryDate}</span>
                            <span className="block text-[9px] text-zinc-400">إنتاج: {b.prodDate}</span>
                          </td>
                          <td className="p-3 font-mono font-black text-zinc-950 dark:text-white">{b.remainingQty} حبة</td>
                          <td className="p-3">
                            <span className={`px-2 py-0.5 rounded text-[9px] font-black ${b.status === "Healthy" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>
      ...
      {selectedItemForBin && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-[9999]" dir="rtl">
          <div className="bg-white dark:bg-zinc-900 rounded-[2rem] border w-full max-w-sm p-6 space-y-4">
            <h4 className="font-black text-xs text-zinc-900 dark:text-zinc-100 border-b pb-2">تحديد موقع الرف (Bin Location Config)</h4>
      ...
      {selectedItemForReorder && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-[9999]" dir="rtl">
          <div className="bg-white dark:bg-zinc-900 rounded-[2rem] border w-full max-w-sm p-6 space-y-4">
            <h4 className="font-black text-xs text-zinc-900 dark:text-zinc-100 border-b pb-2">قوانين حماية المخزون وإعادة الطلب</h4>
            <div className="text-xs font-bold space-y-3">
              <div>
                <label className="block text-zinc-400 mb-1">الحد الأدنى للمخزون (نقطة الإنذار)</label>
                <input
                  type="number"
                  value={minStockRule}
              ...
              <div>
                <label className="block text-zinc-400 mb-1">الحد الأقصى للمستودع (السعة الاستيعابية)</label>
                <input
                  type="number"
                  value={maxStockRule}
              ...
              <div>
                <label className="block text-zinc-400 mb-1">مخزون الأمان الإضافي (Safety Stock)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-zinc-400 mb-1">فترة التوريد المعتادة بالأيام (Lead Time)</label>
                <input
                  type="number"
                  value={leadTimeRule}
