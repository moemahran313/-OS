...
import { 
  TrendingUp, AlertTriangle, Cpu, HelpCircle, Package, ArrowRight,
  Sparkles, CheckCircle2, ChevronRight, BarChart3, ShieldAlert
} from "lucide-react";
import { motion, AnimatePresence } from "motion/react";
import { 
  ResponsiveContainer, BarChart, Bar, XAxis, YAxis, 
  CartesianGrid, Tooltip, Legend, LineChart, Line, PieChart, Pie, Cell 
} from "recharts";
...
export default function DashboardOverview({ 
  ...
  const metrics = useMemo(() => {
    ...
    items.forEach(item => {
      ...
      const multiplier = valuationMethod === "fifo" ? 1.02 : valuationMethod === "lifo" ? 0.98 : 1.0;
      ...
      const prodType = item.productType || (item.type === "assembly" ? "Finished Good" : "Raw Material");
  ...
  const abcChartData = useMemo(() => {
    ...
    return [
      { name: "فئة A (الأعلى قيمة 70%)", value: countA || 1, amount: Math.round(valA), color: "#10b981" },
      { name: "فئة B (متوسط القيمة 20%)", value: countB || 1, amount: Math.round(valB), color: "#3b82f6" },
      { name: "فئة C (الأقل قيمة 10%)", value: countC || 1, amount: Math.round(valC), color: "#9ca3af" }
    ];
  }, [items]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm flex flex-col justify-between group hover:border-indigo-500/30 transition-all duration-300 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-wider block">قيمة المخزون الكلية</span>
            <select 
          ...
          <div className="my-4">
            <h3 className="text-2xl font-black text-indigo-600 dark:text-indigo-400 font-mono">
              {metrics.totalValue.toLocaleString(undefined, { minimumFractionDigits: 2 })} <span className="text-xs text-zinc-400 font-sans">ر.س</span>
            </h3>
        ...
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm flex flex-col justify-between group hover:border-amber-500/30 transition-all duration-300 relative overflow-hidden">
          ...
          <span className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-wider block">أصناف منخفضة المخزون</span>
          <div className="my-4">
        ...
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm flex flex-col justify-between group hover:border-red-500/30 transition-all duration-300 relative overflow-hidden">
          ...
          <span className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-wider block">أصناف نفدت تماماً</span>
          <div className="my-4">
            ...
            <p className="text-[10px] text-zinc-400 mt-1">
              مبيعات معلقة بسبب نفاد الكميات
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm flex flex-col justify-between group hover:border-emerald-500/30 transition-all duration-300 relative overflow-hidden">
          ...
          <span className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-wider block">النشاط المستودعي</span>
          <div className="my-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-100 dark:border-zinc-800 p-6 shadow-sm">
          ...
          <div className="h-72 w-full text-xs font-bold">
            {warehouseChartData.length === 0 ? (
              <div className="h-full flex items-center justify-center text-zinc-400">لا توجد مستودعات مدخلة حالياً</div>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={warehouseChartData} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-100 dark:border-zinc-800 p-6 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-[11px] text-zinc-400 mb-4">تصنيف الأصناف حسب المساهمة في القيمة المالية الكلية</p>
          </div>
            ...
            {items.length === 0 ? (
              ...
              <>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  ...
                  <span className="text-xl font-mono font-black text-zinc-900 dark:text-white">{items.length}</span>
                </div>
              </>
            )}
          </div>
          ...
          <div className="space-y-2 mt-4 text-[11px] font-bold">
            {abcChartData.map((data, i) => (
              <div key={i} className="flex items-center justify-between border-b border-zinc-50 dark:border-zinc-800/50 pb-1.5 last:border-0 last:pb-0">
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: data.color }} />
                  <span className="text-zinc-600 dark:text-zinc-300">{data.name}</span>
                </div>
                <div className="text-right">
                  <span className="text-zinc-900 dark:text-zinc-100 font-mono block">{data.amount.toLocaleString()} ر.س</span>
      ...
      <div className="bg-gradient-to-r from-indigo-900/10 via-emerald-900/5 to-transparent dark:from-indigo-950/20 dark:via-emerald-950/10 dark:to-transparent rounded-[2rem] border border-indigo-500/20 p-6 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-black text-zinc-900 dark:text-zinc-100 flex items-center gap-1.5">
                ...
                <span className="px-2 py-0.5 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-[9px] rounded-full font-black uppercase">Alpha</span>
              </h3>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 font-bold mt-0.5">
                توليد تحليلات ذكية للتنبؤ بالنقص، تحسين الموارد، كشف ركود المخزون واقتراح كميات الشراء المثالية.
              </p>
            </div>
          </div>
          <button
        ...
        {aiInsights.length > 0 ? (
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-3 pt-2 border-t border-zinc-100 dark:border-zinc-800/60">
                    <button className="text-[10px] text-indigo-600 dark:text-indigo-400 hover:underline">تنفيذ التوصية</button>
          ...
          <div className="py-8 text-center text-zinc-400 dark:text-zinc-500 font-bold text-xs flex flex-col items-center justify-center gap-2">
            ...
            انقر على زر "تشغيل التحليل التنبؤي" للقيام بجرد ذكي وتقديم تنبؤات المخزون والموردين للـ 30 يوماً القادمة.
          </div>
        )}
      </div>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/30 rounded-2xl p-4 flex items-center justify-between gap-4 animate-in slide-in-from-top duration-300">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-black text-amber-800 dark:text-amber-400">
              تنبيه حرج: هناك {metrics.lowStockCount} أصناف مخزنية انخفضت تحت حد الأمان المسموح به وقد تسبب تعطل المبيعات قريباً.
            </span>
          </div>
          <button 
            onClick={() => onTabChange && onTabChange("inventory")}
