...
import { 
  ArrowRightLeft, Layers, Plus, Search, Filter, HelpCircle, FileText,
  CheckCircle2, AlertTriangle, Play, ClipboardList, RefreshCw, Send,
  FileSpreadsheet, Trash2, ShieldCheck, Printer, Landmark
} from "lucide-react";
...
export default function OperationsAndTransfers({
  ...
  const selectedBOMComponents = useMemo(() => {
    ...
    return prod.bomComponents.map((c: any) => {
      ...
      return {
        ...
        currentStockInWh: componentItem ? Number(componentItem.warehouseQuantities?.[assemblyWh] || 0) : 0
      };
    });
  ...
  const assemblyReadiness = useMemo(() => {
    if (selectedBOMComponents.length === 0) return { ready: false, msg: "لا يوجد هيكل منتج (BOM) معرف لهذا الصنف" };
    ...
    selectedBOMComponents.forEach((comp: any) => {
      ...
      if (comp.currentStockInWh < requiredTotal) {
        allReady = false;
        missingParts.push(`${comp.name} (المطلوب: ${requiredTotal}, المتوفر: ${comp.currentStockInWh})`);
      }
    });

    if (allReady) {
      return { ready: true, msg: "جاهز للتجميع المباشر! جميع المدخلات والمواد الخام متوفرة في المستودع المحدد ✅" };
    } else {
  ...
  const handleAssembleBOM = async () => {
    ...
    try {
      ...
      selectedBOMComponents.forEach((comp: any) => {
        ...
        currentWhQuantities[assemblyWh] = Number(currentWhQuantities[assemblyWh] || 0) - requiredTotal;
      ...
      toast.success(`تم تجميع ${orderQty} حبة من الصنف (${finishedGood.nameAr}) وتحديث أرصدة المكونات فورياً! 🏭✅`);
      setAssemblyProduct("");
    } catch (err: any) {
  ...
  const generateTransferPdf = (transfer: any) => {
    try {
      ...
      doc.text(`Date Initiated: ${transfer.createdAt?.substring(0,10) || new Date().toISOString().substring(0,10)}`, 20, 48);
      doc.text(`Status: ${transfer.status === 'completed' ? 'DELIVERED / COMPLETED' : 'IN TRANSIT'}`, 20, 54);

      // Warehouse information
      const sourceWh = warehouses.find(w => w.id === transfer.sourceWarehouseId)?.nameAr || transfer.sourceWarehouseId;
      const destWh = warehouses.find(w => w.id === transfer.destinationWarehouseId)?.nameAr || transfer.destinationWarehouseId;
  ...
  const handleInitiateTransfer = async (e: React.FormEvent) => {
    ...
    if (availableStock < qty) {
      toast.error(`خطأ: الكمية المطلوبة للتحويل (${qty}) أكبر من المخزون المتوفر في المستودع المصدر (${availableStock})! ⚠️`);
      return;
    }
  ...
  const handleCompleteTransferClick = async (transfer: any) => {
    ...
    try {
      ...
      if (prod) {
        ...
        updatedDestQuantities[transfer.destinationWarehouseId] = Number(updatedDestQuantities[transfer.destinationWarehouseId] || 0) + Number(transfer.quantity);
            ...
            <form onSubmit={handleInitiateTransfer} className="space-y-4 text-xs font-bold">
              ...
              <div>
                ...
                >
                  ...
                  {items.map(i => {
                    const stock = transSourceWh ? Number(i.warehouseQuantities?.[transSourceWh] || 0) : 0;
                    return (
              ...
              <div>
                <label className="block text-zinc-400 mb-1">ملاحظات وسند الشحن (مهم للجمارك والضرائب)</label>
                <textarea
                  value={transNotes}
          ...
          <div className="lg:col-span-2 bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm flex flex-col justify-between">
            <div>
              ...
              <div className="space-y-4 max-h-[50vh] overflow-y-auto pr-1">
                {transfers.length === 0 ? (
                  <div className="py-12 text-center text-zinc-400 font-bold">لا توجد عمليات تحويل جارية حالياً.</div>
                ) : (
                  transfers.map((t) => {
                    const fromWh = warehouses.find(w => w.id === t.sourceWarehouseId)?.nameAr || t.sourceWarehouseId;
                    const toWh = warehouses.find(w => w.id === t.destinationWarehouseId)?.nameAr || t.destinationWarehouseId;
                    
                    return (
                      <div key={t.id} className="p-4 border border-zinc-100 dark:border-zinc-800/80 rounded-2xl flex items-center justify-between gap-4">
                        <div className="space-y-1">
                          <span className="text-[10px] font-mono font-black text-indigo-600">ID: {t.id.substring(0,8).toUpperCase()}</span>
                          <span className="font-black text-zinc-900 dark:text-zinc-100 block">{t.itemName} ({t.quantity} حبة)</span>
                          <div className="flex flex-wrap items-center gap-1.5 text-[10px] text-zinc-400">
                            <span>من: <strong className="text-zinc-600 dark:text-zinc-300">{fromWh}</strong></span>
                            <span>➔</span>
                            <span>إلى: <strong className="text-zinc-600 dark:text-zinc-300">{toWh}</strong></span>
                          </div>
                          {t.notes && <p className="text-[10px] text-zinc-400 italic">ملاحظة: {t.notes}</p>}
                        </div>

                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-0.5 rounded text-[9px] font-black ${t.status === "completed" ? "bg-emerald-50 text-emerald-700" : "bg-indigo-50 text-indigo-700 animate-pulse"}`}>
          ...
          <div className="lg:col-span-2 bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
            ...
            <div className="space-y-4 max-h-[55vh] overflow-y-auto pr-1">
              {adjustments.length === 0 ? (
                <div className="py-12 text-center text-zinc-400 font-bold">لا توجد تسويات جرد مسجلة.</div>
              ) : (
                adjustments.map((a, idx) => {
                  const matchingGl = GL_ACCOUNTS.find(g => g.code === a.glAccount)?.name || a.glAccount;
                  const matchingCc = COST_CENTERS.find(c => c.code === a.costCenter)?.name || a.costCenter;
                  return (
                    <div key={idx} className="p-4 border border-zinc-50 dark:border-zinc-800 rounded-2xl space-y-3 hover:shadow-md transition-all duration-300">
                      <div className="flex items-center justify-between">
                        <div>
                          <span className="font-black text-zinc-900 dark:text-zinc-100 block">{a.itemName}</span>
                          <span className="text-[10px] text-zinc-400 font-mono block">SKU: {a.sku} • مستودع: {warehouses.find(w => w.id === a.warehouseId)?.nameAr || a.warehouseId}</span>
                        </div>
                        <div className="text-left">
                          <span className={`px-2 py-1 rounded font-mono font-black text-xs block ${a.type === "add" ? "bg-emerald-50 text-emerald-700" : a.type === "subtract" ? "bg-red-50 text-red-700" : "bg-amber-50 text-amber-700"}`}>
                            {a.type === "add" ? "+" : a.type === "subtract" ? "-" : "="} {a.quantity} حبة
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="bg-zinc-50 dark:bg-zinc-800/40 p-2.5 rounded-xl grid grid-cols-2 gap-2 text-[10px] text-zinc-500 font-bold">
                        <div>
                          ...
                          <strong className="text-zinc-700 dark:text-zinc-300">{reasonTranslations[a.reason || ""] || a.reason}</strong>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <strong className="text-zinc-700 dark:text-zinc-300 font-sans">{matchingCc}</strong>
                        </div>
                        <div>
                          <span>المسؤول: </span>
                          <strong className="text-zinc-700 dark:text-zinc-300">أمين المستودع العام</strong>
                        </div>
                      </div>
                      
                      {a.notes && <p className="text-[10px] text-zinc-400 font-bold bg-zinc-50 dark:bg-zinc-800/30 p-2 rounded-lg italic">ملاحظة: {a.notes}</p>}
                    </div>
                  );
                })
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm space-y-4">
            ...
            <div className="space-y-4 text-xs font-bold">
              <div>
                <label className="block text-zinc-400 mb-1">المنتج النهائي المطلوب تجميعه (BOM Product)</label>
                <select
                  required
                ...
                >
                  ...
                  {items.filter(i => i.bomComponents && i.bomComponents.length > 0).map(i => (
                    <option key={i.id} value={i.id}>{i.nameAr} ({i.sku})</option>
                  ))}
                </select>
                <span className="text-[9px] text-zinc-400 mt-1 block">* يظهر فقط الأصناف التي تمتلك تركيبة مادية (BOM) مدخلة في النظام.</span>
              </div>
              ...
              <div>
                <label className="block text-zinc-400 mb-1">الكمية المستهدفة للتجميع (كمية الإنتاج)</label>
                <input
                  type="number"
                  value={assemblyQty}
              ...
              {assemblyProduct && (
                <div className="border border-indigo-100 p-4 rounded-2xl bg-indigo-50/20 space-y-2">
                  <h4 className="text-[10px] font-black text-indigo-600">تقرير جاهزية خط الإنتاج:</h4>
                  <p className="text-[10px] text-zinc-600 dark:text-zinc-300 leading-relaxed font-bold">{assemblyReadiness.msg}</p>
                </div>
              )}
          ...
          <div className="lg:col-span-2 bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
            <h3 className="text-xs font-black text-zinc-900 dark:text-zinc-100 mb-4 border-b pb-2">هيكلية المكونات والمواد اللازمة (Bill of Materials Breakdown)</h3>
            ...
            {assemblyProduct ? (
              <div className="space-y-4">
                <div className="bg-zinc-50 dark:bg-zinc-800 p-4 rounded-2xl flex items-center justify-between">
                  <div>
                    ...
                    <strong className="text-sm text-zinc-900 dark:text-zinc-100 block">{items.find(i => i.id === assemblyProduct)?.nameAr}</strong>
                  </div>
                      ...
                      {selectedBOMComponents.map((comp: any, idx: number) => {
                        ...
                        return (
                          <tr key={idx} className="border-b">
                            <td className="p-3 text-zinc-900 dark:text-zinc-100 font-black">{comp.name}</td>
                            <td className="p-3 font-mono">{comp.sku}</td>
                            <td className="p-3 font-mono text-zinc-500">{comp.quantity} حبة</td>
                            <td className="p-3 font-mono text-indigo-600 font-black">{neededTotal} حبة</td>
                            <td className="p-3 font-mono text-center">{comp.currentStockInWh} حبة</td>
                            <td className="p-3">
                              <span className={`px-2 py-0.5 rounded text-[9px] font-black inline-block ${hasSufficient ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>
              ...
              <div className="py-12 text-center text-zinc-400 font-bold flex flex-col items-center justify-center gap-2">
                ...
                يرجى تحديد صنف تجميعي من لوحة التحكم لإظهار المواد والمكونات المادية المكونة له (BOM) والتحقق من أرصدة المواد الخام ومستويات كفايتها.
              </div>
            )}
          </div>
