...
export default function KpiDashboard({ employees }: { employees: any[] }) {
  ...
  const wpsComplianceDetails = useMemo(() => {
    ...
    if (ibanCount < total) {
      issues.push(
        `يوجد عدد ${total - ibanCount} موظفاً بدون رقم حساب بنكي سعودي (IBAN) مكتمل (يبدأ بـ SA).`
      );
    }
    if (salaryFloorCount < total) {
      issues.push(
        `يوجد عدد ${total - salaryFloorCount} موظفاً يقل صافي أجرهم الإجمالي عن 3,000 ر.س (الحد الأدنى للتسجيل المحمي).`
      );
    }
  ...
  const handleWpsAudit = () => {
    ...
    setTimeout(() => {
      ...
      if (wpsComplianceDetails.issues.length === 0) {
        toast.success(
          "✅ تم الانتهاء من فحص نظام حماية الأجور (WPS): الامتثال 100% متطابق وجاهز للصرف البنكي!"
        );
      } else {
        toast.warning(
          `⚠️ فحص حماية الأجور (WPS): تم العثور على ${wpsComplianceDetails.issues.length} تنبيهات تتطلب المعالجة العاجلة لضمان عدم تأخر الصرف.`
        );
      }
    }, 1200);
  };
  ...
  const { perfect: perfectAttendance, totalActive } = useMemo(() => {
    const perfect = activeEmployees.filter(
      (e) => (e.absenceDays || 0) === 0 && (e.lateMinutes || 0) === 0
    ).length;
    return {
    ...
    return months.map((m, idx) => {
      ...
      return {
        ...
        attendance: Math.round(
          Math.min(100, Math.max(50, avgAttendance + attOffset * (1 - factor)))
        ),
      };
    });
      ...
      <div className="bg-zinc-900 p-8 rounded-[2rem] flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl relative overflow-hidden text-white">
        ...
        <div className="relative z-10 w-full md:w-auto">
          ...
          <p className="text-zinc-400 text-sm font-medium">
            مراقبة الأداء، الحضور، وتأثيرهما المباشر على الأجور والمكافآت.
          </p>
        </div>
      </div>
            ...
            <div className="flex justify-between items-start">
              <div className="space-y-1">
                <span className="text-[10px] font-black uppercase tracking-widest text-indigo-300">
                  موعد مسير الرواتب القادم
                </span>
                <h4 className="text-base font-black text-white">
                  إطلاق مسير شهر {new Date().toLocaleDateString("ar-SA", { month: "long" })}
                </h4>
              </div>
            ...
            <div className="space-y-2 pt-2 border-t border-white/5">
              <div className="flex justify-between text-xs text-zinc-400">
                ...
                <span className="font-bold text-white">
                  27 {new Date().toLocaleDateString("ar-SA", { month: "long", year: "numeric" })}
                </span>
              </div>
              <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.max(10, Math.min(100, (30 - daysUntilPayroll) * 3.3))}%`,
                  }}
                />
              </div>
            </div>
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-black text-white">
                    مؤشر الامتثال لنظام حماية الأجور (WPS)
                  </h3>
                </div>
                <p className="text-xs text-zinc-400">
                  تدقيق فوري لمطابقة كشوف الأجور مع متطلبات وزارة الموارد البشرية والتنمية
                  الاجتماعية.
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10 space-y-1.5">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xl font-black font-mono text-emerald-400">
                    {wpsComplianceDetails.overallPercent}%
                  </span>
              ...
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10 space-y-1.5">
                <span className="text-[10px] text-zinc-400 block">
                  اكتمال الحسابات البنكية (IBAN)
                </span>
                <div className="flex justify-between items-center">
                  <span className="text-xl font-black font-mono text-white">
                    {wpsComplianceDetails.ibanCount} / {activeEmployees.length}
                  </span>
                  <span className="text-[9px] bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 px-1.5 py-0.5 rounded-full font-bold">
                    {wpsComplianceDetails.ibanPercent}% مكتمل
                  </span>
                </div>
              </div>
              ...
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10 space-y-1.5">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xl font-black font-mono text-white">
                    {wpsComplianceDetails.salaryFloorCount} / {activeEmployees.length}
                  </span>
                  <span className="text-[9px] bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-1.5 py-0.5 rounded-full font-bold">
                    {wpsComplianceDetails.salaryFloorPercent}% ممتثل
                  </span>
                </div>
              </div>
            </div>
            ...
            {wpsComplianceDetails.issues.length > 0 ? (
              ...
              <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-200 rounded-2xl p-3.5 flex gap-3 items-center">
                ...
                <p className="text-[11px] font-bold leading-relaxed">
                  تهانينا! كافة ملفات وسجلات الموظفين النشطين متوافقة بالكامل مع قواعد نظام حماية
                  الأجور (WPS) والحد الأدنى للرواتب.
                </p>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <p className="text-emerald-600 text-sm font-bold mt-2 flex items-center gap-1">
              ...
              {activeEmployees.length > 0
                ? "محسوب ديناميكياً من السجلات الحالية"
                : "+3.2% من الشهر الماضي"}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-[2.5rem] border border-zinc-200 shadow-sm p-8">
          <h3 className="text-lg font-black text-zinc-900 mb-6">
            تطور أداء الشركة والحضور (أخر 6 أشهر)
          </h3>
          ...
          <div className="space-y-4 overflow-y-auto flex-1 pr-2">
            {activeEmployees.slice(0, 5).map((emp) => {
              ...
              return (
                <div
                  key={emp.id}
                  className="flex items-center justify-between p-4 rounded-2xl border border-zinc-100 hover:bg-zinc-50 transition cursor-pointer"
                >
                  <div>
                    <h4 className="font-bold text-zinc-900">
                      {emp.name || emp.nameAr || emp.nameEn || emp.email}
                    </h4>
                    <p className="text-xs text-zinc-500 font-medium">
                      {emp.department || "عام"} • {emp.position}
                    </p>
                  </div>
            ...
            {activeEmployees.length === 0 && (
              <div className="text-center py-10 text-zinc-400 font-bold">
                لا يوجد بيانات موظفين لعرض التقييمات.
              </div>
            )}
          </div>
        </div>
