...
export default function PayrollPortals({ employees }: PayrollPortalsProps) {
  ...
  const handleActionAdvance = async (id: string, status: "approved" | "rejected", req: any) => {
    try {
      ...
      toast.success(
        `تم ${status === "approved" ? "اعتماد" : "رفض"} السلفة وتحديث خصومات مسير الرواتب.`
      );
    } catch (err: any) {
      toast.error("فشل الإجراء: " + err.message);
    }
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-[2rem] border border-zinc-200 p-6 space-y-4">
              <label className="text-xs font-bold text-zinc-500">
                اختر موظفاً لعرض حساب الخدمة الذاتية
              </label>
              <select
                ...
                <form onSubmit={handleApplyAdvance} className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAdvanceForm({ ...advanceForm, amount: Number(e.target.value) })
                      }
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-zinc-500">
                      عدد الشهور المقترحة للتسديد
                    </label>
                    <input
                      type="number"
                      required
                      ...
                      onChange={(e) =>
                        setAdvanceForm({ ...advanceForm, installments: Number(e.target.value) })
                      }
                    />
                  </div>
                  <button
            ...
            {selectedEmp ? (
              <div className="bg-zinc-50 rounded-[2.5rem] border border-zinc-200 p-8 space-y-6">
                <div className="flex justify-between items-center border-b pb-6">
                  <div>
                    <h3 className="text-xl font-black text-zinc-900">
                      كشف الراتب الرسمي والمستحقات
                    </h3>
                    <p className="text-xs text-zinc-500 font-medium">
                      مستند إلكتروني معد بنظام حماية الأجور والآيبان المرتبط
                    </p>
                  </div>
                  <button
                    onClick={() => window.print()}
                    className="px-4 py-2 text-xs font-bold text-zinc-700 bg-white border rounded-xl hover:bg-zinc-50"
                  >
                    طباعة كشف الراتب
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white p-4 rounded-2xl border">
                    ...
                    <p className="text-lg font-black text-zinc-900">
                      {((selectedEmp.baseSalaryHalalas || 0) / 100).toLocaleString()} ر.س
                    </p>
                  </div>
                  <div className="bg-white p-4 rounded-2xl border">
                    <span className="text-[10px] font-bold text-zinc-400">
                      بدل السكن والنقل والهاتف
                    </span>
                    <p className="text-lg font-black text-blue-600">
                      {(
                        ((selectedEmp.housingAllowanceHalalas || 0) +
                          (selectedEmp.transportAllowanceHalalas || 0) +
                          (selectedEmp.phoneAllowanceHalalas || 0)) /
                        100
                      ).toLocaleString()}{" "}
                      ر.س
                    </p>
                ...
                <div className="bg-white rounded-3xl border p-6 space-y-4">
                  <h4 className="font-black text-zinc-900 text-xs border-b pb-2 mb-2">
                    تفصيل كشف الدفع الحالي
                  </h4>
                  ...
                  <div className="space-y-3 text-xs">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold">
                        +{((selectedEmp.natureOfWorkAllowanceHalalas || 0) / 100).toLocaleString()}{" "}
                        ر.س
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-zinc-500 font-semibold">
                        تأمين اجتماعي GOSI اقتطاع (9.75%)
                      </span>
                      <span className="font-extrabold text-rose-500">
                        -
                        {(
                          (((selectedEmp.baseSalaryHalalas || 0) +
                            (selectedEmp.housingAllowanceHalalas || 0)) /
                            100) *
                          0.0975
                        ).toFixed(2)}{" "}
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-zinc-500 font-semibold">
                        خصومات الحضور والغيابات (المرتبطة بالبصمة)
                      </span>
                    ...
                    {(() => {
                      ...
                      return (
                        <span className="text-xl text-emerald-600">
                          {Math.round(net).toLocaleString()} ر.س
                        </span>
                      );
                    })()}
                  </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-[2rem] border border-zinc-200 p-8 space-y-6">
            <h4 className="text-lg font-black text-zinc-900 flex items-center gap-2">
              ...
              طلبات سلف الموظفين المعلقة للحساب
            </h4>
            ...
            <div className="space-y-4">
              {advances
                ...
                .map((req) => {
                  ...
                  return (
                    <div
                      key={req.id}
                      className="p-4 bg-zinc-50 border border-zinc-200 rounded-2xl flex flex-col gap-4"
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-extrabold text-sm text-zinc-900">{req.employeeName}</p>
                          <p className="text-xs text-zinc-500 font-medium mt-1">
                            يطلب سلفة قدرها {req.amount.toLocaleString()} ر.س مقسمة على{" "}
                            {req.installments} أشهر تسديد.
                          </p>
                        </div>
                        <div className="flex gap-2">
                          <button
                            onClick={() => handleActionAdvance(req.id, "approved", req)}
                            className="p-2 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 flex items-center gap-2 font-bold text-xs"
                          >
                            <Check className="w-4 h-4" /> قبول الجدولة
                          </button>
                          <button
                            onClick={() => handleActionAdvance(req.id, "rejected", req)}
                            className="p-2 bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-100 flex items-center gap-2 font-bold text-xs"
                          >
                      ...
                      <div className="bg-white border rounded-xl p-3 flex justify-between items-center mt-2">
                        <span className="text-[10px] font-bold text-zinc-400">
                          جدولة الاستقطاع الشهري (Amortization Schedule)
                        </span>
                        <span className="text-xs font-black text-rose-600">
                          - {monthlyInstallment.toLocaleString()} ر.س تخصم من راتب الموظف في كل مسير
                          القادم
                        </span>
                      </div>
                    </div>
              ...
              {advances.filter((a) => a.status === "pending").length === 0 && (
                <div className="text-center py-6 text-xs text-zinc-400 font-bold border-2 border-dashed rounded-xl">
                  لا توجد طلبات سلف جديدة حالياً.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-[2rem] border border-zinc-200 p-8 space-y-6">
            <h4 className="text-lg font-black text-zinc-900 flex items-center gap-2">
              ...
              طلبات الإجازات السنوية والمرضية المعالجة
            </h4>
            ...
            <div className="space-y-4">
                ...
                .map((req) => (
                  ...
                  >
                    <div>
                      <p className="font-extrabold text-sm text-zinc-900">{req.employeeName}</p>
                      <p className="text-xs text-zinc-500 font-medium mt-1">
                        يطلب إجازة {req.leaveType === "Annual" ? "سنوية" : "مرضية"} من تاريخ{" "}
              ...
              {leaves.filter((l) => l.status === "pending").length === 0 && (
                <div className="text-center py-6 text-xs text-zinc-400 font-bold border-2 border-dashed rounded-xl">
                  لا توجد إجازات جديدة معلقة للموافقة.
                </div>
              )}
            </div>
          </div>
