...
export default function PayrollAttendanceSim({ employees }: PayrollAttendanceSimProps) {
  ...
  useEffect(() => {
    ...
    employees.forEach((emp) => {
      initial[emp.id] = {
        ...
        overtimeMultiplier:
          typeof emp.overtimeMultiplier !== "undefined" ? emp.overtimeMultiplier : 1.5,
        commissionAmount:
          typeof emp.commissionAmount !== "undefined"
            ? emp.commissionAmount
            : emp.commissionHalalas
              ? emp.commissionHalalas / 100
              : 0,
  ...
  const handleSimulateCheckIn = () => {
    ...
    activeEmployees.forEach((emp) => {
      ...
      simulated[emp.id] = {
        ...
        adjustmentNotes:
          seed % 4 === 0
            ? "ساعات عمل إضافية لإنهاء الربع الثاني"
            : seed % 9 === 0
              ? "غياب بدون عذر مقبول"
              : "تسوية نظامية",
      };
  ...
  const handleResetAll = () => {
    if (
      !confirm("هل أنت متأكد من رغبتك في إعادة تعيين جميع قيم التسويات والخصومات لجميع الموظفين؟")
    )
      return;
  ...
  const handleInjectIntoPayroll = async () => {
    ...
    try {
      ...
      toast.success(
        "تم حفظ وتطبيق جميع التسويات، الخصومات، والعمل الإضافي بنجاح! ☁️ ستنعكس في مسير الرواتب القادم."
      );
    } catch (e: any) {
      console.error(e);
  ...
  return (
    <div
      className="bg-white rounded-[2.5rem] border border-zinc-200 shadow-sm p-8 space-y-6 select-none"
      dir="rtl"
    >
      {/* Header Block */}
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b border-zinc-100 pb-6">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="px-2.5 py-0.5 bg-indigo-50 text-indigo-600 border border-indigo-100 text-[10px] font-black rounded-lg">
              Saudi Labor Law v2026
            </span>
            <span className="px-2.5 py-0.5 bg-emerald-50 text-emerald-600 border border-emerald-100 text-[10px] font-black rounded-lg">
              WPS & GOSI Aligned
            </span>
          </div>
          ...
          <p className="text-xs text-zinc-500 font-medium mt-1 leading-relaxed">
            تمكين المدراء من تعديل وتخصيص ساعات العمل الإضافي ومضاعفاتها، أيام الخصم والغياب،
            العمولات الإضافية والمكافآت، مع الحساب التلقائي اللحظي ومزامنتها سحابياً.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-zinc-50 p-5 rounded-[2rem] border border-zinc-200/60 shadow-xs">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-black text-zinc-400 uppercase">
              إجمالي مكافآت العمل الإضافي
            </span>
            <Clock className="w-4 h-4 text-emerald-500" />
          </div>
          <div className="text-lg font-black text-zinc-900 mt-1">
            +{Math.round(totals.overtimeSum).toLocaleString()}{" "}
            <span className="text-[10px] font-bold text-zinc-400">ر.س</span>
          </div>
          <div className="text-[9px] font-bold text-emerald-600 mt-1">
            بمعدل احتساب 1.5x فأكثر قانونياً
          </div>
        </div>
        ...
        <div className="bg-zinc-50 p-5 rounded-[2rem] border border-zinc-200/60 shadow-xs">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-black text-zinc-400 uppercase">
              إجمالي العمولات والمكافآت
            </span>
            <TrendingUp className="w-4 h-4 text-indigo-500" />
          </div>
          <div className="text-lg font-black text-zinc-900 mt-1">
            +{Math.round(totals.commissionSum).toLocaleString()}{" "}
            <span className="text-[10px] font-bold text-zinc-400">ر.س</span>
          </div>
          <div className="text-[9px] font-bold text-indigo-600 mt-1">
            بما يشمل حوافز المبيعات والمكافآت
          </div>
        </div>
        ...
        <div className="bg-zinc-50 p-5 rounded-[2rem] border border-zinc-200/60 shadow-xs">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-black text-rose-400 uppercase">
              إجمالي الخصومات والغيابات
            </span>
            <ShieldAlert className="w-4 h-4 text-rose-400" />
          </div>
          <div className="text-lg font-black text-rose-600 mt-1">
            -{Math.round(totals.deductionSum).toLocaleString()}{" "}
            <span className="text-[10px] font-bold text-rose-400">ر.س</span>
          </div>
          <div className="text-[9px] font-bold text-rose-500 mt-1">
            إجمالي أيام الخصم: {totals.totalAbsenceDays} أيام
          </div>
        </div>
                ...
                <tr key={emp.id} className="hover:bg-zinc-50/40 transition-colors group">
                  ...
                  <td className="p-4">
                    ...
                    <div className="text-[10px] text-zinc-400 font-medium mt-0.5">
                      {emp.position} • {emp.department || "غير محدد"}
                    </div>
                    ...
                    <div className="flex gap-2 mt-2">
                      ...
                      <span
                        className="text-[9px] font-mono text-zinc-400 bg-zinc-50 border border-zinc-100 px-1.5 py-0.5 rounded"
                        title="معدل الأجر اليومي = الأساسي / 30"
                      >
                        اليومي: {Math.round(calcs.dailyWage)} ر.س
                      </span>
                      <span
                        className="text-[9px] font-mono text-zinc-400 bg-zinc-50 border border-zinc-100 px-1.5 py-0.5 rounded"
                        title="معدل أجر الساعة = اليومي / 8"
                      >
                  ...
                  <td className="p-4 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      <input
                        ...
                        placeholder="0"
                        onChange={(e) =>
                          updateEmpAtt(emp.id, "absenceDays", Math.max(0, Number(e.target.value)))
                        }
                      />
                  ...
                  <td className="p-4 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      <input
                        ...
                        placeholder="0"
                        onChange={(e) =>
                          updateEmpAtt(emp.id, "lateMinutes", Math.max(0, Number(e.target.value)))
                        }
                      />
                  ...
                  <td className="p-4 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      <input
                        ...
                        placeholder="0"
                        onChange={(e) =>
                          updateEmpAtt(emp.id, "overtimeHours", Math.max(0, Number(e.target.value)))
                        }
                      />
                  ...
                  <td className="p-4 text-center">
                    <select
                      ...
                      onChange={(e) =>
                        updateEmpAtt(emp.id, "overtimeMultiplier", Number(e.target.value))
                      }
                    >
                  ...
                  <td className="p-4 text-center">
                    <div className="relative">
                      ...
                      />
                      <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[9px] font-bold text-zinc-400">
                        ر.س
                      </span>
                    </div>
                  </td>
                      ...
                      if (netImpact > 0) {
                        return (
                          <span className="text-emerald-600 bg-emerald-50 px-2 py-1 rounded text-xs">
                            +{Math.round(netImpact).toLocaleString()} ر.س
                          </span>
                        );
                      } else if (netImpact < 0) {
                        return (
                          <span className="text-rose-500 bg-rose-50 px-2 py-1 rounded text-xs">
                            {Math.round(netImpact).toLocaleString()} ر.س
                          </span>
                        );
                      } else {
