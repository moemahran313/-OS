...
export default function InvoiceBuilder({ onSave, onCancel, initialData }: InvoiceBuilderProps) {
  ...
  const [paymentTerms, setPaymentTerms] = useState(initialData?.paymentTerms || "الرجاء الدفع خلال ١٥ يوماً من تاريخ الفاتورة. تطبق رسوم تأخير بنسبة ٢٪ شهرياً.");
  ...
  const [branding, setBranding] = useState<InvoiceBranding>(initialData?.branding || {
    primaryColor: "#10b981",
    template: "modern",
    bilingual: true,
  });
  ...
  const [lateFee, setLateFee] = useState<{ type: 'fixed' | 'percentage'; value: number; overdueDays: number }>(
    initialData?.lateFee ? { 
      type: initialData.lateFee.type, 
      value: initialData.lateFee.type === 'fixed' ? (initialData.lateFee.valueHalalas / 100) : (initialData.lateFee.valueHalalas || 0),
      overdueDays: initialData.lateFee.overdueDays || 0
    } : { type: 'percentage', value: 0, overdueDays: 0 }
  );
  const [statusConfig, setStatusConfig] = useState<Record<string, { label: string, color: string }>>(initialData?.statusConfig || {
  ...
  const [zatcaConfig, setZatcaConfig] = useState(initialData?.zatcaConfig || {
    sellerVat: "",
    sellerName: "",
    buyerVat: "",
    isPhasedTwo: true,
  ...
  }, [clientName, clientEmail, dueDate, currency, paymentTerms, notes, lineItems, branding, numberFormat, lateFee]);
  ...
  const addLineItem = () => {
    setLineItems([...lineItems, { id: Date.now().toString(), name: "", quantity: 1, unitPriceHalalas: 0, taxRate: 15, totalHalalas: 0 }]);
  };
  ...
  const updateCustomField = (itemId: string, fieldIndex: number, field: 'key' | 'value', value: string) => {
    setLineItems(lineItems.map(item => {
      if (item.id === itemId) {
  ...
  const updateLineItem = (id: string, field: string, value: any) => {
    const updated = lineItems.map(item => {
      if (item.id === id) {
        ...
        const priceHalalas = field === 'unitPriceHalalas' ? Math.round(Number(value) * 100) : newItem.unitPriceHalalas;
    ...
    img.onload = () => {
      ...
      try {
        ...
        for (let i = 0; i < data.length; i += 4) {
          const a = data[i + 3];
          if (a > 200) { // Only high opacity pixels
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2">
    ...
    try {
       ...
       await fetch("/api/audit-logs", {
         ...
         body: JSON.stringify({
           module: "INVOICE",
           action: "Send Invoice",
           payload: { invoiceNumber: currentDraft.number, clientName: currentDraft.clientName, email: clientEmail }
         })
       });
  ...
  const restoreDraft = (draft: any) => {
    ...
    setZatcaConfig(draft.zatcaConfig || { sellerVat: "", sellerName: "", buyerVat: "", isPhasedTwo: true, certificate: "", prevHash: "" });
      ...
      <div className="flex-1 overflow-y-auto flex">
        ...
        <div className="flex-1 p-8 space-y-12 border-l border-zinc-100">
          <DragDropContext onDragEnd={(result) => {
            ...
            <Droppable droppableId="builder-sections">
              {(provided) => (
                <div {...provided.droppableProps} ref={provided.innerRef} className="space-y-12 relative pb-20">
                        ...
                        {(provided, snapshot) => (
                          <section 
                            ...
                            className={cn(
                              ...
                              snapshot.isDragging ? "shadow-2xl border-primary/30 z-50 scale-[1.02]" : "border-transparent hover:border-zinc-100 hover:shadow-sm"
                            )}
                          >
                          {/* Drag/Move Handles */}
                          <div className="absolute -left-10 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover/section:opacity-100 transition-opacity">
                            ...
                            <div {...provided.dragHandleProps} className="p-1.5 text-zinc-300 cursor-grab active:cursor-grabbing hover:text-primary">
                ...
                {sectionId === 'branding' && (
                  <div className="p-6 bg-zinc-50 border border-zinc-100 rounded-3xl space-y-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-lg font-black text-zinc-900">هوية الفاتورة (Header)</h3>
                        <p className="text-xs font-bold text-zinc-500">اختر شعار منشأتك والألوان والنمط العام</p>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="space-y-4">
                        <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest px-1">شعار المنشأة</label>
                        <div className="flex flex-col gap-3">
                          ...
                          <p className="text-[10px] text-zinc-400 text-center font-bold">سيتم استخراج اللون الأساسي تلقائياً عند رفع الشعار</p>
                        </div>
                      </div>

                      <div className="space-y-4">
                        <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest px-1">اللون والموضوع كود HEX</label>
                      ...
                      <div className="space-y-4">
                        <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest px-1">نمط القالب</label>
                        <select 
                          value={branding.template}
                          onChange={(e) => setBranding({...branding, template: e.target.value as any})}
                        ...
                        <div className="flex items-center justify-between px-2">
                           <span className="text-[10px] font-black text-zinc-400">لغة الفاتورة الثنائية (Ar/En)</span>
                           <button 
                ...
                {sectionId === 'details' && (
                  <div className="grid grid-cols-2 gap-6 p-4">
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider">العميل / المؤسسة</label>
                      <input 
                        type="text" 
                        value={clientName}
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider">البريد الإلكتروني للعميل</label>
                      <input 
                        type="email" 
                        value={clientEmail}
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider">إرسال الفاتورة إلى (اختياري)</label>
                      <input 
                        type="email" 
                        value={billingEmail}
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider">تاريخ الاستحقاق</label>
                      <input 
                        type="date" 
                        value={dueDate}
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider">العملة</label>
                      <select 
                         value={currency}
                      ...
                      {lineItems.map((item, index) => (
                        <div key={item.id} className="space-y-3">
                          <div className="grid grid-cols-12 gap-3 items-start group">
                            <div className="col-span-1 py-3 text-zinc-400 font-black text-center">{index + 1}</div>
                            ...
                            <div className="col-span-2 flex items-center justify-between gap-2 pl-2">
                              <span className="text-xs font-black text-zinc-900">{(item.totalHalalas / 100).toLocaleString()}</span>
                              ...
                              {item.customFields.map((cf, cfi) => (
                                <div key={cfi} className="flex items-center gap-2">
                                  <div className="flex-1 space-y-1">
                                    <label className="text-[8px] font-black text-zinc-400 px-1 uppercase tracking-tighter">اسم الحقل (مثال: Project Code)</label>
                                    <input 
                                      type="text" 
                                      value={cf.key}
                                  ...
                                  <div className="flex-1 space-y-1">
                                    <label className="text-[8px] font-black text-zinc-400 px-1 uppercase tracking-tighter">القيمة</label>
                                    <input 
                                      type="text" 
                                      value={cf.value}
                ...
                {sectionId === 'terms' && (
                   <div className="p-4">
                     <div className="space-y-2">
                        <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider">شروط الدفع</label>
                        <input 
                          type="text" 
                          value={paymentTerms}
                ...
                {sectionId === 'notes' && (
                   <div className="p-4">
                     <div className="space-y-2">
                        <label className="text-xs font-bold text-zinc-400 uppercase tracking-wider">ملاحظات إضافية</label>
                        <textarea 
                          value={notes}
        ...
        <aside className="w-80 p-8 bg-zinc-50/50 flex flex-col gap-8 overflow-y-auto no-scrollbar">
          <section className="space-y-4">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-[11px] font-medium text-zinc-500">اللغة الثنائية (Ar/En)</span>
                <button 
                   onClick={() => setBranding({ ...branding, bilingual: !branding.bilingual })}
                   className={cn(
                    "w-10 h-5 rounded-full transition-all relative",
              ...
              <div className="space-y-4">
                 <span className="text-[11px] font-black text-zinc-400 uppercase tracking-widest">نمط التصميم</span>
                 
                 {/* Live Preview Miniature */}
                 <div className="p-4 rounded-3xl bg-white border border-zinc-200 shadow-sm overflow-hidden scale-75 -mx-8 -my-6 origin-center rotate-3 border-t-4" style={{ borderTopColor: branding.primaryColor }}>
                    <div className="flex justify-between items-start mb-4">
                       <div className="w-12 h-12 bg-zinc-100 rounded-lg flex items-center justify-center overflow-hidden">
                          {branding.logo ? <img src={branding.logo} className="w-full h-full object-contain" /> : <Palette className="w-6 h-6 text-zinc-300" />}
                       </div>
                    ...
                    <div className="flex gap-1 justify-end">
                       <div className="w-8 h-4 rounded-md" style={{ backgroundColor: branding.primaryColor + '20' }} />
                       <div className="w-12 h-4 rounded-md" style={{ backgroundColor: branding.primaryColor }} />
                    </div>
                 </div>
                 ...
                 <div className="grid grid-cols-1 gap-2 mt-4">
                    {[
                      ...
                      >
                        ...
                        <span className={cn("text-[9px] font-bold opacity-60", branding.template === t.id ? "text-primary" : "text-zinc-400")}>{t.desc}</span>
                      </button>
                    ))}
                 </div>
              </div>
              ...
              <div className="space-y-2 mt-4">
                <span className="text-[11px] font-medium text-zinc-500">رابط الشعار المخصص أو رفعه</span>
                  ...
                  <div className="flex gap-1">
                    {["#10b981", "#3b82f6", "#6366f1", "#f43f5e", "#f59e0b"].map(c => (
                      <button 
                        key={c}
                        onClick={() => setBranding({...branding, primaryColor: c})}
                        className={cn("w-3 h-3 rounded-full border border-white/20", branding.primaryColor === c && "ring-2 ring-zinc-900 ring-offset-2")}
                        style={{ backgroundColor: c }}
                      />
                    ))}
                  </div>
              ...
              <div className="space-y-4 pt-4 border-t border-zinc-100">
                <span className="text-[11px] font-black text-zinc-400 uppercase tracking-widest">تنسيق رقم الفاتورة</span>
                <div className="space-y-2">
                   <input 
                     type="text" 
                     value={numberFormat}
                   ...
                   />
                   <p className="text-[9px] text-zinc-400">استخدم {'{YYYY}'} للسنة و {'{SEQ}'} للتسلسل.</p>
                </div>
              </div>

              <div className="space-y-4 pt-4 border-t border-zinc-100">
                <span className="text-[11px] font-black text-zinc-400 uppercase tracking-widest">تخصيص الحالات والخدمات</span>
                <div className="space-y-3">
                  {(Object.entries(statusConfig) as [string, { label: string, color: string }][]).map(([key, config]) => (
                    <div key={key} className="flex gap-2 items-center">
                      ...
                      <input 
                        type="color" 
                        value={config.color}
                        onChange={(e) => setStatusConfig({ ...statusConfig, [key]: { ...config, color: e.target.value } })}
              ...
              <button 
                onClick={() => setBranding({ primaryColor: "#10b981", template: "modern", bilingual: true, logo: undefined })}
              ...
              <div className="space-y-4 pt-4 border-t border-zinc-100">
                <span className="text-[11px] font-black text-zinc-400 uppercase tracking-widest">شروط الدفع الافتراضية</span>
                <div className="space-y-2">
                   <textarea 
                     value={paymentTerms} 
                     onChange={(e) => setPaymentTerms(e.target.value)}
          ...
          <footer className="mt-auto space-y-4 border-t border-zinc-100 pt-6">
              ...
              <div className="p-4 bg-zinc-900 rounded-2xl border border-white/10 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">رابط الدفع المباشر</span>
                  </div>
                  <button 
                    onClick={() => {
            ...
            <div className="flex justify-between items-center text-xs">
              ...
              <span className="font-medium">{vatAmount.toLocaleString()} {currency}</span>
            </div>
            {lateFeeAmount > 0 && (
              <div className="flex justify-between items-center text-xs text-rose-600 font-bold">
                ...
                <span>{lateFeeAmount.toLocaleString()} {currency}</span>
              </div>
            )}
            <div className="flex justify-between items-center">
              <span className="text-sm font-bold text-zinc-900">الإجمالي</span>
              <span className="text-lg font-bold text-primary">{totalAmount.toLocaleString()} {currency}</span>
            </div>
            <div className="space-y-2 mb-4">
              <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest px-1">ملاحظة النسخة (اختياري)</label>
              <input 
                type="text"
                value={versionNote}
        ...
        <div className="absolute inset-0 bg-zinc-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {versions.length === 0 ? (
                <div className="text-center text-zinc-400 font-bold py-8">لا توجد مسودات سابقة.</div>
              ) : (
                <div className="space-y-4">
                  {versions.map((v, i) => (
                    <div key={i} className="group p-5 rounded-3xl bg-zinc-50 border border-zinc-100 hover:border-primary/30 transition-all flex justify-between items-center relative overflow-hidden">
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                             <p className="font-black text-zinc-900">فاتورة #{v.data.number || 'جديدة'}</p>
                             <div className="flex items-center gap-2">
                               <p className="text-xs text-zinc-500 font-bold">{new Date(v.timestamp).toLocaleString("ar-SA")}</p>
                               <span className="w-1 h-1 bg-zinc-300 rounded-full" />
                               <p className="text-xs text-primary font-black uppercase tracking-widest">{v.user}</p>
                             </div>
                             {v.note && (
                               <div className="mt-2 flex items-start gap-2 bg-white/50 p-2 rounded-xl border border-zinc-200/50">
                                  ...
                                  <p className="text-[10px] text-zinc-600 font-bold leading-relaxed">{v.note}</p>
                               </div>
                             )}
                          </div>
                       </div>
                       <div className="flex items-center gap-3">
                          <div className="text-left">
                             <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">الإجمالي</p>
                             <p className="text-sm font-black text-zinc-900">{v.data.totalAmount?.toLocaleString()} {v.data.currency}</p>
                          </div>
                          <button 
        ...
        <div className="absolute inset-0 bg-zinc-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <header className="p-6 border-b border-zinc-100 flex justify-between items-center bg-zinc-50/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-zinc-500">تخصيص تنسيق الترقيم، الرسوم، والحالات</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-8 overflow-y-auto flex-1 space-y-8">
              {/* Number Format */}
              <section className="space-y-4">
                <h3 className="text-xs font-black text-zinc-400 uppercase tracking-widest">تنسيق رقم الفاتورة</h3>
                ...
                <p className="text-[10px] text-zinc-400">استخدم {"{YYYY}"} للسنة و {"{SEQ}"} للتسلسل الرقمي.</p>
              </section>

              {/* Custom Payment Link */}
              <section className="space-y-4">
                <h3 className="text-xs font-black text-zinc-400 uppercase tracking-widest">رابط دفع مخصص (Custom Payment Link)</h3>
                <div className="space-y-2">
                  <input 
                    type="text" 
                    value={branding.customPaymentLink || ''} 
                    onChange={(e) => setBranding({ ...branding, customPaymentLink: e.target.value })}
                  ...
                  />
                  <p className="text-[10px] text-zinc-400 font-medium">إذا تم تحديد هذا الرابط، سيتجاهل النظام رابط الدفع الافتراضي للفاتورة.</p>
                </div>
              </section>

              {/* Late Payment Fee */}
              <section className="space-y-4">
                <h3 className="text-xs font-black text-zinc-400 uppercase tracking-widest">رسوم التأخر في السداد</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex bg-zinc-100 p-1 rounded-xl">
                      <button 
                         ...
                         className={cn(
                           ...
                           lateFee.type === 'percentage' ? "bg-white shadow-sm text-zinc-900" : "text-zinc-500"
                         )}
                      >نسبة مئوية (%)</button>
                      <button 
                         ...
                         className={cn(
                           "flex-1 py-1.5 rounded-lg text-[10px] font-bold transition-all",
                           lateFee.type === 'fixed' ? "bg-white shadow-sm text-zinc-900" : "text-zinc-500"
                         )}
                       >مبلغ ثابت ({currency})</button>
                    </div>
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="col-span-2 space-y-2">
                    <label className="text-[10px] font-bold text-zinc-500">تطبق الرسوم بعد (عدد الأيام المتأخرة)</label>
                    <div className="flex items-center gap-3">
                      <input 
                        type="number" 
                        ...
                        placeholder="0"
                      />
                      <span className="text-xs font-bold text-zinc-400">يوماً من تاريخ الاستحقاق</span>
                    </div>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-4">
                ...
                <div className="space-y-3">
                  {(Object.entries(statusConfig) as [string, { label: string, color: string }][]).map(([key, config]) => (
                    <div key={key} className="flex items-center gap-4 bg-zinc-50 p-3 rounded-2xl border border-zinc-100">
                      <div className="w-8 h-8 rounded-full shadow-inner" style={{ backgroundColor: config.color }} />
